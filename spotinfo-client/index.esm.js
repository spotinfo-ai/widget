...
var hc = (i, e, t) => e in i ? uc(i, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : i[e] = t;
var x = (i, e, t) => hc(i, typeof e != "symbol" ? e + "" : e, t);
class es {
  constructor() {
...
class Sn extends Je {
  constructor(e, t, n) {
    super(e, "AUTHENTICATION_ERROR", t, n), this.name = "AuthenticationError", Object.setPrototypeOf(this, Sn.prototype);
  }
}
class Oe extends Je {
  constructor(t, n, s) {
    ...
    x(this, "field");
    this.name = "ValidationError", this.field = n, Object.setPrototypeOf(this, Oe.prototype);
  }
}
class Bt extends Je {
  constructor(e, t) {
    super(e, "NETWORK_ERROR", void 0, t), this.name = "NetworkError", Object.setPrototypeOf(this, Bt.prototype);
  }
}
class Cn extends Je {
  constructor(t, n, s) {
    ...
    x(this, "retryAfter");
    this.name = "RateLimitError", this.retryAfter = n, Object.setPrototypeOf(this, Cn.prototype);
  }
}
class qt extends Je {
  constructor(e, t) {
    super(e, "VOICE_CONNECTION_ERROR", void 0, t), this.name = "VoiceConnectionError", Object.setPrototypeOf(this, qt.prototype);
  }
}
const mc = "https://api.spotinfo.ai", fc = "https://chat.spotinfo.ai";
function En() {
  return mc;
}
function an() {
...
async function et(i, e = {}) {
  ...
  if (o === !0)
    return console.log("[apiRequest] ✅ Mock mode enabled - returning mock response (NO backend call)", { url: i, method: t }), gc(i, t, s);
  console.log("[apiRequest] ⚠️ Mock mode disabled - making real backend request", {
    url: i,
    method: t,
    retryAttempts: r,
    hasBody: !!s,
    hasApiKey: !!n["x-api-key"]
  });
  let c = null;
  for (let d = 0; d <= r; d++) {
    d > 0 && console.log(`[apiRequest] Retry attempt ${d}/${r}`);
    try {
      console.log(`[apiRequest] Fetching ${t} ${i} (attempt ${d + 1})`);
      ...
      if (console.log("[apiRequest] Response received:", {
        status: l.status,
        statusText: l.statusText,
        ok: l.ok,
        url: l.url
      }), l.status === 429) {
        console.error("[apiRequest] Rate limit exceeded (429)");
        const u = l.headers.get("Retry-After"), h = u ? parseInt(u, 10) : void 0;
        throw new Cn(
          "Rate limit exceeded",
          h,
          {
            limit: l.headers.get("X-RateLimit-Limit"),
            remaining: l.headers.get("X-RateLimit-Remaining"),
            resetAt: l.headers.get("X-RateLimit-Reset")
          }
        );
      }
      if (l.status === 401)
        throw console.error("[apiRequest] Authentication failed (401)"), new Sn("Authentication failed", l.status);
      if (l.status === 400) {
        console.error("[apiRequest] Validation error (400)");
        const u = await l.json().catch(() => ({}));
        throw console.error("[apiRequest] Error data:", u), new Oe(
          u.message || "Validation error",
          u.field,
          u
        );
      }
      ...
      return console.log("[apiRequest] Request successful"), l;
    } catch (l) {
      if (c = l, console.error(`[apiRequest] Attempt ${d + 1} failed:`, l), l instanceof Sn || l instanceof Oe || l instanceof Cn)
        throw console.error("[apiRequest] Non-retryable error, throwing immediately"), l;
      if (d < r) {
        const u = a * (d + 1);
        console.log(`[apiRequest] Will retry after ${u}ms`), await new Promise((h) => setTimeout(h, u));
        continue;
      }
      throw console.error(`[apiRequest] All ${r + 1} attempts failed`), l instanceof Bt || l instanceof Je ? l : new Bt(
        `Request failed after ${r + 1} attempts: ${l instanceof Error ? l.message : "Unknown error"}`,
        { originalError: l }
      );
    }
  }
  throw console.error("[apiRequest] All retry attempts exhausted"), c || new Bt("Request failed");
}
function pc() {
...
async function gc(i, e, t) {
  const {
    generateMockRoomCredentials: n,
    createMockResponse: s,
    recordTrackingStart: r,
    shouldDeliverMockHook: a,
    getMockHookMessage: o
  } = await Promise.resolve().then(() => ts);
  if (i.includes("/ensure_rag"))
    return s({ status: "success" }, 200);
  if (i.includes("/create-room")) {
    const c = (t == null ? void 0 : t.client_id) || "mock-client", d = n(c);
    return s({
  ...
  if (i.includes("/delete-room"))
    return s({ status: "success" }, 200);
  if (i.includes("/dispatch-agent"))
    return s({ status: "success" }, 200);
  if (i.includes("register_user_journey"))
    return (t == null ? void 0 : t.event) === "widget_loaded" && (t != null && t.user_id) ? (r(t.user_id), console.log("[Mock] register_user_journey: widget_loaded event received, tracking start recorded (no backend call)")) : console.log("[Mock] register_user_journey: Event tracked locally (NOT sent to backend)", {
      event: t == null ? void 0 : t.event,
      userId: t == null ? void 0 : t.user_id
    }), s({ status: "success" }, 200);
  if (i.includes("get_user_journey_hook")) {
    const c = i.match(/get_user_journey_hook[\/]+([^\/]+)/), d = c ? c[1] : null;
    if (d && a(d)) {
      const l = o();
      return console.log("[Mock] get_user_journey_hook: Returning mock hook (no backend call)", { userId: d, message: l }), s({ message: l }, 200);
    } else
      return console.log("[Mock] get_user_journey_hook: No hook yet, returning 204 (no backend call)", { userId: d }), new Response(null, { status: 204 });
  }
  return s({ status: "success" }, 200);
}
async function vc(i, e, t, n) {
  if (!i.body)
    throw new Error("Response body is null");
  const s = i.body.getReader(), r = new TextDecoder();
  let a = "", o = "", c;
  try {
    for (; ; ) {
      const { done: d, value: l } = await s.read();
      if (d) {
        const h = {
          content: o,
          references: c
        ...
        if (h.startsWith("data: "))
          try {
            ...
            if (f.type === "chunk" && typeof f.content == "string")
              ...
              const g = new Error(f.message || "SSE error");
              n == null || n(g);
            }
          } catch (f) {
...
class bc {
   ...
   */
  async stream(e, t, n) {
    if (console.log("[SpotInfoChat] stream() called", { queryLength: e.length, hasOnComplete: !!(n != null && n.onComplete), hasOnError: !!(n != null && n.onError) }), !e || typeof e != "string")
      throw console.error("[SpotInfoChat] Validation failed: Query is required and must be a string"), new Oe("Query is required and must be a string");
    if (!t || typeof t != "function")
      throw console.error("[SpotInfoChat] Validation failed: onChunk callback is required"), new Oe("onChunk callback is required");
    const s = new AbortController(), r = new yc(s);
    return console.log("[SpotInfoChat] Created stream instance"), n != null && n.onComplete && (console.log("[SpotInfoChat] Registering onComplete handler"), r.on("complete", n.onComplete)), n != null && n.onError && (console.log("[SpotInfoChat] Registering onError handler"), r.on("error", n.onError)), console.log("[SpotInfoChat] Starting stream..."), this.startStream(e, t, r, s, n).catch((a) => {
      var o;
      console.error("[SpotInfoChat] Stream error:", a), r.emit("error", a), (o = n == null ? void 0 : n.onError) == null || o.call(n, a);
    }), r;
  }
  /**
  ...
  async startStream(e, t, n, s, r) {
    ...
    const a = `${En()}/api/v1/sse/conversations`;
    ...
    try {
      ...
      console.log("[SpotInfoChat] startStream() - Response received:", { status: o.status, ok: o.ok }), console.log("[SpotInfoChat] startStream() - Starting to parse SSE stream..."), await vc(
        o,
        (c) => {
          c.type === "chunk" ? console.log("[SpotInfoChat] Received text chunk:", c.content.substring(0, 50) + "...") : c.type === "reference" && console.log("[SpotInfoChat] Received reference chunk:", {
            documentsCount: c.content.documents.length,
            apisCount: c.content.apis.length
          }), t(c), n.emit("chunk", c);
        },
        (c) => {
          var d;
          console.log("[SpotInfoChat] Stream complete:", {
            contentLength: c.content.length,
            referencesCount: ((d = c.references) == null ? void 0 : d.length) || 0
          }), n.emit("complete", c);
        },
        (c) => {
          console.error("[SpotInfoChat] Stream parsing error:", c), n.emit("error", c);
        }
      ), console.log("[SpotInfoChat] startStream() - SSE stream parsing completed");
    } catch (o) {
      throw console.error("[SpotInfoChat] startStream() - Request failed:", o), o;
    }
  }
  /**
   * Start mock SSE stream
   */
  async startMockStream(e, t, n, s) {
    var o;
    console.log("[SpotInfoChat] startMockStream() - Simulating chat stream for query:", e.substring(0, 50) + "...");
    const { generateMockSSEChunks: r, generateMockChatResponse: a } = await Promise.resolve().then(() => ts);
    try {
      const c = r(this.conversationId);
      let d = "", l, u = !1;
        ...
        if (h.startsWith("data: "))
          try {
            ...
            if (f.type === "chunk" && typeof f.content == "string") {
              ...
              typeof f.content == "string" && (d += f.content), f.references && (l = f.references);
              const g = {
                content: d,
                references: l
              };
      ...
      console.log("[SpotInfoChat] startMockStream() - Mock stream completed");
    } catch (c) {
      throw console.error("[SpotInfoChat] startMockStream() - Error:", c), n.emit("error", c), (o = s == null ? void 0 : s.onError) == null || o.call(s, c), c;
    }
  }
}
class kc extends es {
   ...
   */
  setRoomCredentials(t) {
    console.log("[SpotInfoVoice] Setting room credentials", { roomName: t.roomName, url: t.url }), this.roomCredentials = t;
  }
  /**
   * Format transcription data from text stream handler
   */
  formatTranscriptionData(t, n, s) {
    const r = typeof n == "string" ? n : (n == null ? void 0 : n.identity) || "", a = (s == null ? void 0 : s.attributes) || {}, o = a["lk.transcribed_track_id"] != null, c = a["lk.transcription_final"] === "true", d = a["lk.segment_id"], l = s == null ? void 0 : s.timestamp;
    return {
      participantIdentity: {
        identity: r
      },
  ...
  async start(t) {
    var n, s;
    if (console.log("[SpotInfoVoice] start() called", { userId: (n = t.userAttributes) == null ? void 0 : n.user_id, autoConnect: t.autoConnect !== !1 }), !((s = t.userAttributes) != null && s.user_id))
      throw console.error("[SpotInfoVoice] Validation failed: user_id is required in userAttributes"), new Oe("user_id is required in userAttributes");
    if (this.mockMode)
      return console.log("[SpotInfoVoice] Mock mode enabled, simulating voice session"), this.startMockVoice(t);
    if (!this.roomCredentials)
      throw console.error("[SpotInfoVoice] Room credentials not available"), new qt("Room credentials not available. Call setup() first.");
    try {
      console.log("[SpotInfoVoice] Setting state to initializing"), this._state = "initializing", this.emit("stateChanged", this._state), console.log("[SpotInfoVoice] Dispatching agent..."), await this.dispatchAgent(), console.log("[SpotInfoVoice] Agent dispatched successfully"), t.autoConnect !== !1 ? (console.log("[SpotInfoVoice] Auto-connect enabled, connecting to Voice..."), await this.connectToVoice()) : console.log("[SpotInfoVoice] Auto-connect disabled"), console.log("[SpotInfoVoice] Voice session started successfully");
    } catch (r) {
      throw console.error("[SpotInfoVoice] Failed to start voice session:", r), this._state = "disconnected", this.emit("stateChanged", this._state), this.emit("error", r), r;
    }
  }
  /**
  ...
  async dispatchAgent() {
    ...
    try {
      ...
      console.log("[SpotInfoVoice] dispatchAgent() - Success:", n.status);
    } catch (n) {
      throw console.error("[SpotInfoVoice] dispatchAgent() - Failed:", n), n;
    }
  }
  /**
   * Connect to Voice
  ...
  async connectToVoice() {
    if (!this.roomCredentials)
      throw console.error("[SpotInfoVoice] connectToVoice() - Room credentials not available"), new qt("Room credentials not available");
    console.log("[SpotInfoVoice] connectToVoice() - Starting connection", {
      url: this.roomCredentials.url,
      roomName: this.roomCredentials.roomName
    }), this._state = "connecting", this.emit("stateChanged", this._state);
    try {
      console.log("[SpotInfoVoice] connectToVoice() - Importing voice-client...");
      const t = await Promise.resolve().then(() => Om);
      console.log("[SpotInfoVoice] connectToVoice() - voice-client imported successfully"), this.voiceModule = t;
      const n = t.Room;
      this.roomEvent = t.RoomEvent, this.room = new n(), console.log("[SpotInfoVoice] connectToVoice() - Room instance created"), console.log("[SpotInfoVoice] connectToVoice() - Setting up room listeners..."), this.setupRoomListeners(), console.log("[SpotInfoVoice] connectToVoice() - Connecting to room..."), await this.room.connect(this.roomCredentials.url, this.roomCredentials.token), console.log("[SpotInfoVoice] connectToVoice() - Connected to room successfully"), console.log("[SpotInfoVoice] connectToVoice() - Setting up agent state tracking..."), await this.setupAgentStateTracking(), console.log("[SpotInfoVoice] connectToVoice() - Setting initial state to listening"), this._state = "listening", this.emit("stateChanged", this._state), console.log("[SpotInfoVoice] connectToVoice() - Enabling microphone..."), await this.room.localParticipant.setMicrophoneEnabled(!0), console.log("[SpotInfoVoice] connectToVoice() - Microphone enabled"), console.log("[SpotInfoVoice] connectToVoice() - Connection complete");
    } catch (t) {
      throw console.error("[SpotInfoVoice] connectToVoice() - Connection failed:", t), this._state = "disconnected", this.emit("stateChanged", this._state), new qt(
        `Failed to connect to Voice: ${t instanceof Error ? t.message : "Unknown error"}`,
        { originalError: t }
      );
    }
  }
   ...
   */
  setupRoomListeners() {
    if (!(!this.room || !this.roomEvent)) {
      this.room.on(this.roomEvent.Connected, () => {
        console.log("[SpotInfoVoice] Room connected");
      }), this.room.on(this.roomEvent.Disconnected, () => {
        console.log("[SpotInfoVoice] Room disconnected");
        const t = this._state;
        console.log("[SpotInfoVoice] Agent state change:", { from: t, to: "disconnected" }), this._state = "disconnected", this.agentParticipant = null, this.attributesChangedListenerRegistered = !1, this.emit("stateChanged", this._state);
      }), this.room.on(this.roomEvent.ParticipantConnected, (t) => {
        var n;
        console.log("[SpotInfoVoice] Participant connected:", t.identity), (t.isAgent || (n = t.identity) != null && n.includes("agent")) && (this.agentParticipant = t, this.setupAgentStateTracking(), this.subscribeToAgentTracks(t));
      }), this.room.on(this.roomEvent.TrackPublished, (t, n) => {
        var s;
        (n.isAgent || (s = n.identity) != null && s.includes("agent")) && t.kind === "audio" && (console.log("[SpotInfoVoice] Agent audio track published, subscribing..."), t.track ? this.handleAudioTrack(t.track) : t.setSubscribed(!0));
      }), this.room.on(this.roomEvent.TrackSubscribed, (t, n, s) => {
        var r;
        t.kind === "audio" && (s.isAgent || (r = s.identity) != null && r.includes("agent")) && (console.log("[SpotInfoVoice] Agent audio track subscribed"), this.handleAudioTrack(t), this.emit("audioTrack", t));
      }), this.room.on(this.roomEvent.DataReceived, (t, n, s, r) => {
        try {
          const o = new TextDecoder().decode(t);
          if (console.log("[SpotInfoVoice] Original DataReceived event:", {
            topic: r,
            participant: n == null ? void 0 : n.identity,
            participantMetadata: n == null ? void 0 : n.metadata,
            participantAttributes: n == null ? void 0 : n.attributes,
            kind: s,
            payloadLength: t.length,
            payloadRaw: Array.from(t),
            // Log raw bytes
            decodedString: o
            // Log decoded string before parsing
          }), r === "lk.transcription") {
            console.log("[SpotInfoVoice] Transcription received via DataReceived", {
              topic: r,
              participant: n == null ? void 0 : n.identity,
              payloadLength: t.length
            });
            let c = null, d = null;
            try {
              d = JSON.parse(o), console.log("[SpotInfoVoice] Original transcription data (parsed JSON):", JSON.stringify(d, null, 2)), c = d.text || d.transcription || d.content || o;
            } catch {
              console.log("[SpotInfoVoice] Original transcription data (plain text):", o), c = o;
            }
            if (c && c.trim()) {
              console.log("[SpotInfoVoice] Transcription text:", c);
              const l = (n == null ? void 0 : n.identity) || "", u = (d == null ? void 0 : d.attributes) || (n == null ? void 0 : n.attributes) || {}, h = u["lk.transcribed_track_id"] != null || (d == null ? void 0 : d.transcribed_track_id) != null, f = u["lk.transcription_final"] === "true" || (d == null ? void 0 : d.transcription_final) === "true", g = u["lk.segment_id"] || (d == null ? void 0 : d.segment_id), v = (d == null ? void 0 : d.timestamp) || Date.now(), E = {
                participantIdentity: {
                  identity: l
                },
                text: c,
              ...
              this._transcription = c, this.emit("transcription", E);
            }
          } else
            try {
              const c = JSON.parse(o);
              if (console.log("[SpotInfoVoice] Original custom transcription data:", JSON.stringify(c, null, 2)), c.type === "transcription" || c.transcription) {
                const d = c.text || c.transcription;
                if (d) {
                  console.log("[SpotInfoVoice] Custom transcription via DataReceived", { text: d, topic: r });
                  const u = {
                    participantIdentity: {
                  ...
                  this._transcription = d, this.emit("transcription", u);
                }
              }
            } catch {
            }
        } catch (a) {
          console.error("[SpotInfoVoice] Error processing DataReceived event:", a);
        }
      });
      try {
        typeof this.room.registerTextStreamHandler == "function" && (this.room.registerTextStreamHandler("lk.transcription", async (t, n) => {
          var s, r, a, o, c, d, l;
          try {
            console.log("[SpotInfoVoice] Original text stream handler reader object:", {
              participantIdentity: n,
              readerType: typeof t,
              readerConstructor: (s = t == null ? void 0 : t.constructor) == null ? void 0 : s.name,
              readerKeys: t ? Object.keys(t) : [],
            ...
            if (typeof t.readAll == "function")
              ...
              let g = !1;
              for (; !g; ) {
                const { value: v, done: E } = await h.read();
                g = E, v && (u += f.decode(v, { stream: !0 }));
              }
            ...
            if (u && u.trim()) {
              ...
              if (console.log("[SpotInfoVoice] Original transcription from text stream handler:", {
                ...
                const g = this.formatTranscriptionData(
                  u,
                  n,
                  t.info
                );
                this._transcription = u, this.emit("transcription", g);
              }
            }
          } catch (u) {
            console.error("[SpotInfoVoice] Error processing transcription text stream:", u);
          }
        }), console.log("[SpotInfoVoice] Registered text stream handler for lk.transcription"));
      } catch {
        console.log("[SpotInfoVoice] Text stream handler registration not available, using DataReceived events");
      }
      this.room.on(this.roomEvent.MediaDevicesError, (t) => {
        console.error("[SpotInfoVoice] Media device error:", t), this.emit("error", new Error(`Media device error: ${t.message || "Unknown error"}`));
      });
    }
  }
  /**
   ...
   */
  subscribeToAgentTracks(t) {
    if (!t) return;
    console.log("[SpotInfoVoice] Subscribing to agent tracks...");
    const n = t.audioTrackPublications;
    n && n.forEach((s) => {
      if (s.track)
        this.handleAudioTrack(s.track);
      else
        try {
          typeof s.setSubscribed == "function" && s.setSubscribed(!0);
        } catch (r) {
          console.warn("[SpotInfoVoice] Failed to subscribe to track:", r);
        }
    });
  }
  /**
   * Handle audio track - attach to audio element and play
   */
  handleAudioTrack(t) {
    try {
      console.log("[SpotInfoVoice] Handling audio track:", t.sid), this.audioElement || (this.audioElement = new Audio(), this.audioElement.autoplay = !0, this.audioElement.onerror = (n) => {
        console.error("[SpotInfoVoice] Audio element error:", n);
      }), t.attach(this.audioElement), console.log("[SpotInfoVoice] Audio track attached to element"), this.audioElement.play().catch((n) => {
        console.error("[SpotInfoVoice] Failed to play audio:", n);
      });
    } catch (n) {
      console.error("[SpotInfoVoice] Error handling audio track:", n);
    }
  }
  /**
   * Set up agent state tracking
   * Tracks agent state changes similar to how useVoiceAssistant does
   */
  async setupAgentStateTracking() {
    if (!(!this.room || !this.voiceModule)) {
      if (!this.agentParticipant) {
        const t = Array.from(this.room.remoteParticipants.values());
        this.agentParticipant = t.find(
          (n) => {
            var s;
            return n.isAgent || ((s = n.identity) == null ? void 0 : s.includes("agent"));
          }
        );
      }
      if (!this.agentParticipant) {
        console.warn("[SpotInfoVoice] Agent participant not found");
        return;
      }
      if (this.subscribeToAgentTracks(this.agentParticipant), console.log("[SpotInfoVoice] Agent participant found:", {
        identity: this.agentParticipant.identity,
        isAgent: this.agentParticipant.isAgent,
        attributes: this.agentParticipant.attributes
      }), !this.attributesChangedListenerRegistered && this.room && this.roomEvent && (this.room.on(this.roomEvent.ParticipantAttributesChanged, (t, n) => {
        var s;
        if ((n.isAgent || (s = n.identity) != null && s.includes("agent")) && "lk.agent.state" in t) {
          const r = t["lk.agent.state"];
          if (console.log(`[SpotInfoVoice] Agent ${n.identity} state changed to: ${r}`), this.isValidAgentState(r)) {
            const a = this._state;
            console.log("[SpotInfoVoice] Agent state change (from ParticipantAttributesChanged):", {
              from: a,
              to: r,
              changedAttributes: t
            }), this._state = r, this.emit("stateChanged", this._state);
          } else
            console.warn("[SpotInfoVoice] Invalid agent state received:", r);
        }
      }), this.attributesChangedListenerRegistered = !0, console.log("[SpotInfoVoice] ParticipantAttributesChanged listener registered")), this.agentParticipant.attributes && "lk.agent.state" in this.agentParticipant.attributes) {
        const t = this.agentParticipant.attributes["lk.agent.state"];
        if (console.log("[SpotInfoVoice] Initial agent state from attributes:", t), this.isValidAgentState(t)) {
          const n = this._state;
          console.log("[SpotInfoVoice] Agent state change (initial from attributes):", { from: n, to: t }), this._state = t, this.emit("stateChanged", this._state);
        }
      }
    }
  }
  /**
   * Validate agent state matches AgentState
   */
  isValidAgentState(t) {
    return ["connecting", "listening", "thinking", "speaking", "disconnected", "initializing"].includes(t);
  }
  /**
   * Mute microphone
   * Aligned with MuteAudioToggle component from web-widget
   */
  async mute() {
    if (this.room && this.room.localParticipant)
      try {
        await this.room.localParticipant.setMicrophoneEnabled(!1), this._muted = !0, this.emit("muteChanged", this._muted);
      } catch (t) {
        throw console.error("[SpotInfoVoice] Failed to mute microphone:", t), t;
      }
    else
      this._muted = !0, this.emit("muteChanged", this._muted);
    this.mockMode && this._state !== "listening" && this._state !== "disconnected" && (this._state = "listening", this.emit("stateChanged", this._state), this.stopSpeaking());
  }
  /**
   * Unmute microphone
   * Aligned with MuteAudioToggle component from web-widget
  ...
  async unmute() {
    if (this.room && this.room.localParticipant)
      try {
        await this.room.localParticipant.setMicrophoneEnabled(!0), this._muted = !1, this.emit("muteChanged", this._muted);
      } catch (t) {
        throw console.error("[SpotInfoVoice] Failed to unmute microphone:", t), t;
      }
    else
      this._muted = !1, this.emit("muteChanged", this._muted);
  }
  /**
   * Toggle mute state
   */
  async toggleMute() {
    this._muted ? await this.unmute() : await this.mute();
  }
  /**
   * Close voice session
   */
  ...
  async deleteRoom() {
    ...
    try {
      ...
      console.warn("Failed to delete room:", n);
    }
  }
  /**
   * Create room
   */
  async createRoom() {
    const t = `${an()}/create-room`;
    try {
      const s = await (await et(t, {
        method: "POST",
        headers: {
          "x-api-key": this.apiKey,
          "Content-Type": "application/json"
      ...
      this.roomCredentials = s;
    } catch (n) {
      console.warn("Failed to create new room:", n);
    }
  }
  /**
   * Start mock voice session
   */
  async startMockVoice(t) {
    console.log("[SpotInfoVoice] startMockVoice() - Simulating voice session");
    try {
      this._state = "connecting", this.emit("stateChanged", this._state), await new Promise((n) => setTimeout(n, 2e3)), this._state = "initializing", this.emit("stateChanged", this._state), await new Promise((n) => setTimeout(n, 700)), this._state = "listening", this.emit("stateChanged", this._state), t.autoConnect !== !1 && this.startMockTranscriptions(), console.log("[SpotInfoVoice] Mock voice session started successfully");
    } catch (n) {
      throw console.error("[SpotInfoVoice] Failed to start mock voice session:", n), this._state = "disconnected", this.emit("stateChanged", this._state), this.emit("error", n), n;
    }
  }
  /**
   * Calculate estimated speech duration for text (in milliseconds)
   * Based on average speaking rate: ~150 words per minute = ~2.5 words/second
   * At rate 1.0, that's approximately 400ms per word
   */
  calculateSpeechDuration(t, n = 1) {
    const s = t.trim().split(/\s+/).length, r = 2.5 * n, a = s / r;
    return Math.max(a * 1e3, 100);
  }
  /**
   * Speak text using Web Speech API (for mock mode agent responses)
  ...
  speakText(t) {
    return new Promise((n) => {
      ...
      if (this.stopSpeaking(), !("speechSynthesis" in window)) {
        console.warn("[SpotInfoVoice] Speech synthesis not available"), n();
        return;
      }
      try {
        const s = new SpeechSynthesisUtterance(t), r = () => {
          const o = speechSynthesis.getVoices();
          console.log("[SpotInfoVoice] Available voices:", o);
          const c = o.length > 0 ? o[18] : null;
          c ? (s.voice = c, console.log("[SpotInfoVoice] Using voice:", c.name, c.lang)) : console.warn("[SpotInfoVoice] No voices available, using default"), s.rate = 1.1, s.pitch = 1.2, s.volume = 0.3, this.currentSpeechUtterance = s, s.onstart = () => {
            n();
          }, s.onend = () => {
            this.currentSpeechUtterance = null;
          }, s.onerror = (d) => {
            console.error("[SpotInfoVoice] Speech synthesis error:", d), this.currentSpeechUtterance = null, n();
          }, speechSynthesis.speak(s), console.log("[SpotInfoVoice] Speaking agent response:", t);
        };
        speechSynthesis.getVoices().length === 0 ? speechSynthesis.onvoiceschanged = () => {
          speechSynthesis.onvoiceschanged = null, r();
        } : r();
      } catch (s) {
        console.error("[SpotInfoVoice] Failed to speak text:", s), this.currentSpeechUtterance = null, n();
      }
    });
  }
  /**
   ...
   */
  stopSpeaking() {
    "speechSynthesis" in window && (this.currentSpeechUtterance && console.log("[SpotInfoVoice] Stopping ongoing speech"), speechSynthesis.cancel()), this.currentSpeechUtterance = null;
  }
  /**
   * Start mock transcriptions
   * Flow: listening -> send user chunks -> thinking -> speaking -> send agent chunks -> listening -> repeat
   */
  startMockTranscriptions() {
    Promise.resolve().then(() => ts).then(({ generateMockTranscription: t, selectNewPair: n }) => {
      this.mockTranscriptionRunning = !0, (async () => {
        for (; this.mockTranscriptionRunning && !this._muted; )
          try {
            const r = n();
            console.log("[SpotInfoVoice] Selected new transcription pair");
            let a;
            this._state !== "listening" && (this._state = "listening", this.emit("stateChanged", this._state)), this._transcription = "";
            const o = r.user.join(""), d = this.calculateSpeechDuration(o, 0.9);
            let l = 0;
            const u = [];
            for (let _ = 0; _ < r.user.length; _++)
            ...
            const h = r.agent.join(""), g = this.calculateSpeechDuration(h, 0.95);
            let v = 0;
            const E = [];
            for (let _ = 0; _ < r.agent.length; _++)
              E.push(v), v += r.agent[_].length;
            E.push(h.length);
            ...
            for (let _ = 0; _ < r.agent.length && this.mockTranscriptionRunning && !this._muted; _++) {
              ...
              if (_ === 0)
                ...
                const D = E[_ - 1] / h.length * g, N = b * g - D;
                N > 0 && await new Promise((j) => setTimeout(j, N));
              }
              if (!this.mockTranscriptionRunning || this._muted) break;
              const S = t(
            ...
            console.error("[SpotInfoVoice] Error in transcription cycle:", r), await new Promise((a) => setTimeout(a, 2e3));
          }
      })();
    });
  }
...
class Ds extends es {
  constructor(t, n, s, r = !1, a = 1e3, o = 1e4, c = !1) {
    ...
    this.apiKey = t, this.clientId = n, this.userId = s, this.enabled = r, this.pollInterval = a, this.initialDelay = o, this.mockMode = c, console.log("[SpotInfoAnalytics] Constructor called", {
      ...
      enabled: r,
      mockMode: this.mockMode
    }), this.hookDelivered = this.checkHookDeliveredFromStorage();
  }
  /**
   * Check if hook was delivered by checking localStorage and cookies
   */
  checkHookDeliveredFromStorage() {
    const t = `spotinfo_hook_delivered_user_${this.userId}`;
    try {
      if (localStorage.getItem(t) === "true")
        return !0;
    } catch (n) {
      console.warn("[SpotInfoAnalytics] Error accessing localStorage:", n);
    }
    try {
      const n = document.cookie.split(";");
      for (const s of n) {
        const [r, a] = s.trim().split("=");
        if (r === t && a === "true")
          return !0;
      }
    } catch (n) {
      console.warn("[SpotInfoAnalytics] Error accessing cookies:", n);
    }
    return !1;
  }
  /**
   * Start tracking user journey events
   */
  startTracking() {
    if (this.isTracking) {
      console.log("[SpotInfoAnalytics] Tracking already started");
      return;
    }
    if (!this.enabled) {
      console.log("[SpotInfoAnalytics] User journey tracking is disabled");
      return;
    }
    if (this.hookDelivered = this.checkHookDeliveredFromStorage(), this.hookDelivered) {
      console.log("[SpotInfoAnalytics] Hook already delivered (found in storage) - tracking will not start");
      return;
    }
    console.log("[SpotInfoAnalytics] Starting user journey tracking"), this.isTracking = !0, this.trackEvent("widget_loaded", this.getDeviceProperties()), this.setupEventListeners(), this.startInitialDelay();
  }
  /**
   * Stop tracking user journey events
   */
  stopTracking() {
    this.isTracking && (console.log("[SpotInfoAnalytics] Stopping user journey tracking"), this.isTracking = !1, this.removeEventListeners(), this.stopPolling());
  }
  /**
   * Track a custom event
   */
  trackEvent(t, n = {}) {
    if (!this.isTracking && t !== "widget_loaded" || (this.hookDelivered = this.checkHookDeliveredFromStorage(), this.hookDelivered))
      return;
    const s = {
      event: t,
      properties: {
    ...
    this.sendEvent(s).catch((r) => {
      console.error("[SpotInfoAnalytics] Error sending event:", r);
    }), this.emit("eventTracked", { event: t, properties: s.properties });
  }
  /**
   * Register callback for when personalized hook is received
   */
  onHook(t) {
    this.on("hook", t);
  }
  ...
  getDeviceProperties() {
    ...
    try {
      if (t.userAgent = navigator.userAgent, t.language = navigator.language, t.platform = navigator.platform, t.cookieEnabled = navigator.cookieEnabled, t.screenResolution = `${screen.width}x${screen.height}`, t.viewportSize = `${window.innerWidth}x${window.innerHeight}`, t.devicePixelRatio = window.devicePixelRatio, t.timezone = Intl.DateTimeFormat().resolvedOptions().timeZone, "connection" in navigator) {
        ...
        n && (t.connectionType = n.effectiveType || n.type);
      }
      t.url = window.location.href, t.title = document.title, t.referrer = document.referrer;
    } catch (n) {
      console.warn("[SpotInfoAnalytics] Error collecting device properties:", n);
    }
    return t;
  }
  /**
   * Get basic properties for events
   */
  getBasicProperties() {
    return {
      timestamp: (/* @__PURE__ */ new Date()).toISOString(),
   ...
   */
  async sendEvent(t) {
    const n = `${En()}/api/v1/register_user_journey`;
    ...
    try {
      ...
      console.log("[SpotInfoAnalytics] sendEvent completed", {
        mockMode: this.mockMode,
        status: s.status
      });
    } catch (s) {
      throw console.warn("[SpotInfoAnalytics] Failed to send event:", s), s;
    }
  }
  /**
  ...
  setupEventListeners() {
    this.textSelectionHandler = () => {
      this.textSelectionDebounceTimer && clearTimeout(this.textSelectionDebounceTimer), this.textSelectionDebounceTimer = window.setTimeout(() => {
        const n = document.getSelection(), s = n == null ? void 0 : n.toString().trim();
        s && s.length > 5 && this.trackEvent("text_selected", { selectedText: s });
      }, 500);
    }, this.clickHandler = (n) => {
      var c, d;
      const s = n.target;
      if (!s) return;
      const r = ["A", "BUTTON", "FORM", "INPUT", "SELECT", "TEXTAREA", "LABEL"];
      if (s.tagName && r.includes(s.tagName.toUpperCase()))
        return;
      const a = s.id || ((c = s.tagName) == null ? void 0 : c.toLowerCase()) || "unknown", o = ((d = s.textContent) == null ? void 0 : d.trim().slice(0, 100)) || "";
      this.trackEvent("click", { target: a, text: o });
    }, this.formSubmitHandler = (n) => {
      n.preventDefault();
      const s = n.target;
      if (!s) return;
      const r = {};
      Array.from(s.elements).forEach((a) => {
        var o;
        if ((a instanceof HTMLInputElement || a instanceof HTMLSelectElement || a instanceof HTMLTextAreaElement) && a.name) {
          let c = "";
          a instanceof HTMLSelectElement ? c = ((o = a.options[a.selectedIndex]) == null ? void 0 : o.value) || "" : c = a.value || "", r[a.name] = c;
        }
      ...
      let r;
      if (s.id) {
        const g = document.querySelector(`label[for="${s.id}"]`);
        r = ((o = g == null ? void 0 : g.textContent) == null ? void 0 : o.trim()) || void 0;
      }
    ...
    const t = () => {
      const n = window.pageYOffset || window.scrollY || document.documentElement.scrollTop, s = Date.now();
      if (n === this.lastScrollPosition || s - this.lastScrollTimestamp < 500)
        return;
      const r = /* @__PURE__ */ new Set(), a = window.innerHeight;
      document.body && this.findAndCollectVisibleText(document.body, a, r);
      const o = Array.from(r), c = o.join("|");
      o.length > 0 && c !== this.lastVisibleSectionsHash && (this.lastScrollPosition = n, this.lastScrollTimestamp = s, this.lastVisibleSectionsHash = c, this.trackEvent("scroll_stopped_with_visible_content", {
        visibleSections: o
      }));
    };
    this.scrollHandler = () => {
      (window.pageYOffset || window.scrollY || document.documentElement.scrollTop) !== this.lastScrollPosition && (this.scrollDebounceTimer && clearTimeout(this.scrollDebounceTimer), this.scrollDebounceTimer = window.setTimeout(t, 500));
      ...
      (n) => {
        n.forEach((s) => {
          ...
          if (a) {
            if (s.isIntersecting)
              this.trackEvent("section_scrolled", { sectionId: a }), r.setAttribute("data-enter-time", Date.now().toString());
            else if (r.getAttribute("data-enter-time")) {
              const o = r.getAttribute("data-enter-time"), c = o ? Date.now() - Number(o) : 0;
              this.trackEvent("section_time_spent", {
                sectionId: a,
                timeSpent: c,
                textContent: r.textContent || ""
              }), r.removeAttribute("data-enter-time");
            }
          }
        });
      },
      { threshold: 0.5 }
    ), this.observedSections = document.querySelectorAll("section"), this.observedSections.forEach((n) => {
      n && this.intersectionObserver && this.intersectionObserver.observe(n);
    }), document.addEventListener("selectionchange", this.textSelectionHandler), document.addEventListener("click", this.clickHandler), document.addEventListener("submit", this.formSubmitHandler), document.addEventListener("scroll", this.scrollHandler, !0), document.addEventListener("change", this.inputChangeHandler, !0);
  }
  /**
   * Remove DOM event listeners
   */
  removeEventListeners() {
    this.textSelectionHandler && document.removeEventListener("selectionchange", this.textSelectionHandler), this.clickHandler && document.removeEventListener("click", this.clickHandler), this.formSubmitHandler && document.removeEventListener("submit", this.formSubmitHandler), this.scrollHandler && document.removeEventListener("scroll", this.scrollHandler, !0), this.inputChangeHandler && document.removeEventListener("change", this.inputChangeHandler, !0), this.textSelectionDebounceTimer && clearTimeout(this.textSelectionDebounceTimer), this.scrollDebounceTimer && clearTimeout(this.scrollDebounceTimer), this.intersectionObserver && (this.observedSections && this.observedSections.forEach((t) => {
      t && this.intersectionObserver.unobserve(t);
    }), this.intersectionObserver.disconnect()), this.textSelectionHandler = null, this.clickHandler = null, this.formSubmitHandler = null, this.scrollHandler = null, this.inputChangeHandler = null, this.intersectionObserver = null, this.observedSections = null;
  }
  /**
   * Find and collect visible text
   */
  findAndCollectVisibleText(t, n, s, r = 0) {
    var g;
    if (r > 20) return;
    const a = ((g = t.tagName) == null ? void 0 : g.toUpperCase()) || "", o = t.getAttribute("role") || "", c = t.id ? t.id.toLowerCase() : "";
    if (["SCRIPT", "STYLE", "NOSCRIPT", "HEADER", "FOOTER", "NAV", "ASIDE"].includes(a) || ["banner", "contentinfo", "navigation", "complementary", "search", "form"].includes(o) || c.includes("navbar") || c.includes("navigation") || c.includes("sidebar") || c.includes("header") || c.includes("footer"))
      return;
    const d = t.getBoundingClientRect();
    if (d.width === 0 && d.height === 0 && d.top === 0 && d.left === 0 || getComputedStyle(t).display === "none" || Math.max(0, Math.min(d.bottom, n) - Math.max(d.top, 0)) < 5 || d.width < 5 || d.right <= 0 || d.left >= window.innerWidth)
      return;
    const u = 10;
    if (d.top >= -u && d.bottom <= n + u && d.left >= -u && d.right <= window.innerWidth + u) {
      const v = this.getCleanedTextContent(t);
      v.length >= 10 && s.add(v);
      return;
    }
   ...
   */
  getCleanedTextContent(t) {
    const n = t.cloneNode(!0);
    n.querySelectorAll(
      'script, style, header, footer, nav, aside, [role="navigation"], [role="banner"], [role="contentinfo"], [role="complementary"]'
    ).forEach((r) => r.remove()), n.querySelectorAll("*").forEach((r) => {
      if (r instanceof HTMLElement && r.parentElement) {
        const a = r.id ? r.id.toLowerCase() : "";
        (a.includes("navbar") || a.includes("navigation")) && r.remove();
      }
    });
    let s = n.textContent || "";
  ...
  /**
   * Start initial delay before polling
   */
  startInitialDelay() {
    this.hookDelivered || this.isPolling || (this.clearInitialDelay(), console.log(`[SpotInfoAnalytics] Starting initial delay of ${this.initialDelay}ms before polling`), this.initialDelayTimeout = setTimeout(() => {
      this.initialDelayTimeout = null, !this.hookDelivered && !this.isPolling && this.startPolling();
    }, this.initialDelay));
  }
  /**
   * Clear initial delay
   */
  clearInitialDelay() {
    this.initialDelayTimeout && (clearTimeout(this.initialDelayTimeout), this.initialDelayTimeout = null);
  }
  /**
   * Start polling for hooks
   */
  startPolling() {
    if (this.isPolling || this.hookDelivered)
      return;
    console.log("[SpotInfoAnalytics] Starting hook polling"), this.isPolling = !0, this.pollCounter = 0;
    const t = async () => {
      if (!this.isPolling || this.hookDelivered)
        return;
      const n = `spotinfo_hook_delivered_user_${this.userId}`;
      if (localStorage.getItem(n) === "true") {
        console.log("[SpotInfoAnalytics] Hook already delivered, stopping polling"), this.stopPolling();
        return;
      }
      try {
        const s = `${En()}/api/v1/get_user_journey_hook/${this.userId}/`;
        ...
        if (console.log("[SpotInfoAnalytics] Hook poll response", {
          mockMode: this.mockMode,
          status: r.status
        }), r.status === 200) {
          const o = (await r.json()).message;
          if (o) {
            console.log("[SpotInfoAnalytics] Received hook message:", o), this.handleHook(o), this.stopPolling();
            return;
          }
        } else if (r.status === 204) {
          if (this.pollCounter++, this.pollCounter > this.pollCounterLimit) {
            console.log(
              `[SpotInfoAnalytics] No hook received after ${this.pollCounter} attempts, stopping polling`
            ), this.stopPolling();
            return;
          }
        } else if (this.pollCounter++, this.pollCounter > this.pollCounterLimit) {
          console.log(
            `[SpotInfoAnalytics] Poll failed after ${this.pollCounter} attempts, stopping polling`
          ), this.stopPolling();
          return;
        }
      } catch (s) {
        if (console.error("[SpotInfoAnalytics] Poll error:", s), this.pollCounter++, this.pollCounter > this.pollCounterLimit) {
          this.stopPolling();
          return;
        }
      }
      this.isPolling && !this.hookDelivered && (this.pollingInterval = setTimeout(t, this.pollInterval));
    };
    t();
  }
  /**
   * Stop polling for hooks
   */
  stopPolling() {
    this.pollingInterval && (clearTimeout(this.pollingInterval), this.pollingInterval = null), this.isPolling = !1, console.log("[SpotInfoAnalytics] Stopped hook polling");
  }
  /**
   * Store hook delivered flag in both localStorage and cookies
   */
  storeHookDelivered() {
    const t = `spotinfo_hook_delivered_user_${this.userId}`;
    try {
      ...
      console.warn("[SpotInfoAnalytics] Error storing hook delivered in localStorage:", n);
    }
    try {
      const n = /* @__PURE__ */ new Date();
      n.setFullYear(n.getFullYear() + 1), document.cookie = `${t}=true; expires=${n.toUTCString()}; path=/`;
    } catch (n) {
      console.warn("[SpotInfoAnalytics] Error storing hook delivered in cookies:", n);
    }
  }
  /**
   * Handle received hook
   */
  handleHook(t) {
    this.storeHookDelivered(), this.hookDelivered = !0, console.log("[SpotInfoAnalytics] Hook delivered - stopping event tracking, logging, and backend sends"), this.stopTracking(), this.emit("hook", t);
  }
  /**
   * Mark hook as delivered (e.g., when widget is opened)
   */
  markHookDelivered() {
    this.storeHookDelivered(), this.hookDelivered = !0, this.stopPolling(), this.stopTracking();
  }
}
class Mm {
  constructor(e) {
    ...
    if (console.log("[SpotInfoClient] Initializing client...", { clientId: e.clientId, userId: (t = e.userAttributes) == null ? void 0 : t.user_id }), !e.apiKey)
      throw console.error("[SpotInfoClient] Validation failed: apiKey is required"), new Oe("apiKey is required");
    this.config = {
      autoSetup: !0,
      // Default: true
      retryAttempts: 3,
      ...
      enableUserJourney: !0,
      // Default: true
      userJourneyPollInterval: 1e3,
      userJourneyInitialDelay: 1e4,
      mockMode: !1,
      // Default: false
      ...e
    }, this.conversationId = pc(), console.log("[SpotInfoClient] Generated conversation ID:", this.conversationId), console.log("[SpotInfoClient] Config:", {
      clientId: this.config.clientId,
      autoSetup: this.config.autoSetup,
      retryAttempts: this.config.retryAttempts
    }), this.config.autoSetup ? (console.log("[SpotInfoClient] Auto-setup enabled, will call setup() automatically"), setTimeout(() => {
      this.setup().catch((n) => {
        console.error("[SpotInfoClient] Auto-setup failed:", n);
      });
    }, 0)) : console.log("[SpotInfoClient] Auto-setup disabled, call setup() manually"), this.config.enableUserJourney && (console.log("[SpotInfoClient] User journey tracking enabled"), this.analyticsInstance = new Ds(
      this.config.apiKey,
      this.config.clientId,
      this.config.userAttributes.user_id,
      !0,
  ...
  async setup(e) {
    var n;
    if (console.log("[SpotInfoClient] setup() called"), this.setupPromise)
      return console.log("[SpotInfoClient] Setup already in progress, returning existing promise"), this.setupPromise;
    if (this.setupCalled && this.roomCredentials)
      return console.log("[SpotInfoClient] Already set up, returning cached result", { roomName: this.roomCredentials.roomName }), {
        success: !0,
        clientId: this.config.clientId,
        roomName: this.roomCredentials.roomName
      };
    console.log("[SpotInfoClient] Starting setup process..."), this.setupPromise = this.performSetup();
    const t = await this.setupPromise;
    return this.setupPromise = null, console.log("[SpotInfoClient] Setup completed:", {
      success: t.success,
      roomName: t.roomName,
  ...
  async performSetup() {
    var e;
    try {
      if (console.log("[SpotInfoClient] performSetup() started"), !this.config.clientId)
        throw console.error("[SpotInfoClient] Validation failed: client_id is required"), new Oe("client_id is required");
      if (!this.config.apiKey)
        throw new Oe("api_key is required");
      if (!((e = this.config.userAttributes) != null && e.user_id))
        throw new Oe("user_id is required in userAttributes");
      console.log("[SpotInfoClient] Step 2: Using conversation ID:", this.conversationId), console.log("[SpotInfoClient] Step 3: Ensuring RAG setup..."), await this.ensureRag(), console.log("[SpotInfoClient] Step 3: RAG setup complete"), console.log("[SpotInfoClient] Step 4: Ensuring room exists...");
      const t = await this.ensureRoom();
      return console.log("[SpotInfoClient] Step 4: Room credentials received:", {
        roomName: t.roomName,
        url: t.url
      }), this.roomCredentials = t, this.setupCalled = !0, console.log("[SpotInfoClient] Step 5: Room credentials stored, setup complete"), {
        success: !0,
        clientId: this.config.clientId,
        roomName: t.roomName
      };
    } catch (t) {
      return console.error("[SpotInfoClient] Setup failed:", t), this.setupCalled = !1, {
        success: !1,
        clientId: this.config.clientId,
        roomName: null,
        error: t
  ...
  /**
   * Ensure RAG setup
   * Aligned with conversationService.ts ensureRAG implementation
   */
  async ensureRag() {
    var e, t;
    try {
      const s = `${En()}/engine/manage/ensure_rag/`, r = {
        client_id: this.config.clientId,
        conversation_id: this.conversationId,
        client: "js_library",
        user_attributes: {
          email: ((e = this.config.userAttributes) == null ? void 0 : e.email) || "",
          user_id: ((t = this.config.userAttributes) == null ? void 0 : t.user_id) || ""
        }
      };
      try {
        const a = await et(s, {
          ...
          body: r,
          retryAttempts: 0,
          // Don't retry setup operations
          mockMode: this.config.mockMode || !1
        });
        if ((await a.json()).status === "success") {
          console.log("[SpotInfoClient] ensureRag() - Request successful:", a.status);
          return;
        }
      } catch (a) {
        throw console.warn("[SpotInfoClient] ensureRAG API call error:", a), a;
      }
    } catch (n) {
      throw console.warn("[SpotInfoClient] ensureRAG API call error:", n), n;
    }
  }
  /**
   * Ensure room exists or create new one
   */
  async ensureRoom() {
    var t, n;
    const e = `${an()}/create-room`;
    try {
      const s = {
        client_id: this.config.clientId,
        conversation_id: this.conversationId,
        client: "js_library",
        user_attributes: {
          email: ((t = this.config.userAttributes) == null ? void 0 : t.email) || "",
      ...
      throw console.error("[SpotInfoClient] ensureRoom() - Error:", s), s;
    }
  }
  /**
   * Get chat instance
  ...
  voice() {
    return this.voiceInstance ? console.log("[SpotInfoClient] Returning existing SpotInfoVoice instance") : (console.log("[SpotInfoClient] Creating new SpotInfoVoice instance"), this.voiceInstance = new kc(
      ...
      this.config.clientId,
      this.config.mockMode || !1
    ), this.roomCredentials ? (console.log("[SpotInfoClient] Setting room credentials on voice instance"), this.voiceInstance.setRoomCredentials(this.roomCredentials)) : console.warn("[SpotInfoClient] No room credentials available for voice instance")), this.voiceInstance;
  }
  /**
   * Get analytics instance
   */
  analytics() {
    return this.analyticsInstance ? console.log("[SpotInfoClient] Returning existing SpotInfoAnalytics instance") : (console.log("[SpotInfoClient] Creating new SpotInfoAnalytics instance"), this.analyticsInstance = new Ds(
      this.config.apiKey,
      this.config.clientId,
      this.config.userAttributes.user_id,
      this.config.enableUserJourney || !1,
      this.config.userJourneyPollInterval || 1e3,
      this.config.userJourneyInitialDelay || 1e4,
      this.config.mockMode || !1
...
If you have any specific questions or need further assistance, please don't hesitate to ask. I'm here to help!`,
  `This is a sample response.
...
Feel free to reach out anytime if you need support or have questions. I'm ready to help!`,
  `This is a sample response.
...
Don't hesitate to ask if you need any additional information or have specific questions. I'm here for you!`,
  `This is a sample response.
...
If you have any questions or need assistance, I'm available to help. Just let me know how I can support you!`,
  `This is a sample response.
...
Please feel free to ask any questions or request further help. I'm here to support your success!`,
  `This is a sample response.
...
If you need any clarification or have specific questions, don't hesitate to reach out. I'm ready to assist!`
];
function Lr() {
...
const Tc = [
  {
    ...
    name: "Product Guide",
    resource_id: 1,
    url: "https://example.com/docs/product-guide",
    page_labels: [1, 2, 3],
...
], _n = [
  {
    ...
    agent: [
      "This is a mock output in robotic voide for developement only.",
      " Actual output will be natural voice.",
      " I can help",
      " improve sales by converting",
      " more visitors",
      " to qualified leads and engage",
      " all visitors for you."
    ]
  },
  {
    user: [
    ...
    agent: [
      ...
      " Actual output will be natural voice.",
      " I'm here to",
      " assist you with increasing",
      " your conversion rates",
      " and helping turn",
      " website visitors into valuable ",
      " customers."
    ]
  },
  {
    user: [
      "I need help engaging",
      " visitors",
      " on my website. Can you assist?"
    ],
    agent: [
      "This is a mock output in robotic voide for developement only.",
      " Actual output will be natural voice.",
      " I can help boost your",
      " sales performance",
      " by engaging visitors more effectively and",
  ...
  },
  {
    ...
    agent: [
      ...
      " and better engage with all website visitors."
    ]
  },
  {
    user: [
    ...
    agent: [
      ...
      " Actual output will be natural voice.",
      " I can assist you",
      " in transforming more visitors into",
      " qualified leads",
      " and enhancing engagement across all your website traffic."
    ]
  },
  {
    user: [
      "What strategies can",
      " help me better engage",
    ...
    agent: [
      ...
      " and engage every visitor that comes",
      " to your website, turning them into valuable leads."
    ]
  }
];
let Ht = null;
function Sc() {
  return Ht || (Ht = _n[Math.floor(Math.random() * _n.length)]), Ht;
}
function Cc() {
  return Ht = _n[Math.floor(Math.random() * _n.length)], Ht;
}
function Ec(i) {
  return {
    roomName: `mock-room-${_i()}`,
    token: `mock-token-${_i()}`,
    url: "wss://mock-voice.example.com",
    clientId: i
  };
}
function _c(i) {
...
async function* Pc(i) {
  const e = Lr(), t = e.split(" "), n = [];
  for (let a = 0; a < t.length; a += 4) {
    const o = t.slice(a, a + 4).join(" ") + " ";
    n.push(o);
  }
...
`, await new Promise((d) => setTimeout(d, 50 + Math.random() * 50));
  }
  yield `data: ${JSON.stringify({
    type: "reference",
    content: {
      documents: Tc.filter((a) => a.type === "document")
      // apis: SAMPLE_REFERENCES.filter((r) => r.type === 'api').map((r) => ({ name: r.name })),
    },
    conversation_id: i
  })}
...
function Rc(i = !1, e, t, n) {
  const s = t === "listening", r = Sc(), a = s ? r.user : r.agent;
  let o;
  return n !== void 0 && n >= 0 && n < a.length ? o = a[n] : o = a.join(""), {
    participantIdentity: {
      identity: s ? "user" : "agent"
    },
    text: o,
    isTranscription: !0,
    isFinal: i,
...
function wc(i, e = 200) {
  const t = JSON.stringify(i), n = new Headers({
    "Content-Type": "application/json"
  });
  return new Response(t, {
    status: e,
    statusText: e === 200 ? "OK" : "Error",
    headers: n
  });
}
const Ur = /* @__PURE__ */ new Map();
function Ic(i) {
  Ur.set(i, Date.now()), console.log(`[Mock] Recorded tracking start for user ${i}`);
}
function Oc(i) {
  const e = Ur.get(i);
  if (!e)
    return !1;
  const t = Date.now() - e;
  return t >= 25e3 && t <= 35e3;
}
function Ac() {
  return "This is a test engagement hook message.";
}
const ts = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  createMockResponse: wc,
  generateMockChatResponse: _c,
  generateMockRoomCredentials: Ec,
  generateMockSSEChunks: Pc,
  generateMockTranscription: Rc,
  getMockHookMessage: Ac,
  recordTrackingStart: Ic,
  selectNewPair: Cc,
  shouldDeliverMockHook: Oc
...
var Dc = Object.defineProperty, xc = (i, e, t) => e in i ? Dc(i, e, {
  enumerable: !0,
  configurable: !0,
  writable: !0,
  value: t
}) : i[e] = t, Ns = (i, e, t) => xc(i, typeof e != "symbol" ? e + "" : e, t);
class ae {
  constructor() {
    Ns(this, "_locking"), Ns(this, "_locks"), this._locking = Promise.resolve(), this._locks = 0;
  }
  isLocked() {
...
const Nc = 34028234663852886e22, Lc = -34028234663852886e22, Uc = 4294967295, Fc = 2147483647, jc = -2147483648;
function cn(i) {
  if (typeof i != "number") throw new Error("invalid int 32: " + typeof i);
  if (!Number.isInteger(i) || i > Fc || i < jc) throw new Error("invalid int 32: " + i);
}
function Pi(i) {
  if (typeof i != "number") throw new Error("invalid uint 32: " + typeof i);
  if (!Number.isInteger(i) || i > Uc || i < 0) throw new Error("invalid uint 32: " + i);
}
function Fr(i) {
  if (typeof i != "number") throw new Error("invalid float 32: " + typeof i);
  if (Number.isFinite(i) && (i > Nc || i < Lc))
    throw new Error("invalid float 32: " + i);
}
const jr = Symbol("@bufbuild/protobuf/enum-type");
function Vc(i) {
  const e = i[jr];
...
function qc(i, e, t, n) {
  ...
  return Object.setPrototypeOf(a.prototype, new ns()), Object.assign(a, {
    runtime: i,
    typeName: e,
...
function Ls(i) {
  ...
  function r(a, o) {
    ...
    s *= t, n = n * t + c, n >= dn && (s = s + (n / dn | 0), n = n % dn);
  }
  return r(-24, -18), r(-18, -12), r(-12, -6), r(-6), e ? Kr(n, s) : is(n, s);
}
function Kc(i, e) {
  let t = is(i, e);
  const n = t.hi & 2147483648;
  n && (t = Kr(t.lo, t.hi));
  const s = Hr(t.lo, t.hi);
  return n ? "-" + s : s;
}
function Hr(i, e) {
  if ({
...
function Gc() {
  ...
  const t = (s) => Z(/^-?[0-9]+$/.test(s), "int64 invalid: ".concat(s)), n = (s) => Z(/^[0-9]+$/.test(s), "uint64 invalid: ".concat(s));
  return {
    zero: "0",
    supported: !1,
    parse(s) {
    ...
    uEnc(s) {
      return typeof s != "string" && (s = s.toString()), n(s), Ls(s);
    },
    dec(s, r) {
      return Kc(s, r);
    },
...
class zc {
   ...
   */
  int32(e) {
    return cn(e), Fs(e, this.buf), this;
  }
  /**
   ...
   */
  sint32(e) {
    return cn(e), e = (e << 1 ^ e >> 31) >>> 0, Fs(e, this.buf), this;
  }
  /**
   * Write a `fixed64` value, a signed, fixed-length 64-bit integer.
   */
  sfixed64(e) {
    let t = new Uint8Array(8), n = new DataView(t.buffer), s = z.enc(e);
   ...
   */
  fixed64(e) {
    let t = new Uint8Array(8), n = new DataView(t.buffer), s = z.uEnc(e);
    return n.setInt32(0, s.lo, !0), n.setInt32(4, s.hi, !0), this.raw(t);
  }
  /**
...
class Yc {
  constructor(e, t) {
    this.varint64 = Hc, this.uint32 = Jc, this.buf = e, this.len = e.length, this.pos = 0, this.view = new DataView(e.buffer, e.byteOffset, e.byteLength), this.textDecoder = t ?? new TextDecoder();
  }
  /**
...
let Ve = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".split(""), qn = [];
for (let i = 0; i < Ve.length; i++) qn[Ve[i].charCodeAt(0)] = i;
qn[45] = Ve.indexOf("+");
qn[95] = Ve.indexOf("/");
const Gr = {
  /**
  ...
  dec(i) {
    ...
    for (let o = 0; o < i.length; o++) {
      if (r = qn[i.charCodeAt(o)], r === void 0)
        switch (i[o]) {
          case "=":
            s = 0;
          case `
`:
  ...
  enc(i) {
    ...
    for (let r = 0; r < i.length; r++)
      switch (n = i[r], t) {
        case 0:
          e += Ve[n >> 2], s = (n & 3) << 4, t = 1;
          break;
        case 1:
          e += Ve[s | n >> 4], s = (n & 15) << 2, t = 2;
          break;
        case 2:
          e += Ve[s | n >> 6], e += Ve[n & 63], t = 0;
          break;
      }
    return t && (e += Ve[s], e += "=", t == 1 && (e += "=")), e;
  }
};
function Zc(i, e, t) {
  Yr(e, i);
  const n = e.runtime.bin.makeReadOptions(t), s = Xc(i.getType().runtime.bin.listUnknownFields(i), e.field), [r, a] = Jr(e);
  for (const o of s)
...
function Yr(i, e) {
  Z(i.extendee.typeName == e.getType().typeName, "extension ".concat(i.typeName, " can only be applied to message ").concat(i.extendee.typeName));
}
function Qr(i, e) {
  const t = i.localName;
  if (i.repeated)
  ...
  switch (i.kind) {
    case "enum":
    case "scalar":
      return i.opt || i.req ? e[t] !== void 0 : i.kind == "enum" ? e[t] !== i.T.values[0].no : !Wr(i.T, e[t]);
    case "message":
      return e[t] !== void 0;
    case "map":
...
function Be(i, e) {
  if (i === null || typeof i != "object" || !Object.getOwnPropertyNames(ns.prototype).every((n) => n in i && typeof i[n] == "function"))
    return !1;
  const t = i.getType();
  return t === null || typeof t != "function" || !("typeName" in t) || typeof t.typeName != "string" ? !1 : e === void 0 ? !0 : t.typeName == e.typeName;
}
function $r(i, e) {
  return Be(e) || !i.fieldWrapper ? e : i.fieldWrapper.wrapField(e);
}
...
function nd(i) {
  return i ? Object.assign(Object.assign({}, Bs), i) : Bs;
}
const Pn = Symbol(), ln = Symbol();
function id() {
  return {
    makeReadOptions: td,
    makeWriteOptions: nd,
      ...
      try {
        for (s of t.fields.byNumber()) {
          if (!Qr(s, i)) {
            ...
            if (!e.emitDefaultValues || !rd(s))
              continue;
          }
          const a = s.oneof ? i[s.oneof.localName].value : i[s.localName], o = Hs(s, a, e);
        ...
        if (r != null && r.findExtensionFor)
          for (const a of t.runtime.bin.listUnknownFields(i)) {
            ...
            if (o && zr(i, o)) {
              const c = Zc(i, o, e), d = Hs(o.field, c, e);
    ...
    readScalar(i, e, t) {
      return Kt(i, e, t ?? st.BIGINT, !0);
    },
    writeScalar(i, e, t) {
      if (e !== void 0 && (t || Wr(i, e)))
        return un(i, e);
    },
    debug: xe
  };
    ...
    for (const o of e) {
      ...
      switch (t.kind) {
        case "message":
          ...
          break;
        case "enum":
          const c = ai(t.T, o, n.ignoreUnknownFields, !0);
          c !== ln && a.push(c);
          break;
        case "scalar":
          try {
            a.push(Kt(t.T, o, t.L, !0));
          } catch (d) {
    ...
    for (const [o, c] of Object.entries(e)) {
      ...
      switch (t.V.kind) {
        case "message":
          ...
          break;
        case "enum":
          const l = ai(t.V.T, c, n.ignoreUnknownFields, !0);
          l !== ln && (a[d] = l);
          break;
        case "scalar":
          try {
            a[d] = Kt(t.V.T, c, st.BIGINT, !0);
          } catch (u) {
    ...
    switch (t.oneof && (i = i[t.oneof.localName] = {
      ...
      case "message":
        ...
        if (e === null && a.typeName != "google.protobuf.Value")
          return;
        let o = i[r];
        Be(o) ? o.fromJson(e, n) : (i[r] = o = a.fromJson(e, n), a.fieldWrapper && !t.oneof && (i[r] = a.fieldWrapper.unwrapField(o)));
        break;
      case "enum":
        const c = ai(t.T, e, n.ignoreUnknownFields, !1);
        switch (c) {
          case Pn:
            js(t, i);
            break;
...
function sd(i, e) {
  ...
  return Kt(i, e, st.BIGINT, !0).toString();
}
function Kt(i, e, t, n) {
  if (e === null)
    return n ? _t(i, t) : Pn;
  switch (i) {
    ...
    case C.UINT32:
      ...
      return i == C.UINT32 || i == C.FIXED32 ? Pi(r) : cn(r), r;
    case C.INT64:
    case C.SFIXED64:
    case C.SINT64:
...
function Hs(i, e, t) {
  if (i.kind == "map") {
    ...
    switch (i.V.kind) {
      ...
      case "enum":
        const r = i.V.T;
        for (const [a, o] of s)
          n[a.toString()] = ci(r, o, t.enumAsInteger);
        break;
    }
  ...
  if (i.repeated) {
    ...
    switch (i.kind) {
      ...
      case "enum":
        for (let s = 0; s < e.length; s++)
          n.push(ci(i.T, e[s], t.enumAsInteger));
        break;
      case "message":
        for (let s = 0; s < e.length; s++)
          n.push(e[s].toJson(t));
  ...
  switch (i.kind) {
    case "scalar":
      return un(i.T, e);
    case "enum":
      return ci(i.T, e, t.enumAsInteger);
    case "message":
      return $r(i.T, e).toJson(t);
  }
}
function ci(i, e, t) {
  var n;
...
function un(i, e) {
  switch (i) {
    ...
    case C.BYTES:
      return Z(e instanceof Uint8Array), Gr.enc(e);
  }
}
const vt = Symbol("@bufbuild/protobuf/unknown-fields"), Ks = {
  readUnknownFields: !0,
  readerFactory: (i) => new Yc(i)
}, Ws = {
  writeUnknownFields: !0,
  writerFactory: () => new zc()
};
function od(i) {
  return i ? Object.assign(Object.assign({}, Ks), i) : Ks;
}
function ad(i) {
  return i ? Object.assign(Object.assign({}, Ws), i) : Ws;
}
function cd() {
  return {
    makeReadOptions: od,
    ...
    readMessage(i, e, t, n, s) {
      ...
      let o, c;
      for (; e.pos < a && ([o, c] = e.tag(), !(s === !0 && c == ee.EndGroup)); ) {
...
function Js(i, e, t, n, s) {
  ...
  switch (t.oneof && (i = i[t.oneof.localName], i.case != a && delete i.value, i.case = a, a = "value"), t.kind) {
    case "scalar":
    case "enum":
      const o = t.kind == "enum" ? C.INT32 : t.T;
      let c = Rn;
      if (t.kind == "scalar" && t.L > 0 && (c = ld), r) {
        let h = i[a];
        if (n == ee.LengthDelimited && o != C.STRING && o != C.BYTES) {
    ...
    case "message":
      const d = t.T;
      r ? i[a].push(hn(e, new d(), s, t)) : Be(i[a]) ? hn(e, i[a], s, t) : (i[a] = hn(e, new d(), s, t), d.fieldWrapper && !t.oneof && !t.repeated && (i[a] = d.fieldWrapper.unwrapField(i[a])));
      break;
    case "map":
      let [l, u] = dd(t, e, s);
      i[a][l] = u;
      ...
      case 2:
        switch (i.V.kind) {
          ...
          case "enum":
            ...
            break;
          case "message":
            a = hn(e, new i.V.T(), t, void 0);
            break;
        }
        break;
    }
  }
  if (r === void 0 && (r = _t(i.K, st.BIGINT)), typeof r != "string" && typeof r != "number" && (r = r.toString()), a === void 0)
    switch (i.V.kind) {
      case "scalar":
        a = _t(i.V.T, st.BIGINT);
        break;
      case "enum":
        a = i.V.T.values[0].no;
        break;
...
function Gs(i, e, t, n) {
  ...
  switch (i.kind) {
    case "scalar":
    case "enum":
      let r = i.kind == "enum" ? C.INT32 : i.T;
      if (s)
        if (Z(Array.isArray(e)), i.packed)
          hd(t, r, i.no, e);
        else
          for (const a of e)
            Wt(t, r, i.no, a);
...
function ud(i, e, t, n, s) {
  ...
  switch (t.K) {
    ...
    case C.SINT32:
      ...
      break;
    case C.BOOL:
      Z(n == "true" || n == "false"), r = n == "true";
      break;
  }
  switch (Wt(i, t.K, 1, r), t.V.kind) {
    case "scalar":
      Wt(i, t.V.T, 2, s);
      break;
    case "enum":
      ...
      break;
    case "message":
      Z(s !== void 0), i.tag(2, ee.LengthDelimited).bytes(s.toBinary(e));
      break;
  }
  i.join();
}
        ...
        if (a[s] != null)
          switch (n.kind) {
            case "oneof":
              ...
              let d = a[s].value;
              c && c.kind == "message" && !Be(d, c.T) ? d = new c.T(d) : c && c.kind === "scalar" && c.T === C.BYTES && (d = Dt(d)), r[s] = {
                case: o,
                value: d
              };
              break;
            ...
            case "enum":
              let l = a[s];
              n.T === C.BYTES && (l = n.repeated ? l.map(Dt) : Dt(l)), r[s] = l;
              break;
            case "map":
              switch (n.V.kind) {
                case "scalar":
            ...
            case "message":
              const u = n.T;
              if (n.repeated)
                r[s] = a[s].map((h) => Be(h, u) ? h : new u(h));
              else {
                const h = a[s];
                u.fieldWrapper ? /* We can't use BytesValue.typeName as that will create a circular import */ u.typeName === "google.protobuf.BytesValue" ? r[s] = Dt(h) : r[s] = h : r[s] = Be(h, u) ? h : new u(h);
              }
              break;
          }
      }
    ...
    equals(i, e, t) {
      return e === t ? !0 : !e || !t ? !1 : i.fields.byMember().every((n) => {
        ...
        switch (n.kind) {
          case "message":
            let a = s, o = r;
            return n.T.fieldWrapper && (a !== void 0 && !Be(a) && (a = n.T.fieldWrapper.wrapField(a)), o !== void 0 && !Be(o) && (o = n.T.fieldWrapper.wrapField(o))), n.T.equals(a, o);
          case "enum":
...
const yd = /* @__PURE__ */ new Set([
  ...
  "toObject"
]), to = (i) => "".concat(i, "$"), kd = (i) => bd.has(i) ? to(i) : i, Td = (i) => yd.has(i) ? to(i) : i;
class Sd {
  constructor(e) {
    this.kind = "oneof", this.repeated = !1, this.packed = !1, this.opt = !1, this.req = !1, this.default = void 0, this.fields = [], this.name = e, this.localName = gd(e);
  }
  addField(e) {
...
class me extends ns {
  constructor(e) {
    super(), this.seconds = z.zero, this.nanos = 0, p.util.initPartial(e, this);
  }
  fromJson(e, t) {
    if (typeof e != "string")
    ...
    return this.seconds = z.parse(s / 1e3), this.nanos = 0, n[7] && (this.nanos = parseInt("1" + n[7] + "0".repeat(9 - n[7].length)) - 1e9), this;
  }
  toJson(e) {
  ...
  static now() {
    return me.fromDate(/* @__PURE__ */ new Date());
  }
  static fromDate(e) {
    const t = e.getTime();
    return new me({
      seconds: z.parse(Math.floor(t / 1e3)),
      nanos: t % 1e3 * 1e6
    });
  }
  ...
  static equals(e, t) {
    return p.util.equals(me, e, t);
  }
}
me.runtime = p;
me.typeName = "google.protobuf.Timestamp";
me.fields = p.util.newFieldList(() => [{
  no: 1,
  name: "seconds",
  kind: "scalar",
  T: 3
...
const Ed = /* @__PURE__ */ p.makeMessageType("livekit.MetricsBatch", () => [{
  no: 1,
  name: "timestamp_ms",
  kind: "scalar",
  T: 3
  ...
  T: Rd,
  repeated: !0
}]), _d = /* @__PURE__ */ p.makeMessageType("livekit.TimeSeriesMetric", () => [{
  no: 1,
  name: "label",
  kind: "scalar",
  T: 13
  ...
  T: 13
  /* ScalarType.UINT32 */
}]), Pd = /* @__PURE__ */ p.makeMessageType("livekit.MetricSample", () => [{
  no: 1,
  name: "timestamp_ms",
  kind: "scalar",
  T: 3
  ...
  T: 2
  /* ScalarType.FLOAT */
}]), Rd = /* @__PURE__ */ p.makeMessageType("livekit.EventMetric", () => [{
  no: 1,
  name: "label",
  kind: "scalar",
  T: 13
  ...
  T: 13
  /* ScalarType.UINT32 */
}]), wd = /* @__PURE__ */ p.makeEnum("livekit.AudioCodec", [{
  no: 0,
  name: "DEFAULT_AC"
}, {
  no: 1,
  ...
  no: 3,
  name: "AC_MP3"
}]), Id = /* @__PURE__ */ p.makeEnum("livekit.VideoCodec", [{
  no: 0,
  name: "DEFAULT_VC"
}, {
  no: 1,
  ...
  no: 4,
  name: "VP8"
}]), Od = /* @__PURE__ */ p.makeEnum("livekit.ImageCodec", [{
  no: 0,
  name: "IC_DEFAULT"
}, {
  no: 1,
  name: "IC_JPEG"
}]), no = /* @__PURE__ */ p.makeEnum("livekit.BackupCodecPolicy", [{
  no: 0,
  ...
  no: 2,
  name: "REGRESSION"
}]), Se = /* @__PURE__ */ p.makeEnum("livekit.TrackType", [{
  no: 0,
  name: "AUDIO"
}, {
  no: 1,
  ...
  no: 2,
  name: "DATA"
}]), te = /* @__PURE__ */ p.makeEnum("livekit.TrackSource", [{
  no: 0,
  name: "UNKNOWN"
}, {
  no: 1,
...
}]), ss = /* @__PURE__ */ p.makeEnum("livekit.VideoQuality", [{
  no: 0,
  name: "LOW"
}, {
  no: 1,
  ...
  no: 3,
  name: "OFF"
}]), jt = /* @__PURE__ */ p.makeEnum("livekit.ConnectionQuality", [{
  no: 0,
  name: "POOR"
}, {
  no: 1,
  ...
  no: 3,
  name: "LOST"
}]), zt = /* @__PURE__ */ p.makeEnum("livekit.ClientConfigSetting", [{
  no: 0,
  name: "UNSET"
}, {
  no: 1,
  ...
  no: 2,
  name: "ENABLED"
}]), Ce = /* @__PURE__ */ p.makeEnum("livekit.DisconnectReason", [{
  no: 0,
  name: "UNKNOWN_REASON"
}, {
  no: 1,
...
}]), io = /* @__PURE__ */ p.makeEnum("livekit.SubscriptionError", [{
  no: 0,
  name: "SE_UNKNOWN"
}, {
  no: 1,
...
}]), ie = /* @__PURE__ */ p.makeEnum("livekit.AudioTrackFeature", [{
  no: 0,
  name: "TF_STEREO"
}, {
  no: 1,
...
}]), Hn = /* @__PURE__ */ p.makeMessageType("livekit.Room", () => [{
  no: 1,
  name: "sid",
  kind: "scalar",
  T: 9
  ...
  kind: "message",
  T: vo
}]), wn = /* @__PURE__ */ p.makeMessageType("livekit.Codec", () => [{
  no: 1,
  name: "mime",
  kind: "scalar",
  T: 9
  ...
  T: 9
  /* ScalarType.STRING */
}]), Ad = /* @__PURE__ */ p.makeMessageType("livekit.ParticipantPermission", () => [{
  no: 1,
  name: "can_subscribe",
  kind: "scalar",
  T: 8
  ...
  T: p.getEnumType(Md),
  repeated: !0
}]), kt = /* @__PURE__ */ p.makeEnum("livekit.ParticipantInfo.State", [{
  no: 0,
  name: "JOINING"
}, {
  no: 1,
  ...
  no: 7,
  name: "CONNECTOR"
}]), Md = /* @__PURE__ */ p.makeEnum("livekit.ParticipantInfo.KindDetail", [{
  no: 0,
  name: "CLOUD_AGENT"
}, {
  no: 1,
  name: "FORWARDED"
}]), ne = /* @__PURE__ */ p.makeEnum("livekit.Encryption.Type", [{
  no: 0,
  name: "NONE"
}, {
  no: 1,
  ...
  no: 2,
  name: "CUSTOM"
}]), Dd = /* @__PURE__ */ p.makeMessageType("livekit.SimulcastCodecInfo", () => [{
  no: 1,
  name: "mime_type",
  kind: "scalar",
  T: 9
  ...
  T: 9
  /* ScalarType.STRING */
}]), so = /* @__PURE__ */ p.makeEnum("livekit.VideoLayer.Mode", [{
  no: 0,
  name: "MODE_UNUSED"
}, {
  no: 1,
...
}]), le = /* @__PURE__ */ p.makeMessageType("livekit.DataPacket", () => [{
  no: 1,
  name: "kind",
  kind: "enum",
  T: p.getEnumType(V)
  ...
  T: 9
  /* ScalarType.STRING */
}]), V = /* @__PURE__ */ p.makeEnum("livekit.DataPacket.Kind", [{
  no: 0,
  name: "RELIABLE"
}, {
  no: 1,
  name: "LOSSY"
}]), ro = /* @__PURE__ */ p.makeMessageType("livekit.EncryptedPacket", () => [{
  no: 1,
  ...
  T: 12
  /* ScalarType.BYTES */
}]), oo = /* @__PURE__ */ p.makeMessageType("livekit.EncryptedPacketPayload", () => [{
  no: 1,
  name: "user",
  kind: "message",
  T: rs,
  ...
  T: Mn,
  oneof: "value"
}]), xd = /* @__PURE__ */ p.makeMessageType("livekit.ActiveSpeakerUpdate", () => [{
  no: 1,
  name: "speakers",
  kind: "message",
  T: ao,
  repeated: !0
}]), ao = /* @__PURE__ */ p.makeMessageType("livekit.SpeakerInfo", () => [{
  no: 1,
  name: "sid",
  kind: "scalar",
  T: 9
  ...
  T: 8
  /* ScalarType.BOOL */
}]), rs = /* @__PURE__ */ p.makeMessageType("livekit.UserPacket", () => [{
  no: 1,
  name: "participant_sid",
  kind: "scalar",
  T: 9
  ...
  T: 12
  /* ScalarType.BYTES */
}]), co = /* @__PURE__ */ p.makeMessageType("livekit.SipDTMF", () => [{
  no: 3,
  name: "code",
  kind: "scalar",
  T: 13
  ...
  T: 9
  /* ScalarType.STRING */
}]), Nd = /* @__PURE__ */ p.makeMessageType("livekit.Transcription", () => [{
  no: 2,
  ...
  T: Ld,
  repeated: !0
}]), Ld = /* @__PURE__ */ p.makeMessageType("livekit.TranscriptionSegment", () => [{
  no: 1,
  name: "id",
  kind: "scalar",
  T: 9
  ...
  T: 9
  /* ScalarType.STRING */
}]), In = /* @__PURE__ */ p.makeMessageType("livekit.ChatMessage", () => [{
  no: 1,
  name: "id",
  kind: "scalar",
  T: 9
  ...
  T: 8
  /* ScalarType.BOOL */
}]), os = /* @__PURE__ */ p.makeMessageType("livekit.RpcRequest", () => [{
  no: 1,
  name: "id",
  kind: "scalar",
  T: 9
  ...
  T: 13
  /* ScalarType.UINT32 */
}]), as = /* @__PURE__ */ p.makeMessageType("livekit.RpcAck", () => [{
  no: 1,
  name: "request_id",
  kind: "scalar",
  T: 9
  /* ScalarType.STRING */
}]), cs = /* @__PURE__ */ p.makeMessageType("livekit.RpcResponse", () => [{
  no: 1,
  name: "request_id",
  kind: "scalar",
  T: 9
  ...
  T: lo,
  oneof: "value"
}]), lo = /* @__PURE__ */ p.makeMessageType("livekit.RpcError", () => [{
  no: 1,
  name: "code",
  kind: "scalar",
  T: 13
  ...
  T: 9
  /* ScalarType.STRING */
}]), uo = /* @__PURE__ */ p.makeMessageType("livekit.ParticipantTracks", () => [{
  no: 1,
  name: "participant_sid",
  kind: "scalar",
  T: 9
  ...
  T: 9,
  repeated: !0
}]), ho = /* @__PURE__ */ p.makeMessageType("livekit.ServerInfo", () => [{
  no: 1,
  name: "edition",
  kind: "enum",
  T: p.getEnumType(mo)
  ...
  T: 5
  /* ScalarType.INT32 */
}]), mo = /* @__PURE__ */ p.makeEnum("livekit.ServerInfo.Edition", [{
  no: 0,
  name: "Standard"
}, {
  no: 1,
  name: "Cloud"
}]), fo = /* @__PURE__ */ p.makeMessageType("livekit.ClientInfo", () => [{
  no: 1,
  name: "sdk",
  kind: "enum",
  T: p.getEnumType(po)
  ...
  T: 9
  /* ScalarType.STRING */
}]), po = /* @__PURE__ */ p.makeEnum("livekit.ClientInfo.SDK", [{
  no: 0,
  name: "UNKNOWN"
}, {
  no: 1,
  ...
  no: 14,
  name: "ESP32"
}]), go = /* @__PURE__ */ p.makeMessageType("livekit.ClientConfiguration", () => [{
  no: 1,
  name: "video",
  kind: "message",
  T: Ys
  ...
  kind: "enum",
  T: p.getEnumType(zt)
}]), Ys = /* @__PURE__ */ p.makeMessageType("livekit.VideoConfiguration", () => [{
  no: 1,
  name: "hardware_encoder",
  kind: "enum",
  T: p.getEnumType(zt)
}]), Ud = /* @__PURE__ */ p.makeMessageType("livekit.DisabledCodecs", () => [{
  no: 1,
  name: "codecs",
  kind: "message",
  T: wn,
  ...
  T: wn,
  repeated: !0
}]), vo = /* @__PURE__ */ p.makeMessageType("livekit.TimedVersion", () => [{
  no: 1,
  name: "unix_micro",
  kind: "scalar",
  T: 3
  ...
  no: 3,
  name: "REACTION"
}]), yo = /* @__PURE__ */ p.makeMessageType("livekit.DataStream.TextHeader", () => [{
  no: 1,
  name: "operation_type",
  kind: "enum",
  T: p.getEnumType(Ri)
...
}), On = /* @__PURE__ */ p.makeMessageType("livekit.DataStream.Header", () => [{
  no: 1,
  name: "stream_id",
  kind: "scalar",
  T: 9
...
}), An = /* @__PURE__ */ p.makeMessageType("livekit.DataStream.Chunk", () => [{
  no: 1,
  name: "stream_id",
  kind: "scalar",
  T: 9
...
}), Mn = /* @__PURE__ */ p.makeMessageType("livekit.DataStream.Trailer", () => [{
  no: 1,
  name: "stream_id",
  kind: "scalar",
  T: 9
...
}), Fd = /* @__PURE__ */ p.makeMessageType("livekit.FilterParams", () => [{
  no: 1,
  name: "include_events",
  kind: "scalar",
  T: 9,
  ...
  T: 9,
  repeated: !0
}]), jd = /* @__PURE__ */ p.makeMessageType("livekit.WebhookConfig", () => [{
  no: 1,
  name: "url",
  kind: "scalar",
  T: 9
  ...
  kind: "message",
  T: Fd
}]), Vd = /* @__PURE__ */ p.makeMessageType("livekit.SubscribedAudioCodec", () => [{
  no: 1,
  name: "codec",
  kind: "scalar",
  T: 9
  ...
  T: 8
  /* ScalarType.BOOL */
}]), wi = /* @__PURE__ */ p.makeMessageType("livekit.RoomAgentDispatch", () => [{
  no: 1,
  name: "agent_name",
  kind: "scalar",
  T: 9
  ...
  T: 9
  /* ScalarType.STRING */
}]), Bd = /* @__PURE__ */ p.makeEnum("livekit.EncodedFileType", [{
  no: 0,
  ...
  no: 3,
  name: "MP3"
}]), qd = /* @__PURE__ */ p.makeEnum("livekit.SegmentedFileProtocol", [{
  no: 0,
  name: "DEFAULT_SEGMENTED_FILE_PROTOCOL"
}, {
  no: 1,
  name: "HLS_PROTOCOL"
}]), Hd = /* @__PURE__ */ p.makeEnum("livekit.SegmentedFileSuffix", [{
  no: 0,
  name: "INDEX"
}, {
  no: 1,
  name: "TIMESTAMP"
}]), Kd = /* @__PURE__ */ p.makeEnum("livekit.ImageFileSuffix", [{
  no: 0,
...
}]), Wd = /* @__PURE__ */ p.makeEnum("livekit.StreamProtocol", [{
  no: 0,
  ...
  no: 2,
  name: "SRT"
}]), Jd = /* @__PURE__ */ p.makeEnum("livekit.AudioMixing", [{
  no: 0,
  name: "DEFAULT_MIXING"
}, {
  no: 1,
...
}]), ko = /* @__PURE__ */ p.makeEnum("livekit.EncodingOptionsPreset", [{
  no: 0,
  name: "H264_720P_30"
}, {
  no: 1,
...
}]), Gd = /* @__PURE__ */ p.makeMessageType("livekit.RoomCompositeEgressRequest", () => [{
  no: 1,
  name: "room_name",
  kind: "scalar",
  T: 9
  ...
  T: jd,
  repeated: !0
}]), Ii = /* @__PURE__ */ p.makeMessageType("livekit.EncodedFileOutput", () => [{
  no: 1,
  name: "file_type",
  kind: "enum",
  T: p.getEnumType(Bd)
  ...
  T: Gn,
  oneof: "output"
}]), Oi = /* @__PURE__ */ p.makeMessageType("livekit.SegmentedFileOutput", () => [{
  no: 1,
  name: "protocol",
  kind: "enum",
  T: p.getEnumType(qd)
  ...
  T: Gn,
  oneof: "output"
}]), zd = /* @__PURE__ */ p.makeMessageType("livekit.ImageOutput", () => [{
  no: 1,
  ...
  T: Gn,
  oneof: "output"
}]), Kn = /* @__PURE__ */ p.makeMessageType("livekit.S3Upload", () => [{
  no: 1,
  name: "access_key",
  kind: "scalar",
  T: 9
  ...
  kind: "message",
  T: To
}]), Wn = /* @__PURE__ */ p.makeMessageType("livekit.GCPUpload", () => [{
  no: 1,
  name: "credentials",
  kind: "scalar",
  T: 9
  ...
  T: 9
  /* ScalarType.STRING */
}]), Gn = /* @__PURE__ */ p.makeMessageType("livekit.AliOSSUpload", () => [{
  no: 1,
  name: "access_key",
  kind: "scalar",
  T: 9
  ...
  T: 9
  /* ScalarType.STRING */
}]), To = /* @__PURE__ */ p.makeMessageType("livekit.ProxyConfig", () => [{
  no: 1,
  name: "url",
  kind: "scalar",
  T: 9
  ...
  T: 9
  /* ScalarType.STRING */
}]), Qs = /* @__PURE__ */ p.makeMessageType("livekit.StreamOutput", () => [{
  no: 1,
  name: "protocol",
  kind: "enum",
  T: p.getEnumType(Wd)
  ...
  T: 9,
  repeated: !0
}]), So = /* @__PURE__ */ p.makeMessageType("livekit.EncodingOptions", () => [{
  no: 1,
  name: "width",
  kind: "scalar",
  T: 5
  ...
  T: 1
  /* ScalarType.DOUBLE */
}]), Yd = /* @__PURE__ */ p.makeMessageType("livekit.AutoParticipantEgress", () => [{
  no: 1,
  name: "preset",
  kind: "enum",
  T: p.getEnumType(ko),
  ...
  T: Oi,
  repeated: !0
}]), Qd = /* @__PURE__ */ p.makeMessageType("livekit.AutoTrackEgress", () => [{
  no: 1,
  name: "filepath",
  kind: "scalar",
  T: 9
  ...
  T: Gn,
  oneof: "output"
}]), $d = /* @__PURE__ */ p.makeMessageType("livekit.RoomEgress", () => [{
  no: 1,
  name: "room",
  kind: "message",
  T: Gd
  ...
  kind: "message",
  T: Qd
}]), Dn = /* @__PURE__ */ p.makeMessageType("livekit.RoomConfiguration", () => [{
  no: 1,
  name: "name",
  kind: "scalar",
  T: 9
  ...
  no: 1,
  name: "SUBSCRIBER"
}]), Ai = /* @__PURE__ */ p.makeEnum("livekit.StreamState", [{
  no: 0,
  name: "ACTIVE"
}, {
  no: 1,
  name: "PAUSED"
}]), Xd = /* @__PURE__ */ p.makeEnum("livekit.CandidateProtocol", [{
  no: 0,
  name: "UDP"
}, {
  no: 1,
  ...
  no: 2,
  name: "TLS"
}]), Zd = /* @__PURE__ */ p.makeMessageType("livekit.SignalRequest", () => [{
  no: 1,
  name: "offer",
  kind: "message",
  T: rt,
  ...
  T: Eo,
  oneof: "message"
}]), $s = /* @__PURE__ */ p.makeMessageType("livekit.SignalResponse", () => [{
  no: 1,
  name: "join",
  kind: "message",
  T: el,
  ...
  T: hl,
  oneof: "message"
}]), Mi = /* @__PURE__ */ p.makeMessageType("livekit.SimulcastCodec", () => [{
  no: 1,
  name: "codec",
  kind: "scalar",
  T: 9
  ...
  kind: "enum",
  T: p.getEnumType(so)
}]), Yt = /* @__PURE__ */ p.makeMessageType("livekit.AddTrackRequest", () => [{
  no: 1,
  name: "cid",
  kind: "scalar",
  T: 9
  ...
  T: p.getEnumType(ie),
  repeated: !0
}]), zn = /* @__PURE__ */ p.makeMessageType("livekit.TrickleRequest", () => [{
  no: 1,
  name: "candidateInit",
  kind: "scalar",
  T: 9
  ...
  T: 8
  /* ScalarType.BOOL */
}]), el = /* @__PURE__ */ p.makeMessageType("livekit.JoinResponse", () => [{
  no: 1,
  name: "room",
  kind: "message",
  T: Hn
  ...
  T: 8
  /* ScalarType.BOOL */
}]), tl = /* @__PURE__ */ p.makeMessageType("livekit.ReconnectResponse", () => [{
  no: 1,
  name: "ice_servers",
  kind: "message",
  T: Po,
  ...
  T: 13
  /* ScalarType.UINT32 */
}]), ds = /* @__PURE__ */ p.makeMessageType("livekit.TrackPublishedResponse", () => [{
  no: 1,
  name: "cid",
  kind: "scalar",
  T: 9
  ...
  kind: "message",
  T: yt
}]), nl = /* @__PURE__ */ p.makeMessageType("livekit.TrackUnpublishedResponse", () => [{
  no: 1,
  name: "track_sid",
  kind: "scalar",
  T: 9
...
}]), il = /* @__PURE__ */ p.makeMessageType("livekit.ParticipantUpdate", () => [{
  no: 1,
  name: "participants",
  kind: "message",
  T: mt,
  ...
  T: uo,
  repeated: !0
}]), Co = /* @__PURE__ */ p.makeMessageType("livekit.UpdateTrackSettings", () => [{
  no: 1,
  name: "track_sids",
  kind: "scalar",
  T: 9,
  ...
  T: 13
  /* ScalarType.UINT32 */
}]), ls = /* @__PURE__ */ p.makeMessageType("livekit.UpdateLocalAudioTrack", () => [{
  no: 1,
  name: "track_sid",
  kind: "scalar",
  T: 9
  ...
  T: p.getEnumType(ie),
  repeated: !0
}]), Eo = /* @__PURE__ */ p.makeMessageType("livekit.UpdateLocalVideoTrack", () => [{
  no: 1,
  name: "track_sid",
  kind: "scalar",
  T: 9
  ...
  T: 13
  /* ScalarType.UINT32 */
}]), $n = /* @__PURE__ */ p.makeMessageType("livekit.LeaveRequest", () => [{
  no: 1,
  name: "can_reconnect",
  kind: "scalar",
  T: 8
  ...
  kind: "message",
  T: gl
}]), Tt = /* @__PURE__ */ p.makeEnum("livekit.LeaveRequest.Action", [{
  no: 0,
  name: "DISCONNECT"
}, {
  no: 1,
  ...
  no: 2,
  name: "RECONNECT"
}]), _o = /* @__PURE__ */ p.makeMessageType("livekit.UpdateVideoLayers", () => [{
  no: 1,
  name: "track_sid",
  kind: "scalar",
  T: 9
  ...
  T: tt,
  repeated: !0
}]), us = /* @__PURE__ */ p.makeMessageType("livekit.UpdateParticipantMetadata", () => [{
  no: 1,
  name: "metadata",
  kind: "scalar",
  T: 9
  ...
  T: 9
  /* ScalarType.STRING */
}]), sl = /* @__PURE__ */ p.makeMessageType("livekit.SpeakersChanged", () => [{
  no: 1,
  name: "speakers",
  kind: "message",
  T: ao,
  repeated: !0
}]), rl = /* @__PURE__ */ p.makeMessageType("livekit.RoomUpdate", () => [{
  no: 1,
  name: "room",
  kind: "message",
  T: Hn
}]), ol = /* @__PURE__ */ p.makeMessageType("livekit.ConnectionQualityInfo", () => [{
  no: 1,
  name: "participant_sid",
  kind: "scalar",
  T: 9
  ...
  T: 2
  /* ScalarType.FLOAT */
}]), al = /* @__PURE__ */ p.makeMessageType("livekit.ConnectionQualityUpdate", () => [{
  no: 1,
  name: "updates",
  kind: "message",
  T: ol,
  repeated: !0
}]), cl = /* @__PURE__ */ p.makeMessageType("livekit.StreamStateInfo", () => [{
  no: 1,
  name: "participant_sid",
  kind: "scalar",
  T: 9
  ...
  kind: "enum",
  T: p.getEnumType(Ai)
}]), dl = /* @__PURE__ */ p.makeMessageType("livekit.StreamStateUpdate", () => [{
  no: 1,
  name: "stream_states",
  kind: "message",
  T: cl,
  repeated: !0
}]), hs = /* @__PURE__ */ p.makeMessageType("livekit.SubscribedQuality", () => [{
  no: 1,
  name: "quality",
  kind: "enum",
  T: p.getEnumType(ss)
  ...
  T: 8
  /* ScalarType.BOOL */
}]), ll = /* @__PURE__ */ p.makeMessageType("livekit.SubscribedCodec", () => [{
  no: 1,
  name: "codec",
  kind: "scalar",
  T: 9
  ...
  T: hs,
  repeated: !0
}]), ul = /* @__PURE__ */ p.makeMessageType("livekit.SubscribedQualityUpdate", () => [{
  no: 1,
  name: "track_sid",
  kind: "scalar",
  T: 9
  ...
  T: ll,
  repeated: !0
}]), hl = /* @__PURE__ */ p.makeMessageType("livekit.SubscribedAudioCodecUpdate", () => [{
  no: 1,
  name: "track_sid",
  kind: "scalar",
  T: 9
  ...
  T: Vd,
  repeated: !0
}]), Ro = /* @__PURE__ */ p.makeMessageType("livekit.TrackPermission", () => [{
  no: 1,
  name: "participant_sid",
  kind: "scalar",
  T: 9
  ...
  T: 9
  /* ScalarType.STRING */
}]), wo = /* @__PURE__ */ p.makeMessageType("livekit.SubscriptionPermission", () => [{
  no: 1,
  ...
  T: Ro,
  repeated: !0
}]), ml = /* @__PURE__ */ p.makeMessageType("livekit.SubscriptionPermissionUpdate", () => [{
  no: 1,
  name: "participant_sid",
  kind: "scalar",
  T: 9
  ...
  T: 8
  /* ScalarType.BOOL */
}]), fl = /* @__PURE__ */ p.makeMessageType("livekit.RoomMovedResponse", () => [{
  no: 1,
  name: "room",
  kind: "message",
  T: Hn
  ...
  T: mt,
  repeated: !0
}]), ms = /* @__PURE__ */ p.makeMessageType("livekit.SyncState", () => [{
  no: 1,
  name: "answer",
  kind: "message",
  T: rt
  ...
  T: Io,
  repeated: !0
}]), Io = /* @__PURE__ */ p.makeMessageType("livekit.DataChannelReceiveState", () => [{
  no: 1,
  name: "publisher_sid",
  kind: "scalar",
  T: 9
  ...
  T: 13
  /* ScalarType.UINT32 */
}]), Oo = /* @__PURE__ */ p.makeMessageType("livekit.DataChannelInfo", () => [{
  no: 1,
  name: "label",
  kind: "scalar",
  T: 9
  ...
  T: 8,
  oneof: "scenario"
}]), Ao = /* @__PURE__ */ p.makeMessageType("livekit.Ping", () => [{
  no: 1,
  name: "timestamp",
  kind: "scalar",
  T: 3
  ...
  T: 3
  /* ScalarType.INT64 */
}]), pl = /* @__PURE__ */ p.makeMessageType("livekit.Pong", () => [{
  no: 1,
  ...
  T: 3
  /* ScalarType.INT64 */
}]), gl = /* @__PURE__ */ p.makeMessageType("livekit.RegionSettings", () => [{
  no: 1,
  name: "regions",
  kind: "message",
  T: vl,
  repeated: !0
}]), vl = /* @__PURE__ */ p.makeMessageType("livekit.RegionInfo", () => [{
  no: 1,
  name: "region",
  kind: "scalar",
  T: 9
  ...
  T: 3
  /* ScalarType.INT64 */
}]), yl = /* @__PURE__ */ p.makeMessageType("livekit.SubscriptionResponse", () => [{
  no: 1,
  name: "track_sid",
  kind: "scalar",
  T: 9
  ...
  kind: "enum",
  T: p.getEnumType(io)
}]), bl = /* @__PURE__ */ p.makeMessageType("livekit.RequestResponse", () => [{
  no: 1,
  name: "request_id",
  kind: "scalar",
  T: 13
  ...
  T: Eo,
  oneof: "request"
}]), fs = /* @__PURE__ */ p.makeEnum("livekit.RequestResponse.Reason", [{
  no: 0,
  name: "OK"
}, {
  no: 1,
...
}]), kl = /* @__PURE__ */ p.makeMessageType("livekit.TrackSubscribed", () => [{
  no: 1,
  name: "track_sid",
  kind: "scalar",
  T: 9
  /* ScalarType.STRING */
}]), Mo = /* @__PURE__ */ p.makeMessageType("livekit.ConnectionSettings", () => [{
  no: 1,
  name: "auto_subscribe",
  kind: "scalar",
  T: 8
  ...
  T: 8
  /* ScalarType.BOOL */
}]), Tl = /* @__PURE__ */ p.makeMessageType("livekit.JoinRequest", () => [{
  no: 1,
  name: "client_info",
  kind: "message",
  T: fo
  ...
  kind: "message",
  T: ms
}]), Sl = /* @__PURE__ */ p.makeMessageType("livekit.WrappedJoinRequest", () => [{
  no: 1,
  name: "compression",
  kind: "enum",
  T: p.getEnumType(Cl)
  ...
  T: 12
  /* ScalarType.BYTES */
}]), Cl = /* @__PURE__ */ p.makeEnum("livekit.WrappedJoinRequest.Compression", [{
  no: 0,
  name: "NONE"
}, {
  no: 1,
  name: "GZIP"
}]), El = /* @__PURE__ */ p.makeMessageType("livekit.MediaSectionsRequirement", () => [{
  no: 1,
  name: "num_audios",
  kind: "scalar",
  T: 13
  ...
  T: 13
  /* ScalarType.UINT32 */
}]), _l = /* @__PURE__ */ p.makeMessageType("livekit.TokenSourceRequest", () => [{
  no: 1,
  name: "room_name",
  kind: "scalar",
  T: 9,
  ...
  T: Dn,
  opt: !0
}]), Do = /* @__PURE__ */ p.makeMessageType("livekit.TokenSourceResponse", () => [{
  no: 1,
  name: "server_url",
  kind: "scalar",
  T: 9
...
var mn = { exports: {} }, Rl = mn.exports, Xs;
function wl() {
    ...
    (function(e, t) {
      ...
      function o(v, E) {
        ...
        if (typeof k.bind == "function")
          return k.bind(v);
        try {
          return Function.prototype.bind.call(k, v);
        } catch {
          return function() {
            return Function.prototype.apply.apply(k, [v, arguments]);
          };
        }
      }
      function c() {
      ...
      function d(v) {
        return v === "debug" && (v = "log"), typeof console === t ? !1 : v === "trace" && n ? c : console[v] !== void 0 ? o(console, v) : console.log !== void 0 ? o(console, "log") : e;
      }
      function l() {
        for (var v = this.getLevel(), E = 0; E < s.length; E++) {
          var k = s[E];
          this[k] = E < v ? e : this.methodFactory(k, v, this.name);
        }
      ...
      function f(v, E) {
        ...
        function O() {
          ...
          if (!(typeof window === t || !b)) {
            ...
            if (typeof N === t)
              try {
                var j = window.document.cookie, Y = encodeURIComponent(b), ye = j.indexOf(Y + "=");
                ye !== -1 && (N = /^([^;]+)/.exec(j.slice(ye + Y.length + 1))[1]);
              } catch {
              }
...
var _e;
(function(i) {
  i.Default = "livekit", i.Room = "livekit-room", i.TokenSource = "livekit-token-source", i.Participant = "livekit-participant", i.Track = "livekit-track", i.Publication = "livekit-track-publication", i.Engine = "livekit-engine", i.Signal = "livekit-signal", i.PCManager = "livekit-pc-manager", i.PCTransport = "livekit-pc-transport", i.E2EE = "lk-e2ee";
})(_e || (_e = {}));
let F = en.getLogger("livekit");
const xo = Object.values(_e).map((i) => en.getLogger(i));
F.setDefaultLevel(Rt.info);
function Le(i) {
  const e = en.getLogger(i);
  return e.setDefaultLevel(F.getLevel()), e;
}
function Il(i, e) {
  if (e)
    en.getLogger(e).setLevel(i);
  else
    for (const t of xo)
      t.setLevel(i);
}
function Ol(i, e) {
  (e ? [e] : xo).forEach((n) => {
    ...
    n.methodFactory = (r, a, o) => {
      const c = s(r, a, o), d = Rt[r], l = d >= a && d < Rt.silent;
      return (u, h) => {
        h ? c(u, h) : c(u), l && i(d, u, h);
      };
    }, n.setLevel(n.getLevel());
  });
}
const Al = en.getLogger("lk-e2ee"), xt = 7e3, Ml = [0, 300, 2 * 2 * 300, 3 * 3 * 300, 4 * 4 * 300, xt, xt, xt, xt, xt];
class No {
  constructor(e) {
    this._retryDelays = e !== void 0 ? [...e] : Ml;
  }
...
function qe(i) {
  ...
  return e ? e.call(i) : (i = typeof Zs == "function" ? Zs(i) : i[Symbol.iterator](), t = {}, n("next"), n("throw"), n("return"), t[Symbol.asyncIterator] = function() {
    return this;
  }, t);
  function n(r) {
    t[r] = i[r] && function(a) {
      return new Promise(function(o, c) {
        a = i[r](a), s(o, c, a.done, a.value);
      });
    };
  }
  function s(r, a, o, c) {
...
function Dl() {
  ...
  on.exports = r, on.exports.once = k, r.EventEmitter = r, r.prototype._events = void 0, r.prototype._eventsCount = 0, r.prototype._maxListeners = void 0;
  var a = 10;
  function o(y) {
  ...
  r.prototype.getMaxListeners = function() {
    ...
    if (typeof j == "function")
      ...
      for (var Y = j.length, ye = g(j, Y), O = 0; O < Y; ++O) e(ye[O], this, S);
    return !0;
  };
  function d(y, b, S, O) {
    var L, D, A;
  ...
  r.prototype.once = function(b, S) {
    ...
    if (O === S || O.listener === S)
      ...
      if (D < 0) return this;
      D === 0 ? O.shift() : v(O, D), O.length === 1 && (L[b] = O[0]), L.removeListener !== void 0 && this.emit("removeListener", b, N || S);
    }
    return this;
  ...
  function h(y, b, S) {
    ...
    if (O === void 0) return [];
    var L = O[b];
    return L === void 0 ? [] : typeof L == "function" ? S ? [L.listener || L] : [L] : S ? E(L) : g(L, L.length);
  }
...
function xl(i) {
  return typeof i != "boolean" ? new Error("Argument type: " + typeof i + ". Please use a boolean.") : (Lo = i, i ? "adapter.js logging disabled" : "adapter.js logging enabled");
}
function Nl(i) {
  return typeof i != "boolean" ? new Error("Argument type: " + typeof i + ". Please use a boolean.") : (Uo = !i, "adapter.js deprecation warnings " + (i ? "disabled" : "enabled"));
}
function Fo() {
...
function gs(i, e) {
  Uo && console.warn(i + " is deprecated, please use " + e + " instead.");
}
function Ll(i) {
  const e = {
    browser: null,
  ...
  if (t.mozGetUserMedia)
    e.browser = "firefox", e.version = parseInt(Vt(t.userAgent, /Firefox\/(\d+)\./, 1));
  else if (t.webkitGetUserMedia || i.isSecureContext === !1 && i.webkitRTCPeerConnection)
    e.browser = "chrome", e.version = parseInt(Vt(t.userAgent, /Chrom(e|ium)\/(\d+)\./, 2));
  else if (i.RTCPeerConnection && t.userAgent.match(/AppleWebKit\/(\d+)\./))
    e.browser = "safari", e.version = parseInt(Vt(t.userAgent, /AppleWebKit\/(\d+)\./, 1)), e.supportsUnifiedPlan = i.RTCRtpTransceiver && "currentDirection" in i.RTCRtpTransceiver.prototype, e._safariVersion = Vt(t.userAgent, /Version\/(\d+(\.?\d+))/, 1);
  else
...
function jo(i) {
  return tr(i) ? Object.keys(i).reduce(function(e, t) {
    const n = tr(i[t]), s = n ? jo(i[t]) : i[t], r = n && !Object.keys(s).length;
...
function Di(i, e, t) {
  !e || t.has(e.id) || (t.set(e.id, e), Object.keys(e).forEach((n) => {
    n.endsWith("Id") ? Di(i, i.get(e[n]), t) : n.endsWith("Ids") && e[n].forEach((s) => {
      Di(i, i.get(s), t);
    });
  }));
}
function nr(i, e, t) {
  ...
  return i.forEach((a) => {
    ...
    i.forEach((o) => {
      o.type === n && o.trackId === a.id && Di(i, o, s);
    });
  }), s;
}
const ir = Fo;
        ...
        if (d.exact === "environment" || d.ideal === "environment" ? u = ["back", "rear"] : (d.exact === "user" || d.ideal === "user") && (u = ["front"]), u)
          return t.mediaDevices.enumerateDevices().then((h) => {
            ...
            return !f && h.length && u.includes("back") && (f = h[h.length - 1]), f && (o.video.deviceId = d.exact ? {
              ...
              ideal: f.deviceId
            }), o.video = n(o.video), ir("chrome: " + JSON.stringify(o)), c(o);
          });
      }
      o.video = n(o.video);
    }
    return ir("chrome: " + JSON.stringify(o)), c(o);
  }, r = function(o) {
...
function Ko(i) {
  ...
  if (!("getStats" in i.RTCRtpReceiver.prototype)) {
    ...
    t && (i.RTCPeerConnection.prototype.getReceivers = function() {
      ...
      const s = this;
      return this._pc.getStats().then((r) => nr(r, s.track, !1));
    };
  }
...
function Jo(i, e) {
  ...
  i.RTCPeerConnection.prototype.removeStream = function(l) {
    ...
    if (h.length !== 1 || !h[0].getTracks().find((v) => v === l))
      throw new DOMException("The adapter.js addTrack polyfill only supports a single  stream which is associated with the specified track.", "NotSupportedError");
    if (this.getSenders().find((v) => v.track === l))
      throw new DOMException("Track already exists.", "InvalidAccessError");
    this._streams = this._streams || {}, this._reverseStreams = this._reverseStreams || {};
    const g = this._streams[u.id];
    if (g)
      g.addTrack(l), Promise.resolve().then(() => {
        this.dispatchEvent(new Event("negotiationneeded"));
      });
    else {
      const v = new i.MediaStream([l]);
  ...
  function r(d, l) {
    ...
    return Object.keys(d._reverseStreams || []).forEach((h) => {
      const f = d._reverseStreams[h], g = d._streams[f.id];
  ...
  ["createOffer", "createAnswer"].forEach(function(d) {
    const l = i.RTCPeerConnection.prototype[d], u = {
      [d]() {
        const h = arguments;
        return arguments.length && typeof arguments[0] == "function" ? l.apply(this, [(g) => {
          const v = r(this, g);
          h[0].apply(null, [v]);
        }, (g) => {
          h[1] && h[1].apply(null, g);
        }, arguments[2]]) : l.apply(this, arguments).then((g) => r(this, g));
      }
    };
  ...
  Object.defineProperty(i.RTCPeerConnection.prototype, "localDescription", {
    ...
    Object.keys(this._streams).forEach((f) => {
      this._streams[f].getTracks().find((v) => l.track === v) && (h = this._streams[f]);
...
var sr = /* @__PURE__ */ Object.freeze({ __proto__: null, fixNegotiationNeeded: Go, shimAddTrackRemoveTrack: Jo, shimAddTrackRemoveTrackWithNative: Wo, shimGetSendersWithDtmf: Ho, shimGetUserMedia: Vo, shimMediaStream: Bo, shimOnTrack: qo, shimPeerConnection: xi, shimSenderReceiverGetStats: Ko });
function zo(i, e) {
  const t = i && i.navigator, n = i && i.MediaStreamTrack;
  if (t.getUserMedia = function(s, r, a) {
    gs("navigator.getUserMedia", "navigator.mediaDevices.getUserMedia"), t.mediaDevices.getUserMedia(s).then(r, a);
...
function Xo(i) {
  !i.RTCPeerConnection || "removeStream" in i.RTCPeerConnection.prototype || (i.RTCPeerConnection.prototype.removeStream = function(t) {
    gs("removeStream", "removeTrack"), this.getSenders().forEach((n) => {
...
var rr = /* @__PURE__ */ Object.freeze({ __proto__: null, shimAddTransceiver: ea, shimCreateAnswer: ia, shimCreateOffer: na, shimGetDisplayMedia: Ul, shimGetParameters: ta, shimGetUserMedia: zo, shimOnTrack: Yo, shimPeerConnection: Ni, shimRTCDataChannel: Zo, shimReceiverGetStats: $o, shimRemoveStream: Xo, shimSenderGetStats: Qo });
function sa(i) {
  if (!(typeof i != "object" || !i.RTCPeerConnection)) {
...
function aa(i) {
  ...
  if (e.mediaDevices && e.mediaDevices.getUserMedia) {
    ...
    e.mediaDevices.getUserMedia = (s) => n(ca(s));
  }
  ...
  i.RTCPeerConnection = function(n, s) {
    if (n && n.iceServers) {
      ...
      for (let a = 0; a < n.iceServers.length; a++) {
        ...
        o.urls === void 0 && o.url ? (gs("RTCIceServer.url", "RTCIceServer.urls"), o = JSON.parse(JSON.stringify(o)), o.urls = o.url, delete o.url, r.push(o)) : r.push(n.iceServers[a]);
      }
      n.iceServers = r;
    }
    return new e(n, s);
...
var or = /* @__PURE__ */ Object.freeze({ __proto__: null, shimAudioContext: ha, shimCallbacksAPI: oa, shimConstraints: ca, shimCreateOfferLegacy: ua, shimGetUserMedia: aa, shimLocalStreamsAPI: sa, shimRTCIceServerUrls: da, shimRemoteStreamsAPI: ra, shimTrackEventTransceiver: la }), di = { exports: {} }, ar;
function Fl() {
...
`), n.headerExtensions && n.headerExtensions.forEach((a) => {
      ...
      const n = [], s = e.parseRtpParameters(t), r = s.fecMechanisms.indexOf("RED") !== -1, a = s.fecMechanisms.indexOf("ULPFEC") !== -1, o = e.matchPrefix(t, "a=ssrc:").map((h) => e.parseSsrcMedia(h)).filter((h) => h.attribute === "cname"), c = o.length > 0 && o[0].ssrc;
      let d;
      const l = e.matchPrefix(t, "a=ssrc-group:FID").map((h) => h.substring(17).split(" ").map((g) => parseInt(g, 10)));
      ...
      return u.length && (u[0].indexOf("b=TIAS:") === 0 ? u = parseInt(u[0].substring(7), 10) : u[0].indexOf("b=AS:") === 0 ? u = parseInt(u[0].substring(5), 10) * 1e3 * 0.95 - 50 * 40 * 8 : u = void 0, n.forEach((h) => {
        h.maxBitrate = u;
      })), n;
...
var ma = Fl(), St = /* @__PURE__ */ Pl(ma), jl = /* @__PURE__ */ Mc({ __proto__: null, default: St }, [ma]);
function fn(i) {
  ...
  i.RTCIceCandidate = function(n) {
    if (typeof n == "object" && n.candidate && n.candidate.indexOf("a=") === 0 && (n = JSON.parse(JSON.stringify(n)), n.candidate = n.candidate.substring(2)), n.candidate && n.candidate.length) {
      const s = new e(n), r = St.parseCandidate(n.candidate);
      for (const a in r)
...
function Li(i) {
  !i.RTCIceCandidate || i.RTCIceCandidate && "relayProtocol" in i.RTCIceCandidate.prototype || gt(i, "icecandidate", (e) => {
    if (e.candidate) {
      const t = St.parseCandidate(e.candidate.candidate);
...
function pn(i, e) {
  ...
  const t = function(o) {
    if (!o || !o.sdp)
      return !1;
    const c = St.splitSections(o.sdp);
    return c.shift(), c.some((d) => {
      const l = St.parseMLine(d);
    ...
    const l = St.matchPrefix(o.sdp, "a=max-message-size:");
...
var Vl = /* @__PURE__ */ Object.freeze({ __proto__: null, removeExtmapAllowMixed: Fi, shimAddIceCandidateNullOrEmpty: vn, shimConnectionState: Ui, shimMaxMessageSize: pn, shimParameterlessSetLocalDescription: yn, shimRTCIceCandidate: fn, shimRTCIceCandidateRelayProtocol: Li, shimSendThrowTypeError: gn });
function Bl() {
  let {
    window: i
  ...
  switch (n.browser) {
    case "chrome":
      ...
      t("adapter.js shimming chrome."), s.browserShim = sr, vn(i, n), yn(i), Vo(i, n), Bo(i), xi(i, n), qo(i), Jo(i, n), Ho(i), Ko(i), Go(i, n), fn(i), Li(i), Ui(i), pn(i, n), gn(i), Fi(i, n);
      break;
    case "firefox":
      if (!rr || !Ni || !e.shimFirefox)
        return t("Firefox shim is not included in this adapter release."), s;
      t("adapter.js shimming firefox."), s.browserShim = rr, vn(i, n), yn(i), zo(i, n), Ni(i, n), Yo(i), Xo(i), Qo(i), $o(i), Zo(i), ea(i), ta(i), na(i), ia(i), fn(i), Ui(i), pn(i, n), gn(i);
      break;
    case "safari":
      if (!or || !e.shimSafari)
        return t("Safari shim is not included in this adapter release."), s;
      t("adapter.js shimming safari."), s.browserShim = or, vn(i, n), yn(i), da(i), ua(i), oa(i), sa(i), ra(i), la(i), aa(i), ha(i), fn(i), Li(i), pn(i, n), gn(i), Fi(i, n);
      break;
    default:
...
const Hl = [
  {
    ...
    describe(i) {
      return {
        name: "Firefox",
        version: bn(/(?:firefox|iceweasel|fxios)[\s/](\d+(\.?_?\d+)+)/i, i),
  ...
  },
  {
    ...
    describe(i) {
      return {
        name: "Chrome",
        version: bn(/(?:chrome|chromium|crios|crmo)\/(\d+(\.?_?\d+)+)/i, i),
...
function ui(i) {
  return i.includes("mac os") ? bn(/\(.+?(\d+_\d+(:?_\d+)?)/, i, 1).replace(/_/g, ".") : void 0;
}
var Kl = "2.17.0";
const fa = Kl, pa = 16;
...
class U extends ve {
  constructor(e, t, n, s) {
    super(1, e), this.name = "ConnectionError", this.status = n, this.reason = t, this.context = s, this.reasonName = J[t];
  }
  static notAllowed(e, t, n) {
    return new U(e, J.NotAllowed, t, n);
  }
  static timeout(e) {
    return new U(e, J.Timeout);
  }
  static leaveRequest(e, t) {
    return new U(e, J.LeaveRequest, void 0, t);
  }
  static internal(e, t) {
    return new U(e, J.InternalError, void 0, t);
  }
  static cancelled(e) {
    return new U(e, J.Cancelled);
  }
  static serverUnreachable(e, t) {
    return new U(e, J.ServerUnreachable, t);
  }
  static websocket(e, t, n) {
    return new U(e, J.WebSocket, t, n);
  }
  static serviceNotFound(e, t) {
    return new U(e, J.ServiceNotFound, void 0, t);
  }
}
class tn extends ve {
  constructor(e) {
...
class Vi extends ve {
  constructor(e, t) {
    super(15, e), this.name = "SignalRequestError", this.reason = t, this.reasonName = typeof t == "string" ? t : fs[t];
  }
}
var ce;
(function(i) {
...
re.setInterval = // eslint-disable-next-line @typescript-eslint/no-implied-eval
function() {
...
const va = ["opus", "red"], zl = ["vp8", "h264"], ya = ["vp8", "h264", "vp9", "av1", "h265"];
function ba(i) {
  return !!zl.find((e) => e === i);
}
const ka = ba;
var Bi;
(function(i) {
...
const wt = {
  h90: new W(160, 90, 9e4, 20),
  h180: new W(320, 180, 16e4, 20),
  h216: new W(384, 216, 18e4, 20),
  h360: new W(640, 360, 45e4, 20),
  h540: new W(960, 540, 8e5, 25),
  h720: new W(1280, 720, 17e5, 30),
  h1080: new W(1920, 1080, 3e6, 30),
  h1440: new W(2560, 1440, 5e6, 30),
  h2160: new W(3840, 2160, 8e6, 30)
}, Nn = {
  h120: new W(160, 120, 7e4, 20),
  h180: new W(240, 180, 125e3, 20),
  h240: new W(320, 240, 14e4, 20),
  h360: new W(480, 360, 33e4, 20),
  h480: new W(640, 480, 5e5, 20),
  h540: new W(720, 540, 6e5, 25),
  h720: new W(960, 720, 13e5, 30),
  h1080: new W(1440, 1080, 23e5, 30),
  h1440: new W(1920, 1440, 38e5, 30)
}, nn = {
  h360fps3: new W(640, 360, 2e5, 3, "medium"),
  h360fps15: new W(640, 360, 4e5, 15, "medium"),
  h720fps5: new W(1280, 720, 8e5, 5, "medium"),
  h720fps15: new W(1280, 720, 15e5, 15, "medium"),
  h720fps30: new W(1280, 720, 2e6, 30, "medium"),
  h1080fps15: new W(1920, 1080, 25e5, 15, "medium"),
  h1080fps30: new W(1920, 1080, 5e6, 30, "medium"),
  // original resolution, without resizing
  original: new W(0, 0, 7e6, 30, "medium")
};
function Ta(i, e, t) {
  var n, s, r, a;
  const {
    ...
    audioProcessor: c,
    videoProcessor: d
  } = Ea(i ?? {}), l = e == null ? void 0 : e.processor, u = t == null ? void 0 : t.processor, h = o ?? {};
  return h.audio === !0 && (h.audio = {}), h.video === !0 && (h.video = {}), h.audio && (qi(h.audio, e), (n = (r = h.audio).deviceId) !== null && n !== void 0 || (r.deviceId = {
    ideal: "default"
  }), (c || l) && (h.audio.processor = c ?? l)), h.video && (qi(h.video, t), (s = (a = h.video).deviceId) !== null && s !== void 0 || (a.deviceId = {
    ideal: "default"
  ...
  return m(this, arguments, void 0, function(e) {
    ...
    return function* () {
      ...
      if (n) {
        ...
        n.createMediaStreamSource(new MediaStream([e.mediaStreamTrack])).connect(s), yield oe(t), s.getByteTimeDomainData(a);
...
function Zl(i) {
  switch (i) {
    ...
    case te.SCREEN_SHARE:
      return T.Source.ScreenShare;
    case te.SCREEN_SHARE_AUDIO:
...
class T extends Fe.EventEmitter {
  /**
  ...
  constructor(e, t) {
    ...
    super(), this.attachedElements = [], this.isMuted = !1, this._streamState = T.StreamState.Active, this.isInBackground = !1, this._currentBitrate = 0, this.log = F, this.appVisibilityChangedListener = () => {
      this.backgroundTimeout && clearTimeout(this.backgroundTimeout), document.visibilityState === "hidden" ? this.backgroundTimeout = setTimeout(() => this.handleAppVisibilityChanged(), tu) : this.handleAppVisibilityChanged();
    }, this.log = Le((s = n.loggerName) !== null && s !== void 0 ? s : _e.Track), this.loggerContextCb = n.loggerContextCb, this.setMaxListeners(100), this.kind = t, this._mediaStreamTrack = e, this._mediaStreamID = e.id, this.source = T.Source.Unknown;
  }
  get logContext() {
    var e;
  ...
  attach(e) {
    ...
    }), e && Nt.splice(Nt.indexOf(e), 1)), e || (e = document.createElement(t))), this.attachedElements.includes(e) || this.attachedElements.push(e), lt(this.mediaStreamTrack, e);
    const n = e.srcObject.getTracks(), s = n.some((r) => r.kind === "audio");
    return e.play().then(() => {
      this.emit(s ? R.AudioPlaybackStarted : R.VideoPlaybackStarted);
    }).catch((r) => {
      r.name === "NotAllowedError" ? this.emit(s ? R.AudioPlaybackFailed : R.VideoPlaybackFailed, r) : r.name === "AbortError" ? F.debug("".concat(s ? "audio" : "video", " playback aborted, likely due to new play request")) : F.warn("could not playback ".concat(s ? "audio" : "video"), r), s && e && n.some((a) => a.kind === "video") && r.name === "NotAllowedError" && (e.muted = !0, e.play().catch(() => {
      }));
  ...
  detach(e) {
    try {
      ...
      return this.attachedElements.forEach((n) => {
        ht(this.mediaStreamTrack, n), t.push(n), this.recycleElement(n), this.emit(R.ElementDetached, n);
  ...
  updateLoggerOptions(e) {
    e.loggerName && (this.log = Le(e.loggerName)), e.loggerContextCb && (this.loggerContextCb = e.loggerContextCb);
  }
  recycleElement(e) {
    if (e instanceof HTMLAudioElement) {
      let t = !0;
      e.pause(), Nt.forEach((n) => {
        n.parentElement || (t = !1);
      }), t && Nt.push(e);
    }
  }
  ...
  addAppVisibilityListener() {
    ge() ? (this.isInBackground = document.visibilityState === "hidden", document.addEventListener("visibilitychange", this.appVisibilityChangedListener)) : this.isInBackground = !1;
  }
...
(function(i) {
  ...
  function a(d) {
    switch (d) {
      case t.Camera:
        return te.CAMERA;
      case t.Microphone:
        return te.MICROPHONE;
      case t.ScreenShare:
        return te.SCREEN_SHARE;
  ...
  i.streamStateFromProto = c;
})(T || (T = {}));
const nu = "|", dr = "https://aomediacodec.github.io/av1-rtp-spec/#dependency-descriptor-rtp-header-extension";
function iu(i) {
  const e = i.split(nu);
  return e.length > 1 ? [e[0], i.substr(e[0].length + 1)] : [i, ""];
}
function oe(i) {
  return new fe((e) => re.setTimeout(e, i));
}
function Ln() {
...
function Un(i) {
  return !document || $t() ? !1 : (i || (i = document.createElement("audio")), "setSinkId" in i);
}
function ou() {
  return Un();
}
function Ra() {
  return typeof RTCPeerConnection > "u" ? !1 : Ln() || Ki();
}
function ft() {
  var i;
  return ((i = ue()) === null || i === void 0 ? void 0 : i.name) === "Firefox";
}
function lr() {
  const i = ue();
...
function pt() {
  var i;
  return ((i = ue()) === null || i === void 0 ? void 0 : i.name) === "Safari";
}
function $t() {
  const i = ue();
...
function au(i) {
  return i || (i = ue()), (i == null ? void 0 : i.name) === "Safari" && Me(i.version, "18.3") > 0 || (i == null ? void 0 : i.os) === "iOS" && !!(i != null && i.osVersion) && Me(i.osVersion, "18.3") > 0;
}
function Ia() {
  var i, e;
  return ge() ? (
...
function hi(i) {
  return It(i) ? i.hostname.split(".")[0] : null;
}
function Oa() {
...
let mi = null;
const hr = () => (mi || (mi = new ResizeObserver(du)), mi);
let fi = null;
const mr = () => (fi || (fi = new IntersectionObserver(lu, {
  root: null,
  rootMargin: "0px"
})), fi);
function uu() {
  ...
  return Ue() && (e.os = (i = Aa()) !== null && i !== void 0 ? i : ""), e;
}
let pi;
function hu() {
  return pi || (pi = Wi()), pi.clone();
}
function Wi() {
...
function kn() {
  if (!Lt) {
    ...
    if (e.connect(t), t.connect(n), e.start(), [Lt] = n.stream.getAudioTracks(), !Lt)
...
class Ee {
  ...
  constructor(e, t) {
    this._isResolved = !1, this.onFinally = t, this.promise = new Promise((n, s) => m(this, void 0, void 0, function* () {
      ...
      var n;
      this._isResolved = !0, (n = this.onFinally) === null || n === void 0 || n.call(this);
    });
  }
}
function mu(i, e) {
...
function gu(i, e) {
  return i.segments.map((t) => {
    ...
    var d;
    const l = (d = e.get(n)) !== null && d !== void 0 ? d : Date.now(), u = Date.now();
    return c ? e.delete(n) : e.set(n, l), {
      id: n,
      text: s,
...
function fr(i) {
  switch (i.reason) {
    case J.LeaveRequest:
      return i.context;
    case J.Cancelled:
      return Ce.CLIENT_INITIATED;
    case J.NotAllowed:
      return Ce.USER_REJECTED;
    case J.ServerUnreachable:
      return Ce.JOIN_FAILURE;
    default:
      return Ce.UNKNOWN_REASON;
  }
}
function Tn(i) {
...
function Cu(i, e) {
  const t = new URL(pu(i));
  return e.forEach((n, s) => {
    t.searchParams.set(s, n);
  }), bs(t, "rtc");
}
function Eu(i) {
  const e = new URL(Xt(i));
...
function bs(i, e) {
  return i.pathname = "".concat(xa(i.pathname)).concat(e), i;
}
function pr(i) {
  if (typeof i == "string")
    return $s.fromJson(JSON.parse(i), {
      ignoreUnknownFields: !0
    });
  if (i instanceof ArrayBuffer)
    return $s.fromBinary(new Uint8Array(i));
...
const Na = "AES-GCM", Pu = 10, Ut = "lk_e2ee", Ru = "LKFrameEncryptionKey", wu = {
  sharedKey: !1,
  ratchetSalt: Ru,
  ratchetWindowSize: 8,
  failureTolerance: Pu,
...
function Au(i, e) {
  return m(this, void 0, void 0, function* () {
    const t = Va(i.algorithm.name, e), n = yield crypto.subtle.deriveKey(t, i, {
      name: Na,
      length: 128
    }, !1, ["encrypt", "decrypt"]);
    return {
...
function Uu(i) {
  ...
  for (var t = 0, n = 0; n < i.length; ++n) {
    var s = i[n];
    s <= gr && t >= Lu && (e.push(gr), t = 0), e.push(s), s == 0 ? ++t : t = 0;
  }
  return new Uint8Array(e);
}
function Ba(i) {
...
class qa extends Fe.EventEmitter {
  constructor() {
    let e = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : {};
    super(), this.onKeyRatcheted = (t, n, s) => {
      F.debug("key ratcheted event received", {
        ratchetResult: t,
        participantId: n,
        keyIndex: s
      });
    }, this.keyInfoMap = /* @__PURE__ */ new Map(), this.options = Object.assign(Object.assign({}, wu), e), this.on(We.KeyRatcheted, this.onKeyRatcheted);
  }
  /**
  ...
  onSetEncryptionKey(e, t, n) {
    ...
    this.keyInfoMap.set("".concat(t ?? "shared", "-").concat(n ?? 0), s), this.emit(We.SetKey, s);
  }
  getKeys() {
  ...
  ratchetKey(e, t) {
    this.emit(We.RatchetRequest, e, t);
  }
}
class Fu extends qa {
  constructor() {
  ...
  setKey(e) {
    return m(this, void 0, void 0, function* () {
      const t = typeof e == "string" ? yield Fa(e) : yield ja(e);
...
class ju extends ve {
  constructor(e) {
    let t = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : Vn.InternalError, n = arguments.length > 2 ? arguments[2] : void 0;
    super(40, e), this.reason = t, this.participantIdentity = n;
  }
}
class Vu extends Fe.EventEmitter {
  constructor(e, t) {
    super(), this.decryptDataRequests = /* @__PURE__ */ new Map(), this.encryptDataRequests = /* @__PURE__ */ new Map(), this.onWorkerMessage = (n) => {
      ...
      switch (a) {
        case "error":
          if (F.error(o.error.message), o.uuid) {
          ...
          this.emit(Ke.EncryptionError, o.error, o.participantIdentity);
          break;
        case "initAck":
        ...
        case "enable":
          if (o.enabled && this.keyProvider.getKeys().forEach((l) => {
            ...
            this.emit(Ke.ParticipantEncryptionStatusChanged, o.enabled, this.room.localParticipant), this.encryptionEnabled = o.enabled;
            ...
            this.emit(Ke.ParticipantEncryptionStatusChanged, o.enabled, l);
          }
          break;
        case "ratchetKey":
          this.keyProvider.emit(We.KeyRatcheted, o.ratchetResult, o.participantIdentity, o.keyIndex);
          break;
      ...
      F.error("e2ee worker encountered an error:", {
        error: n.error
      }), this.emit(Ke.EncryptionError, n.error, void 0);
  ...
  setup(e) {
    if (!La())
      throw new tn("tried to setup end-to-end encryption on an unsupported browser");
    if (F.info("setting up e2ee"), e !== this.room) {
      this.room = e, this.setupEventListeners(e, this.keyProvider);
      const t = {
        kind: "init",
        data: {
          keyProviderOptions: this.keyProvider.getOptions(),
          loglevel: Al.getLevel()
        }
      };
  ...
  setParticipantCryptorEnabled(e, t) {
    F.debug("set e2ee to ".concat(e, " for participant ").concat(t)), this.postEnable(e, t);
  }
  /**
   * @internal
   */
    ...
    e.on(P.TrackPublished, (n, s) => this.setParticipantCryptorEnabled(n.trackInfo.encryption !== ne.NONE, s.identity)), e.on(P.ConnectionStateChanged, (n) => {
      ...
      const s = {
        ...
        data: {
          ...
          codec: Jt(n.trackInfo.codecs[0].mimeType),
          participantIdentity: this.room.localParticipant.identity
        }
      };
      this.worker.postMessage(s);
    }), t.on(We.SetKey, (n) => this.postKey(n)).on(We.RatchetRequest, (n, s) => this.postRatchetRequest(n, s));
  }
  encryptData(e) {
    return m(this, void 0, void 0, function* () {
      if (!this.worker)
  ...
  postKey(e) {
    ...
    const a = {
      ...
      data: {
        participantIdentity: n,
        isPublisher: n === ((r = this.room) === null || r === void 0 ? void 0 : r.localParticipant.identity),
        key: t,
        keyIndex: s
      }
    };
  ...
  setupE2EEReceiver(e, t, n) {
    if (e.receiver) {
      ...
      this.handleReceiver(e.receiver, e.mediaStreamID, t, e.kind === "video" ? Jt(n.mimeType) : void 0);
    }
  }
  setupE2EESender(e, t) {
    if (!it(e) || !t) {
  ...
  handleReceiver(e, t, n, s) {
    return m(this, void 0, void 0, function* () {
      if (this.worker) {
        if (jn() && // Chrome occasionally throws an `InvalidState` error when using script transforms directly after introducing this API in 141.
  ...
  handleSender(e, t, n) {
    ...
    if (!(Ut in e || !this.worker)) {
      ...
      // Disabling it for Chrome based browsers until the API has stabilized
      !lr()) {
        F.info("initialize script transform");
        const r = {
          kind: "encode",
          participantIdentity: this.room.localParticipant.identity,
          trackId: t,
        ...
        F.info("initialize encoded streams");
...
class Ct {
  ...
  addFailedConnectionAttempt(e) {
    var t;
    const n = new URL(e), s = hi(n);
    if (!s)
      return;
    let r = (t = this.failedConnectionAttempts.get(s)) !== null && t !== void 0 ? t : 0;
    this.failedConnectionAttempts.set(s, r + 1), this.backOffPromises.set(s, oe(Math.min(Bu * Math.pow(2, r), qu)));
  }
  getBackOffPromise(e) {
    const t = new URL(e), n = t && hi(t);
...
class Hu {
  ...
  run(e) {
    return m(this, void 0, void 0, function* () {
      ...
      try {
        return t.executedAt = Date.now(), t.status = Gt.RUNNING, yield e();
      } finally {
        t.status = Gt.COMPLETED, this.pendingTasks.delete(t.id), n();
      }
    });
  }
  flush() {
...
class Ku {
  ...
  constructor(e) {
    ...
    this.opened = new fe((o, c) => {
      const d = () => {
        c(U.websocket("Encountered websocket error during connection establishment"));
      };
      r.onopen = () => {
        o({
      ...
      const d = () => m(this, void 0, void 0, function* () {
        ...
        }), u = yield fe.race([oe(250), l]);
        u ? o(u) : c(U.websocket("Encountered unspecified websocket error without a timely close event"));
      });
      r.onclose = (l) => {
        let {
          code: u,
...
const Wu = ["syncState", "trickle", "offer", "answer", "simulate", "leave"];
function Ju(i) {
  const e = Wu.indexOf(i.case) >= 0;
  return F.trace("request allowed to bypass queue:", {
    canPass: e,
    req: i
  }), e;
}
...
class ks {
  ...
  constructor() {
    ...
    var n;
    this.rtt = 0, this.state = G.DISCONNECTED, this.log = F, this._requestId = 0, this.useV0SignalPath = !1, this.resetCallbacks = () => {
    ...
    }, this.log = Le((n = t.loggerName) !== null && n !== void 0 ? n : _e.Signal), this.loggerContextCb = t.loggerContextCb, this.useJSON = e, this.requestQueue = new Hu(), this.queuedRequests = [], this.closingLock = new ae(), this.connectionLock = new ae(), this.state = G.DISCONNECTED;
  }
  get logContext() {
    var e, t;
  ...
  join(e, t, n, s) {
    return m(this, arguments, void 0, function(r, a, o, c) {
      ...
      return function* () {
        return d.state = G.CONNECTING, d.options = o, yield d.connect(r, a, o, c, l);
      }();
    });
  }
  reconnect(e, t, n, s) {
    return m(this, void 0, void 0, function* () {
      ...
      return this.state = G.RECONNECTING, this.clearPingInterval(), yield this.connect(e, t, Object.assign(Object.assign({}, this.options), {
        reconnect: !0,
        sid: n,
        reconnectReason: s
  ...
  connect(e, t, n, s) {
    return m(this, arguments, void 0, function(r, a, o, c) {
      ...
      return function* () {
        ...
        const h = uu(), f = l ? zu(a, h, o) : Yu(a, h, o), g = Su(r, f, l).toString(), v = Eu(g).toString();
          ...
          try {
            ...
            const b = (A) => m(this, void 0, void 0, function* () {
              if (y)
                return;
              y = !0;
              const N = A instanceof Event ? A.currentTarget : A, j = _u(N, "Abort handler called");
              this.streamWriter && !this.isDisconnected ? this.sendLeave().then(() => this.close(j)).catch((Y) => {
                this.log.error(Y), this.close();
            ...
            }, this.logContext)), this.ws && (yield this.close(!1)), this.ws = new Ku(g);
            try {
              this.ws.closed.then((de) => {
                ...
                this.isEstablishingConnection && k(U.internal("Websocket got closed during a (re)connection attempt: ".concat(de.reason))), de.closeCode !== 1e3 && (this.log.warn("websocket closed", Object.assign(Object.assign({}, this.logContext), {
                  ...
                  state: this.state
                })), this.state === G.CONNECTED && this.handleOnClose((Ye = de.reason) !== null && Ye !== void 0 ? Ye : "Unexpected WS error"));
              }).catch((de) => {
              ...
              const A = yield this.ws.opened.catch((de) => m(this, void 0, void 0, function* () {
                if (this.state !== G.CONNECTED) {
                  this.state = G.DISCONNECTED, clearTimeout(O);
                  const Ye = yield this.handleConnectionError(de, v);
                  k(Ye);
                  return;
                }
              ...
              const Y = pr(j.value), ye = this.validateFirstMessage(Y, (_ = o.reconnect) !== null && _ !== void 0 ? _ : !1);
              if (!ye.isValid) {
                k(ye.error);
                return;
              }
              ((M = Y.message) === null || M === void 0 ? void 0 : M.case) === "join" && (this.pingTimeoutDuration = Y.message.value.pingTimeout, this.pingIntervalDuration = Y.message.value.pingInterval, this.pingTimeoutDuration && this.pingTimeoutDuration > 0 && this.log.debug("ping config", Object.assign(Object.assign({}, this.logContext), {
                timeout: this.pingTimeoutDuration,
                interval: this.pingIntervalDuration
              })));
              const ot = ye.shouldProcessFirstMessage ? Y : void 0;
              L(A, ot), E(ye.response);
            } catch (A) {
              k(A);
            } finally {
              S();
      ...
      return function* () {
        ...
        try {
          if (e.clearPingInterval(), t && (e.state = G.DISCONNECTING), e.ws) {
            ...
            const r = e.ws.closed;
            e.ws = void 0, e.streamWriter = void 0, yield Promise.race([r, oe(Gu)]);
          }
        } catch (r) {
  ...
  sendPing() {
    return Promise.all([this.sendRequest({
      case: "ping",
      value: z.parse(Date.now())
    }), this.sendRequest({
      case: "pingReq",
      value: new Ao({
        timestamp: z.parse(Date.now()),
        rtt: z.parse(this.rtt)
      })
    })]);
  }
  ...
  sendLeave() {
    return this.sendRequest({
      case: "leave",
      value: new $n({
        reason: Ce.CLIENT_INITIATED,
        // server doesn't process this field, keeping it here to indicate the intent of a full disconnect
        action: Tt.DISCONNECT
      })
    });
  }
  sendRequest(e) {
    return m(this, arguments, void 0, function(t) {
      ...
      return function* () {
        if (!s && !Ju(t) && n.state === G.RECONNECTING) {
          n.queuedRequests.push(() => m(n, void 0, void 0, function* () {
            yield this.sendRequest(t, !0);
          }));
          return;
        }
        if (s || (yield n.requestQueue.flush()), n.signalLatency && (yield oe(n.signalLatency)), n.isDisconnected) {
  ...
  handleSignalConnected(e, t, n) {
    this.state = G.CONNECTED, clearTimeout(t), this.startPingInterval(), this.startReadingLoop(e.readable.getReader(), n);
  }
  /**
  ...
  validateFirstMessage(e, t) {
    ...
    return ((n = e.message) === null || n === void 0 ? void 0 : n.case) === "join" ? {
    ...
    } : this.state === G.RECONNECTING && ((s = e.message) === null || s === void 0 ? void 0 : s.case) !== "leave" ? ((r = e.message) === null || r === void 0 ? void 0 : r.case) === "reconnect" ? {
      isValid: !0,
      response: e.message.value
      ...
      isValid: !1,
      error: U.leaveRequest("Received leave request while trying to (re)connect", e.message.value.reason)
    } : t ? {
      isValid: !1,
      error: U.internal("Unexpected first message")
    } : {
      isValid: !1,
      error: U.internal("did not receive join response, got ".concat((o = e.message) === null || o === void 0 ? void 0 : o.case, " instead"))
    };
  }
  /**
  ...
  handleConnectionError(e, t) {
    return m(this, void 0, void 0, function* () {
      try {
        ...
        switch (n.status) {
          case 404:
            return U.serviceNotFound("v1 RTC path not found. Consider upgrading your LiveKit server version", "v0-rtc");
          case 401:
          case 403:
            const s = yield n.text();
            return U.notAllowed(s, n.status);
          default:
            break;
        }
        return e instanceof U ? e : U.internal("Encountered unknown websocket error during connection: ".concat(e), {
          status: n.status,
          statusText: n.statusText
        });
      } catch (n) {
        return n instanceof U ? n : U.serverUnreachable(n instanceof Error ? n.message : "server was not reachable");
      }
    });
  }
}
...
var Pe = {}, yi = {}, bi = { exports: {} }, br;
function Ts() {
  if (br) return bi.exports;
  br = 1;
  var i = bi.exports = {
    v: [{
      name: "version",
...
function $u() {
  return kr || (kr = 1, function(i) {
    ...
    }, s = Ts(), r = RegExp.prototype.test.bind(/^([a-z])=(.*)/);
    i.parse = function(o) {
      var c = {}, d = [], l = c;
    ...
    return c.forEach(function(u) {
      ...
      l.push(n("m", i.m[0], u)), d.forEach(function(h) {
        i[h].forEach(function(f) {
          f.name in u && u[f.name] != null ? l.push(n(h, f, u)) : f.push in u && u[f.push] != null && u[f.push].forEach(function(g) {
            l.push(n(h, f, g));
          });
        });
      });
...
function Zu() {
  ...
  Sr = 1;
  var i = $u(), e = Xu(), t = Ts();
  return Pe.grammar = t, Pe.write = e, Pe.parse = i.parse, Pe.parseParams = i.parseParams, Pe.parseFmtpConfig = i.parseFmtpConfig, Pe.parsePayloads = i.parsePayloads, Pe.parseRemoteCandidates = i.parseRemoteCandidates, Pe.parseImageAttributes = i.parseImageAttributes, Pe.parseSimulcastStreamList = i.parseSimulcastStreamList, Pe;
}
var Xe = Zu();
function Ss(i, e, t) {
  var n, s, r;
  ...
  return h.cancel = function(f) {
    r !== void 0 && clearTimeout(r), l.forEach(function(g) {
      return (0, g.reject)(f);
    }), l = [];
  }, h;
}
const eh = 0.7, th = 20, Et = {
  NegotiationStarted: "negotiationStarted",
  NegotiationComplete: "negotiationComplete",
  RTPVideoPayloadTypes: "rtpVideoPayloadTypes"
};
class Cr extends Fe.EventEmitter {
  get pc() {
  ...
  constructor(e) {
    let t = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
    var n;
    super(), this.log = F, this.ddExtID = 0, this.latestOfferId = 0, this.pendingCandidates = [], this.restartingIce = !1, this.renegotiate = !1, this.trackBitrates = [], this.remoteStereoMids = [], this.remoteNackMids = [], this.negotiate = Ss((s) => m(this, void 0, void 0, function* () {
    ...
    }, this.log = Le((n = t.loggerName) !== null && n !== void 0 ? n : _e.PCTransport), this.loggerOptions = t, this.config = e, this._pc = this.createPC(), this.offerLock = new ae();
  }
  createPC() {
  ...
  setRemoteDescription(e, t) {
    return m(this, void 0, void 0, function* () {
      ...
      if (e.type === "offer") {
        ...
        const r = Xe.parse((n = e.sdp) !== null && n !== void 0 ? n : "");
        r.media.forEach((a) => {
          const o = Cs(a.mid);
      ...
      }), this.pendingCandidates = [], this.restartingIce = !1, this.renegotiate ? (this.renegotiate = !1, yield this.createAndSendOffer()) : e.type === "answer" && (this.emit(Et.NegotiationComplete), e.sdp && Xe.parse(e.sdp).media.forEach((a) => {
        a.type === "video" && this.emit(Et.RTPVideoPayloadTypes, a.rtp);
      })), !0;
    });
  }
  createAndSendOffer(e) {
    return m(this, void 0, void 0, function* () {
      ...
      try {
        ...
        const a = Xe.parse((t = r.sdp) !== null && t !== void 0 ? t : "");
        if (a.media.forEach((o) => {
          _r(o), o.type === "audio" ? Er(o, ["all"], []) : o.type === "video" && this.trackBitrates.some((c) => {
            if (!o.msid || !c.cid || !o.msid.includes(c.cid))
              return !1;
            let d = 0;
            if (o.rtp.some((u) => u.codec.toUpperCase() === c.codec.toUpperCase() ? (d = u.payload, !0) : !1), d === 0 || (Ie(c.codec) && !pt() && this.ensureVideoDDExtensionForSVC(o, a), !Ie(c.codec)))
              return !0;
            const l = Math.round(c.maxbr * eh);
        ...
        yield this.setMungedSDP(r, Xe.write(a)), this.onOffer(r, this.latestOfferId);
      } finally {
        n();
      }
    });
  ...
  createAndSetAnswer() {
    return m(this, void 0, void 0, function* () {
      var e;
      const t = yield this.pc.createAnswer(), n = Xe.parse((e = t.sdp) !== null && e !== void 0 ? e : "");
      return n.media.forEach((s) => {
        _r(s), s.type === "audio" && Er(s, this.remoteStereoMids, this.remoteNackMids);
      }), yield this.setMungedSDP(t, Xe.write(n)), t;
    });
  }
  ...
  ensureVideoDDExtensionForSVC(e, t) {
    var n, s;
    if (!((n = e.ext) === null || n === void 0 ? void 0 : n.some((a) => a.uri === dr))) {
      if (this.ddExtID === 0) {
        let a = 0;
        t.media.forEach((o) => {
          var c;
...
function Er(i, e, t) {
  const n = Cs(i.mid);
  let s = 0;
  i.rtp.some((r) => r.codec === "opus" ? (s = r.payload, !0) : !1), s > 0 && (i.rtcpFb || (i.rtcpFb = []), t.includes(n) && !i.rtcpFb.some((r) => r.payload === s && r.type === "nack") && i.rtcpFb.push({
    payload: s,
    type: "nack"
...
function nh(i) {
  var e;
  const t = [], n = [], s = Xe.parse((e = i.sdp) !== null && e !== void 0 ? e : "");
  let r = 0;
  return s.media.forEach((a) => {
    var o;
    const c = Cs(a.mid);
    a.type === "audio" && (a.rtp.some((d) => d.codec === "opus" ? (r = d.payload, !0) : !1), !((o = a.rtcpFb) === null || o === void 0) && o.some((d) => d.payload === r && d.type === "nack") && n.push(c), a.fmtp.some((d) => d.payload === r ? (d.config.includes("sprop-stereo=1") && t.push(c), !0) : !1));
  }), {
    stereoMids: t,
    nackMids: n
  };
...
const zi = "vp8", ih = {
  audioPreset: xn.music,
  dtx: !0,
  red: !0,
  forceStereo: !1,
  simulcast: !0,
  screenShareEncoding: nn.h1080fps15.encoding,
  ...
  resolution: wt.h720.resolution
}, sh = {
  adaptiveStream: !1,
  dynacast: !1,
  stopLocalTrackOnUnpublish: !0,
  reconnectPolicy: new No(),
...
class rh {
  ...
  constructor(e, t, n) {
    var s;
    this.peerConnectionTimeout = Es.peerConnectionTimeout, this.log = F, this.updateState = () => {
      var r, a;
    ...
    }, this.log = Le((s = n.loggerName) !== null && s !== void 0 ? s : _e.PCManager), this.loggerOptions = n, this.isPublisherConnectionRequired = t !== "subscriber-primary", this.isSubscriberConnectionRequired = t === "subscriber-primary", this.publisher = new Cr(e, n), this._mode = t, t !== "publisher-only" && (this.subscriber = new Cr(e, n), this.subscriber.onConnectionStateChange = this.updateState, this.subscriber.onIceConnectionStateChange = this.updateState, this.subscriber.onSignalingStatechange = this.updateState, this.subscriber.onIceCandidate = (r) => {
      var a;
    ...
    }, this.state = X.NEW, this.connectionLock = new ae(), this.remoteOfferLock = new ae();
  }
  get logContext() {
    var e, t;
    return Object.assign({}, (t = (e = this.loggerOptions).loggerContextCb) === null || t === void 0 ? void 0 : t.call(e));
  ...
  negotiate(e) {
    return m(this, void 0, void 0, function* () {
      return new fe((t, n) => m(this, void 0, void 0, function* () {
        const s = setTimeout(() => {
        ...
        e.signal.addEventListener("abort", r), this.publisher.once(Et.NegotiationStarted, () => {
          ...
          clearTimeout(s), a instanceof Error ? n(a) : n(new Error(String(a)));
        });
      }));
    });
  }
        ...
        if (n.getConnectionState() !== "connected")
          return new Promise((c, d) => m(r, void 0, void 0, function* () {
            ...
            const u = re.setTimeout(() => {
              s == null || s.signal.removeEventListener("abort", l), d(U.internal("could not establish pc connection"));
            }, a);
            for (; this.state !== X.CONNECTED; )
              if (yield oe(50), s != null && s.signal.aborted) {
                d(U.cancelled("room connection has been cancelled"));
                return;
              }
...
class H {
  static fetchRegionSettings(e, t, n) {
    return m(this, void 0, void 0, function* () {
      ...
      try {
        const r = yield fetch("".concat(ah(e), "/regions"), {
          headers: {
        ...
        if (r.ok) {
          const a = Tu(r.headers), o = a ? a * 1e3 : Wa;
          return {
          ...
          throw r.status === 401 ? U.notAllowed("Could not fetch region settings: ".concat(r.statusText), r.status) : U.internal("Could not fetch region settings: ".concat(r.statusText));
      } catch (r) {
        throw r instanceof U ? r : n != null && n.aborted ? U.cancelled("Region fetching was aborted") : U.serverUnreachable("Could not fetch region settings, ".concat(r instanceof Error ? "".concat(r.name, ": ").concat(r.message) : r));
      } finally {
        s();
      }
    });
    ...
    return m(this, void 0, void 0, function* () {
      ...
      clearTimeout(s), H.settingsTimeouts.set(e.hostname, setTimeout(() => m(this, void 0, void 0, function* () {
        try {
          ...
          if (r instanceof U && r.reason === J.NotAllowed) {
            F.debug("token is not valid, cancelling auto region refresh");
            return;
          }
          F.debug("auto refetching of region settings failed", {
            error: r
          }), H.scheduleRefetch(e, t, n);
        }
      }), n));
    });
  }
  static updateCachedRegionSettings(e, t, n) {
    H.cache.set(e.hostname, n), H.scheduleRefetch(e, t, n.maxAgeInMs);
  }
  static stopRefetch(e) {
    const t = H.settingsTimeouts.get(e);
    t && (clearTimeout(t), H.settingsTimeouts.delete(e));
  }
  static scheduleCleanup(e) {
    let t = H.connectionTrackers.get(e);
    t && (t.cleanupTimeout && clearTimeout(t.cleanupTimeout), t.cleanupTimeout = setTimeout(() => {
      const n = H.connectionTrackers.get(e);
      n && n.connectionCount === 0 && (F.debug("stopping region refetch after disconnect delay", {
        hostname: e
      }), H.stopRefetch(e)), n && (n.cleanupTimeout = void 0);
    }, oh));
  }
  static cancelCleanup(e) {
    const t = H.connectionTrackers.get(e);
  ...
  notifyConnected() {
    ...
    t || (t = {
      connectionCount: 0
    }, H.connectionTrackers.set(e, t)), t.connectionCount++, H.cancelCleanup(e);
  }
  notifyDisconnected() {
    const e = this.serverUrl.hostname, t = H.connectionTrackers.get(e);
    t && (t.connectionCount = Math.max(0, t.connectionCount - 1), t.connectionCount === 0 && H.scheduleCleanup(e));
  }
  constructor(e, t) {
  ...
  getNextBestRegionUrl(e) {
    return m(this, void 0, void 0, function* () {
      ...
      (!t || Date.now() - t.updatedAtInMs > t.maxAgeInMs) && (t = yield this.fetchRegionSettings(e), H.updateCachedRegionSettings(this.serverUrl, this.token, t));
      const n = t.regionSettings.regions.filter((s) => !this.attemptedRegions.find((r) => r.url === s.url));
      if (n.length > 0) {
        const s = n[0];
        return this.attemptedRegions.push(s), F.debug("next region: ".concat(s.region)), s.url;
      } else
        return null;
    });
  }
  ...
  setServerReportedRegions(e) {
    H.updateCachedRegionSettings(this.serverUrl, this.token, e);
  }
}
H.cache = /* @__PURE__ */ new Map();
H.settingsTimeouts = /* @__PURE__ */ new Map();
H.connectionTrackers = /* @__PURE__ */ new Map();
H.fetchLock = new ae();
function ah(i) {
...
class Q extends Error {
   ...
   */
  constructor(e, t, n) {
    super(t), this.code = e, this.message = Pr(t, Q.MAX_MESSAGE_BYTES), this.data = n ? Pr(n, Q.MAX_DATA_BYTES) : void 0;
  }
  /**
   * @internal
   */
...
const Rs = typeof MediaRecorder < "u";
class ch {
  constructor() {
    throw new Error("MediaRecorder is not available in this environment");
  }
}
const dh = Rs ? MediaRecorder : ch;
class Ga extends dh {
  constructor(e, t) {
    if (!Rs)
      throw new Error("MediaRecorder is not available in this environment");
    ...
    const r = () => s === void 0, a = () => {
      ...
      s == null || s.error(c), this.removeEventListener("dataavailable", n), this.removeEventListener("stop", a), this.removeEventListener("error", o), s = void 0;
    };
...
class ws extends T {
  ...
  constructor(e, t, n) {
    ...
    super(e, t, r), this.manuallyStopped = !1, this._isUpstreamPaused = !1, this.handleTrackMuteEvent = () => this.debouncedTrackMuteHandler().catch(() => this.log.debug("track mute bounce got cancelled by an unmute event", this.logContext)), this.debouncedTrackMuteHandler = Ss(() => m(this, void 0, void 0, function* () {
  ...
  waitForDimensions() {
    return m(this, arguments, void 0, function() {
      var e = this;
      let t = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : uh;
      return function* () {
        var n;
        if (e.kind === T.Kind.Audio)
          throw new Error("cannot get dimensions for audio tracks");
        ((n = ue()) === null || n === void 0 ? void 0 : n.os) === "iOS" && (yield oe(10));
        const s = Date.now();
        for (; Date.now() - s < t; ) {
        ...
        throw new we("unable to get track dimensions after timeout");
      }();
    });
  }
  setDeviceId(e) {
  ...
  replaceTrack(e, t) {
    return m(this, void 0, void 0, function* () {
      ...
      try {
        if (!this.sender)
          throw new we("unable to replace an unpublished track");
        let s, r;
  ...
  restart(e) {
    return m(this, void 0, void 0, function* () {
      ...
      try {
        ...
        this.kind === T.Kind.Video ? a.video = n || s ? {
          deviceId: n,
          facingMode: s
    ...
    return m(this, void 0, void 0, function* () {
      ...
      try {
        ...
        if ((n == null ? void 0 : n.name) === "Safari" && Me(n.version, "12.0") < 0)
          throw new tn("pauseUpstream is not supported on Safari < 12.");
    ...
    return m(this, arguments, void 0, function(t) {
      ...
      return function* () {
        ...
        try {
          ...
          const o = document.createElement(n.kind), c = {
            kind: n.kind,
  ...
  startPreConnectBuffer() {
    ...
    if (this.localTrackRecorder) {
      ...
      MediaRecorder.isTypeSupported(t) || (t = "video/mp4"), this.localTrackRecorder = new Ga(this, {
        mimeType: t
      });
    }
...
class Ot extends ws {
  ...
  constructor(e, t) {
    ...
    super(e, T.Kind.Audio, t, n, r), this.stopOnMute = !1, this.isKrispNoiseFilterEnabled = !1, this.monitorSender = () => m(this, void 0, void 0, function* () {
      ...
      a && this.prevStats && (this._currentBitrate = Xn(a, this.prevStats)), this.prevStats = a;
    }), this.handleKrispNoiseFilterEnable = () => {
      this.isKrispNoiseFilterEnabled = !0, this.log.debug("Krisp noise filter enabled", this.logContext), this.emit(R.AudioTrackFeatureUpdate, this, ie.TF_ENHANCED_NOISE_CANCELLATION, !0);
    }, this.handleKrispNoiseFilterDisable = () => {
      this.isKrispNoiseFilterEnabled = !1, this.log.debug("Krisp noise filter disabled", this.logContext), this.emit(R.AudioTrackFeatureUpdate, this, ie.TF_ENHANCED_NOISE_CANCELLATION, !1);
...
function mh(i, e, t) {
  switch (i.kind) {
    case "audio":
      return new Ot(i, e, !1, void 0, t);
    case "video":
      return new At(i, e, !1, t);
    default:
      throw new we("unsupported track type: ".concat(i.kind));
  }
}
const fh = Object.values(wt), ph = Object.values(Nn), gh = Object.values(nn), vh = [wt.h180, wt.h360], yh = [Nn.h180, Nn.h360], bh = (i) => [{
...
function Qi(i, e, t, n) {
  ...
  if (!a && !o && !c || !e || !t)
    return [{}];
  a || (a = Th(i, e, t, d), F.debug("using video encoding", a));
  ...
  if (c && Ie(d)) {
    ...
    const E = ue();
    if ($t() || // Even tho RN runs M114, it does not produce SVC layers when a single encoding
    // is provided. So we'll use the legacy SVC specification for now.
    // TODO: when we upstream libwebrtc, this will need additional verification
    Ue() || (E == null ? void 0 : E.name) === "Chrome" && Me(E == null ? void 0 : E.version, "113") < 0) {
      const k = g.suffix == "h" ? 2 : 3, _ = au(E);
    ...
    return u.encoding.priority && (v[0].priority = u.encoding.priority, v[0].networkPriority = u.encoding.priority), F.debug("using svc encoding", {
      encodings: v
    }), v;
  }
  if (!o)
    return [a];
  let h = [];
  i ? h = (s = wr(n == null ? void 0 : n.screenShareSimulcastLayers)) !== null && s !== void 0 ? s : Rr(i, u) : h = (r = wr(n == null ? void 0 : n.videoSimulcastLayers)) !== null && r !== void 0 ? r : Rr(i, u);
  let f;
  if (h.length > 0) {
    const g = h[0];
    h.length > 1 && ([, f] = h);
...
function kh(i, e, t) {
  ...
  if (!t.backupCodec || t.backupCodec === !0 || t.backupCodec.codec === t.videoCodec)
    return;
  e !== t.backupCodec.codec && F.warn("requested a different codec than specified as backup", {
    serverRequested: e,
...
function Sh(i, e, t) {
  if (i)
    return gh;
  const n = e > t ? e / t : t / e;
  return Math.abs(n - 16 / 9) < Math.abs(n - 4 / 3) ? fh : ph;
}
function Rr(i, e) {
  if (i)
    return bh(e);
  ...
  return Math.abs(s - 16 / 9) < Math.abs(s - 4 / 3) ? vh : yh;
}
function Ti(i, e, t, n) {
  const s = [];
  if (t.forEach((r, a) => {
    ...
    s.forEach((o) => {
      var c;
      o.maxFramerate != r && (a && (a = !1, F.info("Simulcast on iOS React-Native requires all encodings to share the same framerate.")), F.info('Setting framerate of encoding "'.concat((c = o.rid) !== null && c !== void 0 ? c : "", '" to ').concat(r)), o.maxFramerate = r);
    });
  }
  return s;
}
...
class At extends ws {
  ...
  setPublishingQuality(e) {
    const t = [];
    for (let n = pe.LOW; n <= pe.HIGH; n += 1)
      t.push(new hs({
        quality: n,
        enabled: n <= e
      }));
    this.log.debug("setting publishing quality. max quality ".concat(e), this.logContext), this.setPublishingLayers(Ie(this.codec), t);
  }
  restartTrack(e) {
    return m(this, void 0, void 0, function* () {
      var t, n, s, r, a;
      ...
      try {
        for (var c = !0, d = qe(this.simulcastCodecs.values()), l; l = yield d.next(), t = l.done, !t; c = !0) {
          r = l.value, c = !1;
          const u = r;
    ...
    return m(this, arguments, void 0, function(n) {
      ...
      return function* () {
        ...
        if (yield t.setProcessor.call(s, n, r), !((l = s.processor) === null || l === void 0) && l.processedTrack)
          try {
            for (var h = !0, f = qe(s.simulcastCodecs.values()), g; g = yield f.next(), a = g.done, !a; h = !0)
              d = g.value, h = !1, yield (u = d.sender) === null || u === void 0 ? void 0 : u.replaceTrack(s.processor.processedTrack);
          } catch (v) {
            o = {
              error: v
            };
  ...
  setPublishingCodecs(e) {
    return m(this, void 0, void 0, function* () {
      ...
      if (this.log.debug("setting publishing codecs", Object.assign(Object.assign({}, this.logContext), {
        ...
        return yield this.setPublishingLayers(Ie(e[0].codec), e[0].qualities), [];
      this.subscribedCodecs = e;
      const d = [];
      try {
        for (t = !0, n = qe(e); s = yield n.next(), r = s.done, !r; t = !0) {
          c = s.value, t = !1;
          const l = c;
          if (!this.codec || this.codec === l.codec)
            yield this.setPublishingLayers(Ie(l.codec), l.qualities);
          else {
            ...
            } else u.encodings && (this.log.debug("try setPublishingLayersForSender ".concat(l.codec), this.logContext), yield Ir(u.sender, u.encodings, l.qualities, this.senderLock, Ie(l.codec), this.log, this.logContext));
          }
        }
      } catch (l) {
        a = {
  ...
  setPublishingLayers(e, t) {
    return m(this, void 0, void 0, function* () {
      ...
      this.log.debug("setting publishing layers", Object.assign(Object.assign({}, this.logContext), {
        qualities: t
      })), !(!this.sender || !this.encodings) && (yield Ir(this.sender, this.encodings, t, this.senderLock, e, this.log, this.logContext));
    });
  }
  /**
      ...
      try {
        ...
        t.encodings = t.encodings.map((n, s) => {
          ...
          return Object.assign(Object.assign({}, n), {
            ...
            scalabilityMode: s === 0 && Ie(this.codec) ? "L1T3" : void 0,
  ...
  handleAppVisibilityChanged() {
    ...
    return m(this, void 0, void 0, function* () {
      yield e.handleAppVisibilityChanged.call(this), Ia() && this.isInBackground && this.source === T.Source.Camera && (this._mediaStreamTrack.enabled = !1);
    });
  }
}
function Ir(i, e, t, n, s, r, a) {
  return m(this, void 0, void 0, function* () {
    ...
    try {
      ...
      !1 && d[0].scalabilityMode || (s && t.some((f) => f.enabled) && t.forEach((f) => f.enabled = !0), d.forEach((h, f) => {
        ...
        v === "" && (v = "q");
        const E = Ya(v), k = t.find((_) => _.quality === E);
...
function Or(i, e, t, n) {
  ...
  if (n) {
    const s = t[0].scalabilityMode, r = new za(s), a = [], o = r.suffix == "h" ? 1.5 : 2, c = r.suffix == "h" ? 2 : 3;
    for (let d = 0; d < r.spatial; d += 1)
      a.push(new tt({
        quality: Math.min(pe.HIGH, r.spatial - 1) - d,
...
const Ar = "_lossy", Mr = "_reliable", _h = 2 * 1e3, Si = "leave-reconnect", Ph = 3e4, Rh = 8 * 1024, wh = 256 * 1024;
var ke;
(function(i) {
...
class Ih extends Fe.EventEmitter {
  get isClosed() {
  ...
  constructor(e) {
    var t;
    super(), this.options = e, this.rtcConfig = {}, this.peerConnectionTimeout = Es.peerConnectionTimeout, this.fullReconnectOnNext = !1, this.latestRemoteOfferId = 0, this.subscriberPrimary = !1, this.pcState = ke.New, this._isClosed = !0, this.pendingTrackResolvers = {}, this.reconnectAttempts = 0, this.reconnectStart = 0, this.attemptingReconnect = !1, this.joinAttempts = 0, this.maxJoinAttempts = 1, this.shouldFailNext = !1, this.log = F, this.reliableDataSequence = 1, this.reliableMessageBuffer = new yr(), this.reliableReceivedState = new Qu(Ph), this.lossyDataStatCurrentBytes = 0, this.lossyDataStatByterate = 0, this.lossyDataDropCount = 0, this.midToTrackId = {}, this.isWaitingForNetworkReconnect = !1, this.handleDataChannel = (n) => m(this, [n], void 0, function(s) {
      var r = this;
      let {
        channel: a
      } = s;
      ...
      try {
        ...
        if (((s = u.value) === null || s === void 0 ? void 0 : s.case) === "speaker")
          this.emit(w.ActiveSpeakersUpdate, u.value.value.speakers);
          ...
          const h = yield (a = this.e2eeManager) === null || a === void 0 ? void 0 : a.handleEncryptedData(u.value.value.encryptedValue, u.value.value.iv, u.participantIdentity, u.value.value.keyIndex), f = oo.fromBinary(h.payload), g = new le({
            value: f.value,
            participantIdentity: u.participantIdentity,
            participantSid: u.participantSid
          });
          ((o = g.value) === null || o === void 0 ? void 0 : o.case) === "user" && Dr(g, g.value.value), this.emit(w.DataPacketReceived, g, u.value.value.encryptionType);
        } else
          ((c = u.value) === null || c === void 0 ? void 0 : c.case) === "user" && Dr(u, u.value.value), this.emit(w.DataPacketReceived, u, ne.NONE);
      } finally {
        d();
      }
      ...
      const r = n.currentTarget.maxRetransmits === 0 ? V.LOSSY : V.RELIABLE;
      ...
      n === Si && (o = 0), this.log.debug("reconnecting in ".concat(o, "ms"), this.logContext), this.clearReconnectTimeout(), this.token && this.regionUrlProvider && this.regionUrlProvider.updateToken(this.token), this.reconnectTimeout = re.setTimeout(() => this.attemptReconnect(s).finally(() => this.reconnectTimeout = void 0), o);
    }, this.waitForRestarted = () => new Promise((n, s) => {
      this.pcState === ke.Connected && n();
      const r = () => {
        this.off(w.Disconnected, a), n();
      }, a = () => {
      ...
      typeof s < "u" && s !== this.dcBufferStatus.get(n) && (this.dcBufferStatus.set(n, s), this.emit(w.DCBufferStatusChanged, s, n));
      ...
      !this.url || !(yield fetch(Xt(this.url), {
        method: "HEAD"
      ...
      (this.client.currentState === G.RECONNECTING || // also if the browser went offline before and the engine still thinks it's in a connected state, treat it as a network interruption that we haven't noticed yet
      this.isWaitingForNetworkReconnect && this.client.currentState === G.CONNECTED) && (this.clearReconnectTimeout(), this.attemptReconnect(at.RR_SIGNAL_DISCONNECTED), this.isWaitingForNetworkReconnect = !1));
      ...
      if (this.url)
        try {
          yield Promise.race([
            ...
            oe(4e3).then(() => Promise.reject())
          ]);
        } catch {
          window.navigator.onLine === !1 && (this.log.info("detected network interruption"), this.isWaitingForNetworkReconnect = !0);
        }
    }), this.log = Le((t = e.loggerName) !== null && t !== void 0 ? t : _e.Engine), this.loggerOptions = {
      loggerName: e.loggerName,
      loggerContextCb: () => this.logContext
    }, this.client = new ks(void 0, this.loggerOptions), this.client.signalLatency = this.options.expSignalLatency, this.reconnectPolicy = this.options.reconnectPolicy, this.closingLock = new ae(), this.dataProcessLock = new ae(), this.dcBufferStatus = /* @__PURE__ */ new Map([[V.LOSSY, !0], [V.RELIABLE, !0]]), this.client.onParticipantUpdate = (n) => this.emit(w.ParticipantUpdate, n), this.client.onConnectionQuality = (n) => this.emit(w.ConnectionQualityUpdate, n), this.client.onRoomUpdate = (n) => this.emit(w.RoomUpdate, n), this.client.onSubscriptionError = (n) => this.emit(w.SubscriptionError, n), this.client.onSubscriptionPermissionUpdate = (n) => this.emit(w.SubscriptionPermissionUpdate, n), this.client.onSpeakersChanged = (n) => this.emit(w.SpeakersChanged, n), this.client.onStreamStateUpdate = (n) => this.emit(w.StreamStateChanged, n), this.client.onRequestResponse = (n) => this.emit(w.SignalRequestResponse, n);
  }
  /** @internal */
  get logContext() {
    var e, t, n, s, r, a;
  ...
  join(e, t, n, s) {
    return m(this, arguments, void 0, function(r, a, o, c) {
      ...
      let l = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : !1;
      return function* () {
        d.url = r, d.token = a, d.signalOpts = o, d.maxJoinAttempts = o.maxRetries;
        try {
          d.joinAttempts += 1, d.setupSignalClientCallbacks();
          const u = yield d.client.join(r, a, o, c, l);
          return d._isClosed = !1, d.latestJoinResponse = u, d.subscriberPrimary = u.subscriberPrimary, d.pcManager || (yield d.configure(u, !l)), (!d.subscriberPrimary || u.fastPublish) && d.negotiate().catch((h) => {
            F.error(h, d.logContext);
          }), d.registerOnLineListener(), d.clientConfiguration = u.clientConfiguration, d.emit(w.SignalConnected, u), u;
        } catch (u) {
          if (u instanceof U) {
            if (u.reason === J.ServerUnreachable) {
              if (d.log.warn("Couldn't connect to server, attempt ".concat(d.joinAttempts, " of ").concat(d.maxJoinAttempts), d.logContext), d.joinAttempts < d.maxJoinAttempts)
                return d.join(r, a, o, c, l);
            } else if (u.reason === J.ServiceNotFound)
              return d.log.warn("Initial connection failed: ".concat(u.message, " – Retrying")), d.join(r, a, o, c, !0);
          }
          throw u;
        }
      }();
  ...
  close() {
    return m(this, void 0, void 0, function* () {
      ...
      try {
        this._isClosed = !0, this.joinAttempts = 0, this.emit(w.Closing), this.removeAllListeners(), this.deregisterOnLineListener(), this.clearPendingReconnect(), this.cleanupLossyDataStats(), yield this.cleanupPeerConnections(), yield this.cleanupClient();
      } finally {
        e();
      }
    });
  ...
  cleanupPeerConnections() {
    return m(this, void 0, void 0, function* () {
      ...
      t(this.lossyDC), t(this.lossyDCSub), t(this.reliableDC), t(this.reliableDCSub), this.lossyDC = void 0, this.lossyDCSub = void 0, this.reliableDC = void 0, this.reliableDCSub = void 0, this.reliableMessageBuffer = new yr(), this.reliableDataSequence = 1, this.reliableReceivedState.clear();
    });
  }
  ...
  addTrack(e) {
    if (this.pendingTrackResolvers[e.cid])
      throw new we("a track with the same ID has already been published");
  ...
  configure(e, t) {
    return m(this, void 0, void 0, function* () {
      ...
      this.pcManager = new rh(r, t ? "publisher-only" : e.subscriberPrimary ? "subscriber-primary" : "publisher-primary", this.loggerOptions), this.emit(w.TransportsCreated, this.pcManager.publisher, this.pcManager.subscriber), this.pcManager.onIceCandidate = (a, o) => {
        ...
        } else a === X.FAILED && (this.pcState === ke.Connected || this.pcState === ke.Reconnecting) && (this.pcState = ke.Disconnected, this.handleDisconnect("peerconnection failed", c === "failed" ? at.RR_SUBSCRIBER_FAILED : at.RR_PUBLISHER_FAILED));
        const d = this.client.isDisconnected || this.client.currentState === G.RECONNECTING, l = [X.FAILED, X.CLOSING, X.CLOSED].includes(a);
        d && l && !this._isClosed && this.emit(w.Offline);
      }), this.pcManager.onTrack = (a) => {
        a.streams.length !== 0 && this.emit(w.MediaTrackAdded, a.track, a.streams[0], a.receiver);
      }, Oh((s = e.serverInfo) === null || s === void 0 ? void 0 : s.protocol) || this.createDataChannels();
    });
  }
    ...
    this.client.onAnswer = (e, t, n) => m(this, void 0, void 0, function* () {
      ...
      switch (this.log.debug("client leave request", Object.assign(Object.assign({}, this.logContext), {
        ...
        case Tt.DISCONNECT:
          ...
          break;
        case Tt.RECONNECT:
          this.fullReconnectOnNext = !0, this.handleDisconnect(Si);
          break;
        case Tt.RESUME:
          this.handleDisconnect(Si);
      }
    };
  }
  makeRTCConfiguration(e) {
    ...
    return e.clientConfiguration && e.clientConfiguration.forceRelay === zt.ENABLED && (n.iceTransportPolicy = "relay"), n.sdpSemantics = "unified-plan", n.continualGatheringPolicy = "gather_continually", n;
  }
  createDataChannels() {
    this.pcManager && (this.lossyDC && (this.lossyDC.onmessage = null, this.lossyDC.onerror = null), this.reliableDC && (this.reliableDC.onmessage = null, this.reliableDC.onerror = null), this.lossyDC = this.pcManager.createPublisherDataChannel(Ar, {
      ordered: !1,
      maxRetransmits: 0
    }), this.reliableDC = this.pcManager.createPublisherDataChannel(Mr, {
      ordered: !0
      ...
      const e = this.dataChannelForKind(V.LOSSY);
      if (e) {
        const t = this.lossyDataStatByterate / 10;
        e.bufferedAmountLowThreshold = Math.min(Math.max(t, Rh), wh);
      }
    }, 1e3));
  }
  createSender(e, t, n) {
  ...
  createTransceiverRTCRtpSender(e, t, n) {
    return m(this, void 0, void 0, function* () {
      ...
      const s = [];
      e.mediaStream && s.push(e.mediaStream), Ge(e) && (e.codec = t.videoCodec);
      const r = {
        direction: "sendonly",
        streams: s
      };
  ...
  attemptReconnect(e) {
    return m(this, void 0, void 0, function* () {
      ...
      if (!this._isClosed) {
        ...
        (((t = this.clientConfiguration) === null || t === void 0 ? void 0 : t.resumeConnection) === zt.DISABLED || // signaling state could change to closed due to hardware sleep
        // those connections cannot be resumed
        ((s = (n = this.pcManager) === null || n === void 0 ? void 0 : n.currentState) !== null && s !== void 0 ? s : X.NEW) === X.NEW) && (this.fullReconnectOnNext = !0);
        try {
          ...
          r instanceof $ ? (this.log.debug("received unrecoverable error", Object.assign(Object.assign({}, this.logContext), {
            error: r
          })), a = !1) : r instanceof ct || (this.fullReconnectOnNext = !0), a ? this.handleDisconnect("reconnect", at.RR_UNKNOWN) : (this.log.info("could not recover connection after ".concat(this.reconnectAttempts, " attempts, ").concat(Date.now() - this.reconnectStart, "ms. giving up"), this.logContext), this.emit(w.Disconnected), yield this.close());
        } finally {
    ...
    return m(this, void 0, void 0, function* () {
      ...
      try {
        ...
        try {
          ...
          throw a instanceof U && a.reason === J.NotAllowed ? new $("could not reconnect, token might be expired") : new ct();
        }
        if (this.shouldFailNext)
          throw this.shouldFailNext = !1, new Error("simulated failure");
        if (this.client.setReconnected(), this.emit(w.SignalRestarted, r), yield this.waitForPCReconnected(), this.client.currentState !== G.CONNECTED)
    ...
    return m(this, void 0, void 0, function* () {
      ...
      try {
        ...
        throw s instanceof Error && (r = s.message, this.log.error(s.message, Object.assign(Object.assign({}, this.logContext), {
          error: s
        }))), s instanceof U && s.reason === J.NotAllowed ? new $("could not reconnect, token might be expired") : s instanceof U && s.reason === J.LeaveRequest ? s : new ct(r);
      }
      if (this.emit(w.SignalResumed), n) {
      ...
      if (this.shouldFailNext)
        throw this.shouldFailNext = !1, new Error("simulated failure");
      if (yield this.pcManager.triggerIceRestart(), yield this.waitForPCReconnected(), this.client.currentState !== G.CONNECTED)
        throw new ct("Signal connection got severed during reconnect");
      this.client.setReconnected(), ((t = this.reliableDC) === null || t === void 0 ? void 0 : t.readyState) === "open" && this.reliableDC.id === null && this.createDataChannels(), n != null && n.lastMessageSeq && this.resendReliableMessagesForResume(n.lastMessageSeq), this.emit(w.Resumed);
    });
  }
  ...
  waitForPCReconnected() {
    return m(this, void 0, void 0, function* () {
      ...
      try {
        if (yield oe(_h), !this.pcManager)
          throw new $("PC manager is closed");
        yield this.pcManager.ensurePCTransportConnection(void 0, this.peerConnectionTimeout), this.pcState = ke.Connected;
      } catch (e) {
        throw this.pcState = ke.Disconnected, U.internal("could not establish PC connection, ".concat(e.message));
      }
    });
  }
  /** @internal */
  publishRpcResponse(e, t, n, s) {
    return m(this, void 0, void 0, function* () {
      ...
      yield this.sendDataPacket(r, V.RELIABLE);
    });
  }
  /** @internal */
  publishRpcAck(e, t) {
    return m(this, void 0, void 0, function* () {
      ...
      yield this.sendDataPacket(n, V.RELIABLE);
    });
  }
  /* @internal */
  sendDataPacket(e, t) {
    return m(this, void 0, void 0, function* () {
      ...
      t === V.RELIABLE && (e.sequence = this.reliableDataSequence, this.reliableDataSequence += 1);
      ...
      if (s) {
        if (t === V.RELIABLE)
          ...
          this.lossyDataStatCurrentBytes += n.byteLength;
        }
        if (this.attemptingReconnect)
          return;
        s.send(n);
  ...
  resendReliableMessagesForResume(e) {
    return m(this, void 0, void 0, function* () {
      ...
      t && (this.reliableMessageBuffer.popToSequence(e), this.reliableMessageBuffer.getAll().forEach((n) => {
        t.send(n.data);
      })), this.updateAndEmitDCBufferStatus(V.RELIABLE);
    });
  }
  ...
  ensureDataTransportConnected(e) {
    return m(this, arguments, void 0, function(t) {
      ...
      return function* () {
        ...
        if (!a)
          throw U.internal("".concat(o, " connection not set"));
        let c = !1;
        !s && !n.dataChannelForKind(t, s) && (n.createDataChannels(), c = !0), !c && !s && !n.pcManager.publisher.isICEConnected && n.pcManager.publisher.getICEConnectionState() !== "checking" && (c = !0), c && n.negotiate().catch((u) => {
          F.error(u, n.logContext);
        });
        const d = n.dataChannelForKind(t, s);
        if ((d == null ? void 0 : d.readyState) === "open")
          return;
        ...
        throw U.internal("could not establish ".concat(o, " connection, state: ").concat(a.getICEConnectionState()));
      }();
    });
  }
  ...
  negotiate() {
    return m(this, void 0, void 0, function* () {
      return new fe((e, t) => m(this, void 0, void 0, function* () {
        if (!this.pcManager) {
        ...
        this.isClosed && t(new ut("cannot negotiate on closed engine")), this.on(w.Closing, s), this.pcManager.publisher.once(Et.RTPVideoPayloadTypes, (r) => {
          const a = /* @__PURE__ */ new Map();
          r.forEach((o) => {
            const c = o.codec.toLowerCase();
            Ma(c) && a.set(o.payload, c);
  ...
  sendSyncState(e, t) {
    ...
    e.forEach((g) => {
      g.isDesired !== u && h.push(g.trackSid), g.isEnabled || f.push(g.trackSid);
    }), this.client.sendSyncState(new ms({
      ...
      offer: this.pcManager.mode === "publisher-only" ? o ? bt({
        sdp: o.sdp,
        type: o.type
      }) : void 0 : d ? bt({
        sdp: d.sdp,
      ...
      trackSidsDisabled: f,
      datachannelReceiveStates: this.reliableReceivedState.map((g, v) => new Io({
        publisherSid: v,
        lastSeq: g
      }))
    }));
  ...
  dataChannelsInfo() {
    const e = [], t = (n, s) => {
      (n == null ? void 0 : n.id) !== void 0 && n.id !== null && e.push(new Oo({
        label: n.label,
        id: n.id,
        target: s
      }));
...
class Qa {
  ...
  constructor(e, t, n, s) {
    this.reader = t, this.totalByteSize = n, this._info = e, this.bytesReceived = 0, this.outOfBandFailureRejectingFuture = s;
  }
}
class Ah extends Qa {
  ...
  [Symbol.asyncIterator]() {
    ...
    let t = new Ee(), n = null, s = null;
    if (this.signal) {
      const a = this.signal;
      s = () => {
        var o;
        (o = t.reject) === null || o === void 0 || o.call(t, a.reason);
      }, a.addEventListener("abort", s), n = a;
    }
    const r = () => {
  ...
  readAll() {
    return m(this, arguments, void 0, function() {
      ...
      return function* () {
        ...
        try {
          for (var d = !0, l = qe(c), u; u = yield l.next(), n = u.done, !n; d = !0) {
            a = u.value, d = !1;
            const h = a;
            o.add(h);
          }
...
class Mh extends Qa {
   ...
   */
  constructor(e, t, n, s) {
    super(e, t, n, s), this.receivedChunks = /* @__PURE__ */ new Map();
  }
  handleChunkReceived(e) {
    var t;
    const n = Tn(e.chunkIndex), s = this.receivedChunks.get(n);
  ...
  [Symbol.asyncIterator]() {
    ...
    let n = new Ee(), s = null, r = null;
    if (this.signal) {
      const o = this.signal;
      r = () => {
        var c;
        (c = n.reject) === null || c === void 0 || c.call(n, o.reason);
...
class Dh {
  constructor() {
    this.log = F, this.byteStreamControllers = /* @__PURE__ */ new Map(), this.textStreamControllers = /* @__PURE__ */ new Map(), this.byteStreamHandlers = /* @__PURE__ */ new Map(), this.textStreamHandlers = /* @__PURE__ */ new Map();
  }
  ...
  handleStreamHeader(e, t, n) {
    return m(this, void 0, void 0, function* () {
      ...
      if (e.contentHeader.case === "byteHeader") {
        ...
        r(new Ah(c, d, Tn(e.totalLength), o), {
          identity: t
        });
        ...
        r(new Mh(c, d, Tn(e.totalLength), o), {
          identity: t
        });
      }
    });
...
class Lh {
  ...
  streamText(e) {
    return m(this, void 0, void 0, function* () {
      ...
      const r = (t = e == null ? void 0 : e.streamId) !== null && t !== void 0 ? t : crypto.randomUUID(), a = {
        ...
        encryptionType: !((s = this.engine.e2eeManager) === null || s === void 0) && s.isDataChannelEncryptionEnabled ? ne.GCM : ne.NONE
      }, o = new On({
        streamId: r,
        mimeType: a.mimeType,
        topic: a.topic,
      ...
      }), c = e == null ? void 0 : e.destinationIdentities, d = new le({
      ...
      yield this.engine.sendDataPacket(d, V.RELIABLE);
      let l = 0;
      const u = this.engine, h = new WritableStream({
        // Implement the sink
        write(v) {
          return m(this, void 0, void 0, function* () {
            for (const E of ku(v, xr)) {
              const k = new An({
                content: E,
                streamId: r,
                chunkIndex: dt(l)
        ...
        close() {
          return m(this, void 0, void 0, function* () {
            ...
            yield u.sendDataPacket(E, V.RELIABLE);
          });
        },
        abort(v) {
          console.log("Sink error:", v);
        }
      });
      ...
      u.once(w.Closing, f);
      const g = new xh(h, a, () => this.engine.off(w.Closing, f));
      return g;
    });
  }
  sendFile(e, t) {
  ...
  streamBytes(e) {
    return m(this, void 0, void 0, function* () {
      ...
      let f = 0;
      const g = new ae(), v = this.engine, E = this.log, k = new WritableStream({
        write(M) {
          return m(this, void 0, void 0, function* () {
            ...
            try {
              for (; b < M.byteLength; ) {
                const S = M.slice(b, b + xr), O = new le({
                  destinationIdentities: d,
                  value: {
                    case: "streamChunk",
                    value: new An({
                ...
                yield v.sendDataPacket(O, V.RELIABLE), f += 1, b += S.byteLength;
              }
            } finally {
              y();
            }
        ...
        close() {
          return m(this, void 0, void 0, function* () {
            ...
            yield v.sendDataPacket(y, V.RELIABLE);
          });
        },
        abort(M) {
...
class Is extends T {
  ...
  /* @internal */
  startMonitor() {
    this.monitorInterval || (this.monitorInterval = setInterval(() => this.monitorReceiver(), Ps)), $l() && this.registerTimeSyncUpdate();
  }
...
class Xa extends Is {
  constructor(e, t, n, s, r, a) {
    super(e, t, T.Kind.Audio, n, a), this.monitorReceiver = () => m(this, void 0, void 0, function* () {
      ...
      o && this.prevStats && this.receiver && (this._currentBitrate = Xn(o, this.prevStats)), this.prevStats = o;
  ...
  attach(e) {
    ...
    return e ? super.attach(e) : e = super.attach(), this.sinkId && Un(e) && e.setSinkId(this.sinkId).catch((n) => {
...
class Za extends Is {
  constructor(e, t, n, s, r) {
    super(e, t, T.Kind.Video, n, r), this.elementInfos = [], this.monitorReceiver = () => m(this, void 0, void 0, function* () {
      ...
      a && this.prevStats && this.receiver && (this._currentBitrate = Xn(a, this.prevStats)), this.prevStats = a;
    }), this.debouncedHandleResize = Ss(() => {
...
class Uh {
  ...
  constructor(e, t) {
    this.onVisibilityChanged = (n) => {
      ...
      r === this.element && (this.isIntersecting = a, this.isPiP = Ft(this.element), this.visibilityChangedAt = Date.now(), (s = this.handleVisibilityChanged) === null || s === void 0 || s.call(this));
    }, this.onEnterPiP = () => {
      var n, s, r;
      (s = (n = window.documentPictureInPicture) === null || n === void 0 ? void 0 : n.window) === null || s === void 0 || s.addEventListener("pagehide", this.onLeavePiP), this.isPiP = Ft(this.element), (r = this.handleVisibilityChanged) === null || r === void 0 || r.call(this);
    }, this.onLeavePiP = () => {
      var n;
      this.isPiP = Ft(this.element), (n = this.handleVisibilityChanged) === null || n === void 0 || n.call(this);
    }, this.element = e, this.isIntersecting = t ?? $i(e), this.isPiP = ge() && Ft(e), this.visibilityChangedAt = 0;
  }
  width() {
  ...
  observe() {
    var e, t, n;
    this.isIntersecting = $i(this.element), this.isPiP = Ft(this.element), this.element.handleResize = () => {
      var s;
      (s = this.handleResize) === null || s === void 0 || s.call(this);
    }, this.element.handleVisibilityChanged = this.onVisibilityChanged, mr().observe(this.element), hr().observe(this.element), this.element.addEventListener("enterpictureinpicture", this.onEnterPiP), this.element.addEventListener("leavepictureinpicture", this.onLeavePiP), (e = window.documentPictureInPicture) === null || e === void 0 || e.addEventListener("enter", this.onEnterPiP), (n = (t = window.documentPictureInPicture) === null || t === void 0 ? void 0 : t.window) === null || n === void 0 || n.addEventListener("pagehide", this.onLeavePiP);
  }
  stopObserving() {
    var e, t, n, s, r;
    (e = mr()) === null || e === void 0 || e.unobserve(this.element), (t = hr()) === null || t === void 0 || t.unobserve(this.element), this.element.removeEventListener("enterpictureinpicture", this.onEnterPiP), this.element.removeEventListener("leavepictureinpicture", this.onLeavePiP), (n = window.documentPictureInPicture) === null || n === void 0 || n.removeEventListener("enter", this.onEnterPiP), (r = (s = window.documentPictureInPicture) === null || s === void 0 ? void 0 : s.window) === null || r === void 0 || r.removeEventListener("pagehide", this.onLeavePiP);
  }
}
function Ft(i) {
  var e, t;
  return document.pictureInPictureElement === i ? !0 : !((e = window.documentPictureInPicture) === null || e === void 0) && e.window ? $i(i, (t = window.documentPictureInPicture) === null || t === void 0 ? void 0 : t.window) : !1;
}
function $i(i, e) {
  const t = e || window;
...
class Ne extends Fe.EventEmitter {
  constructor(e, t, n, s) {
    var r;
    super(), this.metadataMuted = !1, this.encryption = ne.NONE, this.log = F, this.handleMuted = () => {
      ...
      this.emit(R.Unmuted);
    }, this.log = Le((r = s == null ? void 0 : s.loggerName) !== null && r !== void 0 ? r : _e.Publication), this.loggerContextCb = this.loggerContextCb, this.setMaxListeners(100), this.kind = e, this.trackSid = t, this.trackName = n, this.source = T.Source.Unknown;
  }
  /** @internal */
  setTrack(e) {
  ...
  get isEncrypted() {
    return this.encryption !== ne.NONE;
  }
  /**
...
class Bn extends Ne {
  ...
  constructor(e, t, n, s) {
    super(e, t.sid, t.name, s), this.track = void 0, this.handleTrackEnded = () => {
      ...
      this.track && Ge(this.track) && this.emit(R.CpuConstrained, this.track);
...
function sn(i, e) {
  return m(this, void 0, void 0, function* () {
    ...
    const c = Ta(r, Ha, Ka), d = vs(c), l = navigator.mediaDevices.getUserMedia(d);
    ...
    try {
      ...
      return yield Promise.all(u.getTracks().map((h) => m(this, void 0, void 0, function* () {
        ...
        typeof E != "boolean" && (v = E);
        const k = h.getSettings().deviceId;
        v != null && v.deviceId && nt(v.deviceId) !== k ? v.deviceId = k : v || (v = {
          deviceId: k
        });
        const _ = mh(h, v, e);
        return _.kind === T.Kind.Video ? _.source = T.Source.Camera : _.kind === T.Kind.Audio && (_.source = T.Source.Microphone), _.mediaStream = u, Ae(_) && n ? yield _.setProcessor(n) : Ge(_) && s && (yield _.setProcessor(s)), _;
      })));
    } catch (u) {
      if (!t)
        throw u;
      return sn(Object.assign(Object.assign({}, i), {
        audio: a,
        video: o
      }), e);
    }
...
function Fh(i) {
  return m(this, void 0, void 0, function* () {
    if (i === void 0 && (i = {}), i.resolution === void 0 && !wa() && (i.resolution = nn.h1080fps30.resolution), navigator.mediaDevices.getDisplayMedia === void 0)
      throw new tn("getDisplayMedia not supported");
    const e = Ca(i), t = yield navigator.mediaDevices.getDisplayMedia(e), n = t.getVideoTracks();
    if (n.length === 0)
      throw new we("no video track found");
    const s = new At(n[0], void 0, !1);
    s.source = T.Source.ScreenShare;
    const r = [s];
    if (t.getAudioTracks().length > 0) {
      const a = new Ot(t.getAudioTracks()[0], void 0, !1);
      a.source = T.Source.ScreenShareAudio, r.push(a);
    }
    return r;
  });
}
...
class Os extends Fe.EventEmitter {
  get logContext() {
    var e, t;
  ...
  get isActive() {
    var e;
    return ((e = this.participantInfo) === null || e === void 0 ? void 0 : e.state) === kt.ACTIVE;
  }
  get kind() {
    return this._kind;
  }
  ...
  constructor(e, t, n, s, r, a) {
    ...
    var c;
    super(), this.audioLevel = 0, this.isSpeaking = !1, this._connectionQuality = He.Unknown, this.log = F, this.log = Le((c = a == null ? void 0 : a.loggerName) !== null && c !== void 0 ? c : _e.Participant), this.loggerOptions = a, this.setMaxListeners(100), this.sid = e, this.identity = t, this.name = n, this.metadata = s, this.audioTrackPublications = /* @__PURE__ */ new Map(), this.videoTrackPublications = /* @__PURE__ */ new Map(), this.trackPublications = /* @__PURE__ */ new Map(), this._kind = o, this._attributes = r ?? {};
  }
   ...
   */
  waitUntilActive() {
    return this.isActive ? Promise.resolve() : this.activeFuture ? this.activeFuture.promise : (this.activeFuture = new Ee(), this.once(I.Active, () => {
      var e, t;
  ...
  setPermissions(e) {
    ...
    return this.permissions = e, d && this.emit(I.ParticipantPermissionsChanged, c), d;
  }
  /** @internal */
  setIsSpeaking(e) {
  ...
  setConnectionQuality(e) {
    ...
    this._connectionQuality = jh(e), t !== this._connectionQuality && this.emit(I.ConnectionQualityChanged, this._connectionQuality);
  }
  /**
   * @internal
   */
...
class nc extends Os {
  ...
  constructor(e, t, n, s, r, a) {
    super(e, t, void 0, void 0, void 0, {
      ...
      this.reconnectFuture || (this.reconnectFuture = new Ee());
      ...
      var c, d;
      o.participant && this.updateInfo(o.participant), this.signalConnectedFuture || (this.signalConnectedFuture = new Ee()), (d = (c = this.signalConnectedFuture).resolve) === null || d === void 0 || d.call(c);
      ...
      u && (d !== fs.OK && u.reject(new Vi(l, d)), this.pendingSignalRequests.delete(c));
      ...
      })), this.engine.client.sendUpdateSubscriptionPermissions(this.allParticipantsAllowedToSubscribe, this.participantTrackPermissions.map((o) => Vh(o)));
    }, this.onTrackUnmuted = (o) => {
      this.onTrackMuted(o, o.isUpstreamPaused);
    }, this.onTrackMuted = (o, c) => {
      if (c === void 0 && (c = !0), !o.sid) {
        this.log.error("could not update mute status for unpublished track", Object.assign(Object.assign({}, this.logContext), B(o)));
        return;
      }
      this.engine.updateMuteStatus(o.sid, c);
    }, this.onTrackUpstreamPaused = (o) => {
      this.log.debug("upstream paused", Object.assign(Object.assign({}, this.logContext), B(o))), this.onTrackMuted(o, !0);
    }, this.onTrackUpstreamResumed = (o) => {
      this.log.debug("upstream resumed", Object.assign(Object.assign({}, this.logContext), B(o))), this.onTrackMuted(o, o.isMuted);
      ...
      try {
        for (var v = !0, E = qe(g), k; k = yield E.next(), c = k.done, !c; v = !0) {
          u = k.value, v = !1;
          const _ = u;
          ka(_) && (this.log.debug("publish ".concat(_, " for ").concat(f.videoTrack.sid), Object.assign(Object.assign({}, this.logContext), B(f))), yield this.publishAdditionalCodecForTrack(f.videoTrack, _, f.options));
        }
      } catch (_) {
        d = {
          error: _
        ...
        try {
          if (ge())
            try {
              ...
              if (c && c.state === "denied")
                throw this.log.warn("user has revoked access to ".concat(o.source), Object.assign(Object.assign({}, this.logContext), B(o))), c.onchange = () => {
                  c.state !== "denied" && (o.isMuted || o.restartTrack(), c.onchange = null);
                }, new Error("GetUserMedia Permission denied");
            } catch {
            }
          o.isMuted || (this.log.debug("track ended, attempting to use a different device", Object.assign(Object.assign({}, this.logContext), B(o))), je(o) ? yield o.restartTrack({
            deviceId: "default"
          }) : yield o.restartTrack());
        } catch {
          this.log.warn("could not restart track, muting instead", Object.assign(Object.assign({}, this.logContext), B(o))), yield o.mute();
        }
  ...
  get isE2EEEnabled() {
    return this.encryptionType !== ne.NONE;
  }
  getTrackPublication(e) {
  ...
  setupEngine(e) {
    var t;
    this.engine = e, this.engine.on(w.RemoteMute, (n, s) => {
  ...
  requestMetadataUpdate(e) {
    return m(this, arguments, void 0, function(t) {
      ...
      return function* () {
        return new fe((o, c) => m(n, void 0, void 0, function* () {
          var d, l;
          try {
            let u = !1;
            ...
            c(new Vi("Request to update local metadata timed out", "TimeoutError"));
          } catch (u) {
  ...
  setCameraEnabled(e, t, n) {
    return this.setTrackEnabled(T.Source.Camera, e, t, n);
  }
  /**
  ...
  setMicrophoneEnabled(e, t, n) {
    return this.setTrackEnabled(T.Source.Microphone, e, t, n);
  }
  /**
  ...
  setScreenShareEnabled(e, t, n) {
    return this.setTrackEnabled(T.Source.ScreenShare, e, t, n);
  }
  /** @internal */
  setE2EEEnabled(e) {
    return m(this, void 0, void 0, function* () {
        ...
        if (o)
          ...
          try {
            ...
            throw c == null || c.forEach((l) => {
              l.stop();
            }), d instanceof Error && this.emit(I.MediaDevicesError, d, Hi(e)), this.pendingPublishing.delete(e), d;
          }
          for (const d of c) {
          ...
          try {
            const d = [];
            for (const u of c)
              this.log.info("publishing track", Object.assign(Object.assign({}, this.logContext), B(u))), d.push(this.publishTrack(u, s));
  ...
  createTracks(e) {
    return m(this, void 0, void 0, function* () {
      var t, n;
      e ?? (e = {});
      const s = Ta(e, (t = this.roomOptions) === null || t === void 0 ? void 0 : t.audioCaptureDefaults, (n = this.roomOptions) === null || n === void 0 ? void 0 : n.videoCaptureDefaults);
      try {
        return (yield sn(s, {
          loggerName: this.roomOptions.loggerName,
          loggerContextCb: () => this.logContext
        })).map((o) => (Ae(o) && (this.microphoneError = void 0, o.setAudioContext(this.audioContext), o.source = T.Source.Microphone, this.emit(I.AudioStreamAcquired)), Ge(o) && (this.cameraError = void 0, o.source = T.Source.Camera), o));
      } catch (r) {
  ...
  createScreenTracks(e) {
    return m(this, void 0, void 0, function* () {
      if (e === void 0 && (e = {}), navigator.mediaDevices.getDisplayMedia === void 0)
        throw new tn("getDisplayMedia not supported");
      e.resolution === void 0 && !wa() && (e.resolution = nn.h1080fps30.resolution);
      const t = Ca(e), n = yield navigator.mediaDevices.getDisplayMedia(t), s = n.getVideoTracks();
      if (s.length === 0)
        throw new we("no video track found");
      const r = new At(s[0], void 0, !1, {
        loggerName: this.roomOptions.loggerName,
        loggerContextCb: () => this.logContext
      });
      ...
      if (n.getAudioTracks().length > 0) {
        ...
        const o = new Ot(n.getAudioTracks()[0], void 0, !1, this.audioContext, {
          loggerName: this.roomOptions.loggerName,
          loggerContextCb: () => this.logContext
        });
  ...
  publishOrRepublishTrack(e, t) {
    return m(this, arguments, void 0, function(n, s) {
      ...
      return function* () {
        var o, c, d, l;
        je(n) && n.setAudioContext(r.audioContext), yield (o = r.reconnectFuture) === null || o === void 0 ? void 0 : o.promise, r.republishPromise && !a && (yield r.republishPromise), it(n) && r.pendingPublishPromises.has(n) && (yield r.pendingPublishPromises.get(n));
        let u;
        ...
        if (n instanceof MediaStreamTrack)
          switch (n.kind) {
            ...
            case "video":
              ...
              break;
            default:
              throw new we("unsupported MediaStreamTrack kind ".concat(n.kind));
          }
        else
        ...
        const f = Object.assign(Object.assign({}, r.roomOptions.publishDefaults), s), g = "channelCount" in n.mediaStreamTrack.getSettings() && // @ts-ignore `channelCount` on getSettings() is currently only available for Safari, but is generally the best way to determine a stereo track https://developer.mozilla.org/en-US/docs/Web/API/MediaTrackSettings/channelCount
        n.mediaStreamTrack.getSettings().channelCount === 2 || n.mediaStreamTrack.getConstraints().channelCount === 2, v = (c = f.forceStereo) !== null && c !== void 0 ? c : g;
        v && (f.dtx === void 0 && r.log.info("Opus DTX will be disabled for stereo tracks by default. Enable them explicitly to make it work.", Object.assign(Object.assign({}, r.logContext), B(n))), f.red === void 0 && r.log.info("Opus RED will be disabled for stereo tracks by default. Enable them explicitly to make it work."), (d = f.dtx) !== null && d !== void 0 || (f.dtx = !1), (l = f.red) !== null && l !== void 0 || (f.red = !1)), !cu() && r.roomOptions.e2ee && (r.log.info("End-to-end encryption is set up, simulcast publishing will be disabled on Safari versions and iOS browsers running iOS < v17.2", Object.assign({}, r.logContext)), f.simulcast = !1), f.source && (n.source = f.source);
        const E = new Promise((k, _) => m(r, void 0, void 0, function* () {
          try {
            if (this.engine.client.currentState !== G.CONNECTED) {
              ...
              let M = !1;
              const y = setTimeout(() => {
                M = !0, n.stop(), _(new ji("publishing rejected as engine not connected within timeout", 408));
              }, 15e3);
              if (yield this.waitUntilEngineConnected(), clearTimeout(y), M)
                return;
              const b = yield this.publish(n, f, v);
              k(b);
            } else
              try {
                const M = yield this.publish(n, f, v);
                k(M);
              } catch (M) {
                _(M);
              }
  ...
  waitUntilEngineConnected() {
    return this.signalConnectedFuture || (this.signalConnectedFuture = new Ee()), this.signalConnectedFuture.promise;
  }
  hasPermissionsToPublish(e) {
    if (!this.permissions)
      return this.log.warn("no permissions present for publishing track", Object.assign(Object.assign({}, this.logContext), B(e))), !1;
    const {
      canPublish: t,
      canPublishSources: n
    } = this.permissions;
    return t && (n.length === 0 || n.map((s) => Zl(s)).includes(e.source)) ? !0 : (this.log.warn("insufficient permissions to publish", Object.assign(Object.assign({}, this.logContext), B(e))), !1);
  }
  publish(e, t, n) {
    return m(this, void 0, void 0, function* () {
      var s, r, a, o, c, d, l, u, h, f;
      if (!this.hasPermissionsToPublish(e))
        throw new ji("failed to publish track, insufficient permissions", 403);
      Array.from(this.trackPublications.values()).find((D) => it(e) && D.source === e.source) && e.source !== T.Source.Unknown && this.log.info("publishing a second track with the same source: ".concat(e.source), Object.assign(Object.assign({}, this.logContext), B(e))), t.stopMicTrackOnMute && Ae(e) && (e.stopOnMute = !0), e.source === T.Source.ScreenShare && ft() && (t.simulcast = !1), t.videoCodec === "av1" && !_a() && (t.videoCodec = void 0), t.videoCodec === "vp9" && !Pa() && (t.videoCodec = void 0), t.videoCodec === void 0 && (t.videoCodec = zi), this.enabledPublishVideoCodecs.length > 0 && (this.enabledPublishVideoCodecs.some((D) => t.videoCodec === Jt(D.mime)) || (t.videoCodec = Jt(this.enabledPublishVideoCodecs[0].mime)));
      ...
      _.autoGainControl && E.push(ie.TF_AUTO_GAIN_CONTROL), _.echoCancellation && E.push(ie.TF_ECHO_CANCELLATION), _.noiseSuppression && E.push(ie.TF_NOISE_SUPPRESSION), _.channelCount && _.channelCount > 1 && E.push(ie.TF_STEREO), k && E.push(ie.TF_NO_DTX), je(e) && e.hasPreConnectBuffer && E.push(ie.TF_PRECONNECT_BUFFER);
      const M = new Yt({
      ...
      if (e.kind === T.Kind.Video) {
        ...
        try {
          ...
          const N = (o = (a = this.roomOptions.videoCaptureDefaults) === null || a === void 0 ? void 0 : a.resolution) !== null && o !== void 0 ? o : wt.h720.resolution;
          D = {
            width: N.width,
            height: N.height
        ...
        M.width = D.width, M.height = D.height, $e(e) && (Ie(v) && (e.source === T.Source.ScreenShare && (t.scalabilityMode = "L1T3", "contentHint" in e.mediaStreamTrack && (e.mediaStreamTrack.contentHint = "motion", this.log.info("forcing contentHint to motion for screenshare with SVC codecs", Object.assign(Object.assign({}, this.logContext), B(e))))), t.scalabilityMode = (c = t.scalabilityMode) !== null && c !== void 0 ? c : "L3T3_KEY"), M.simulcastCodecs = [new Mi({
          codec: v,
          cid: e.mediaStreamTrack.id
        ...
        M.encryption === ne.NONE && (this.roomOptions.dynacast || (this.roomOptions.dynacast = !0), M.simulcastCodecs.push(new Mi({
          codec: t.backupCodec.codec,
          cid: ""
        })))), y = Qi(e.source === T.Source.ScreenShare, M.width, M.height, t), M.layers = Or(M.width, M.height, y, Ie(t.videoCodec));
      ...
      const b = () => m(this, void 0, void 0, function* () {
        ...
        if (e.sender = yield this.engine.createSender(e, t, y), this.emit(I.LocalSenderCreated, e.sender, e), $e(e) && ((D = t.degradationPreference) !== null && D !== void 0 || (t.degradationPreference = Ch(e)), e.setDegradationPreference(t.degradationPreference)), y)
          if (ft() && e.kind === T.Kind.Audio) {
            let j;
            for (const Y of this.engine.pcManager.publisher.getTransceivers())
              if (Y.sender === e.sender) {
                j = Y;
                break;
              }
          ...
          } else e.codec && Ie(e.codec) && (!((N = y[0]) === null || N === void 0) && N.maxBitrate) && this.engine.pcManager.publisher.setTrackCodecBitrate({
            cid: M.cid,
            codec: e.codec,
      ...
      const O = new Promise((D, A) => m(this, void 0, void 0, function* () {
        ...
        try {
          ...
          e.sender && (!((N = this.engine.pcManager) === null || N === void 0) && N.publisher) && (this.engine.pcManager.publisher.removeTrack(e.sender), yield this.engine.negotiate().catch((Y) => {
      ...
      if (this.enabledPublishVideoCodecs.length > 0)
        ...
        if (S.codecs.forEach((A) => {
          ...
          const A = Jt(D);
          A !== v && (this.log.debug("falling back to server selected codec", Object.assign(Object.assign(Object.assign({}, this.logContext), B(e)), {
            codec: A
          })), t.videoCodec = A, y = Qi(e.source === T.Source.ScreenShare, M.width, M.height, t));
        }
        yield b();
      }
      const L = new Bn(e.kind, S, e, {
        loggerName: this.roomOptions.loggerName,
        loggerContextCb: () => this.logContext
      });
      if (L.on(R.CpuConstrained, (D) => this.onTrackCpuConstrained(D, L)), L.options = t, e.sid = S.sid, this.log.debug("publishing ".concat(e.kind, " with encodings"), Object.assign(Object.assign({}, this.logContext), {
        encodings: y,
        trackInfo: S
      })), $e(e) ? e.startMonitor(this.engine.client) : je(e) && e.startMonitor(), this.addTrackPublication(L), this.emit(I.LocalTrackPublished, L), je(e) && S.audioFeatures.includes(ie.TF_PRECONNECT_BUFFER)) {
        ...
        this.on(I.LocalTrackSubscribed, (N) => {
        ...
        }), D && new Promise((j, Y) => m(this, void 0, void 0, function* () {
          var ye, ot, de, Ye, Zn, ei;
          try {
            this.log.debug("waiting for agent", Object.assign(Object.assign({}, this.logContext), B(e)));
            const si = setTimeout(() => {
              Y(new Error("agent not active within 10 seconds"));
            }, 1e4), lc = yield this.waitUntilActiveAgentPresent();
            clearTimeout(si), this.log.debug("sending preconnect buffer", Object.assign(Object.assign({}, this.logContext), B(e)));
            const Ms = yield this.streamBytes({
              name: "preconnect-buffer",
              mimeType: A,
              topic: "lk.agent.pre-connect-audio-buffer",
              destinationIdentities: [lc.identity],
              attributes: {
                trackId: L.trackSid,
                sampleRate: String((Zn = _.sampleRate) !== null && Zn !== void 0 ? Zn : "48000"),
                channels: String((ei = _.channelCount) !== null && ei !== void 0 ? ei : "1")
              }
            });
            try {
              for (var ti = !0, ni = qe(D), ii; ii = yield ni.next(), ye = ii.done, !ye; ti = !0) {
                Ye = ii.value, ti = !1;
                const ri = Ye;
                yield Ms.write(ri);
              }
              ...
              try {
                !ti && !ye && (de = ni.return) && (yield de.call(ni));
              } finally {
          ...
          this.log.error("error sending preconnect buffer", Object.assign(Object.assign(Object.assign({}, this.logContext), B(e)), {
            error: j
          }));
        });
      }
  ...
  publishAdditionalCodecForTrack(e, t, n) {
    return m(this, void 0, void 0, function* () {
      ...
      if (this.trackPublications.forEach((f) => {
        ...
        throw new we("track is not published");
      if (!$e(e))
        throw new we("track is not a video track");
      const a = Object.assign(Object.assign({}, (s = this.roomOptions) === null || s === void 0 ? void 0 : s.publishDefaults), n), o = kh(e, t, a);
      if (!o) {
      ...
      if (d.layers = Or(d.width, d.height, o), !this.engine || this.engine.isClosed)
        throw new $("cannot publish track when not connected");
  ...
  unpublishTrack(e, t) {
    return m(this, void 0, void 0, function* () {
      ...
      const r = this.getPublicationForTrack(e), a = r ? B(r) : void 0;
      ...
      if (e.sender = void 0, this.engine.pcManager && this.engine.pcManager.currentState < X.FAILED && c)
        try {
          ...
          if (this.engine.removeTrack(c) && (o = !0), $e(e)) {
  ...
  republishAllTracks(e) {
    return m(this, arguments, void 0, function(t) {
      ...
      let s = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : !0;
      return function* () {
        n.republishPromise && (yield n.republishPromise), n.republishPromise = new fe((r, a) => m(n, void 0, void 0, function* () {
          try {
            const o = [];
            this.trackPublications.forEach((c) => {
              ...
              const d = c.track;
              yield this.unpublishTrack(d, !1), s && !d.isMuted && d.source !== T.Source.ScreenShare && d.source !== T.Source.ScreenShareAudio && (je(d) || $e(d)) && !d.isUserProvided && (this.log.debug("restarting existing track", Object.assign(Object.assign({}, this.logContext), {
                track: c.trackSid
  ...
  publishData(e) {
    return m(this, arguments, void 0, function(t) {
      ...
      return function* () {
        const r = s.reliable ? V.RELIABLE : V.LOSSY, a = s.destinationIdentities, o = s.topic;
        let c = new rs({
  ...
  publishDtmf(e, t) {
    return m(this, void 0, void 0, function* () {
      ...
      yield this.engine.sendDataPacket(n, V.RELIABLE);
    });
  }
  ...
  sendChatMessage(e, t) {
    return m(this, void 0, void 0, function* () {
      const n = {
        ...
        value: {
          case: "chatMessage",
          value: new In(Object.assign(Object.assign({}, n), {
            timestamp: z.parse(n.timestamp)
          }))
        }
      });
      return yield this.engine.sendDataPacket(s, V.RELIABLE), this.emit(I.ChatMessage, n), n;
    });
  }
  /** @deprecated Consider migrating to {@link sendText} */
  editChatMessage(e, t) {
    return m(this, void 0, void 0, function* () {
      const n = Object.assign(Object.assign({}, t), {
        ...
        value: {
          ...
          value: new In(Object.assign(Object.assign({}, n), {
            timestamp: z.parse(n.timestamp),
            editTimestamp: z.parse(n.editTimestamp)
          }))
        }
      });
      return yield this.engine.sendDataPacket(s, V.RELIABLE), this.emit(I.ChatMessage, n), n;
  ...
  performRpc(e) {
    ...
    const a = 7e3, o = a + 1e3;
    return new fe((c, d) => m(this, void 0, void 0, function* () {
      var l, u, h, f;
      if (_s(s) > Ja) {
        d(Q.builtIn("REQUEST_PAYLOAD_TOO_LARGE"));
        return;
      }
      if (!((u = (l = this.engine.latestJoinResponse) === null || l === void 0 ? void 0 : l.serverInfo) === null || u === void 0) && u.version && Me((f = (h = this.engine.latestJoinResponse) === null || h === void 0 ? void 0 : h.serverInfo) === null || f === void 0 ? void 0 : f.version, "1.8.0") < 0) {
        d(Q.builtIn("UNSUPPORTED_SERVER"));
        return;
      }
      const g = Math.max(r, o), v = crypto.randomUUID();
      yield this.publishRpcRequest(t, v, n, s, g);
      const E = setTimeout(() => {
        this.pendingAcks.delete(v), d(Q.builtIn("CONNECTION_TIMEOUT")), this.pendingResponses.delete(v), clearTimeout(k);
      }, a);
      ...
      const k = setTimeout(() => {
        this.pendingResponses.delete(v), d(Q.builtIn("RESPONSE_TIMEOUT"));
      }, r);
      this.pendingResponses.set(v, {
        resolve: (_, M) => {
          clearTimeout(k), this.pendingAcks.has(v) && (this.log.warn("RPC response received before ack", v), this.pendingAcks.delete(v), clearTimeout(E)), M ? d(M) : c(_ ?? "");
        },
        participantIdentity: t
      });
    }));
  ...
  handleParticipantDisconnected(e) {
    ...
    for (const [t, {
      ...
      n === e && (s(null, Q.builtIn("RECIPIENT_DISCONNECTED")), this.pendingResponses.delete(t));
  }
  /** @internal */
  ...
  updateInfo(e) {
    return super.updateInfo(e) ? (e.tracks.forEach((t) => {
      ...
      if (r) {
        ...
        a !== t.muted && (this.log.debug("updating server mute state after reconcile", Object.assign(Object.assign(Object.assign({}, this.logContext), B(r)), {
          mutedOnServer: a
  ...
  waitUntilActiveAgentPresent() {
    return this.firstActiveAgent ? Promise.resolve(this.firstActiveAgent) : (this.activeAgentFuture || (this.activeAgentFuture = new Ee()), this.activeAgentFuture.promise);
  }
  getPublicationForTrack(e) {
    let t;
    return this.trackPublications.forEach((n) => {
      const s = n.track;
      s && (e instanceof MediaStreamTrack ? (je(s) || $e(s)) && s.mediaStreamTrack === e && (t = n) : e === s && (t = n));
    }), t;
  }
...
class ic extends Ne {
  ...
  setVideoDimensions(e) {
    var t, n;
    this.isManualOperationAllowed() && (((t = this.requestedVideoDimensions) === null || t === void 0 ? void 0 : t.width) === e.width && ((n = this.requestedVideoDimensions) === null || n === void 0 ? void 0 : n.height) === e.height || (gi(this.track) && (this.requestedVideoDimensions = e), this.requestedMaxQuality = void 0, this.emitTrackUpdate()));
  }
  setVideoFPS(e) {
    this.isManualOperationAllowed() && gi(this.track) && this.fps !== e && (this.fps = e, this.emitTrackUpdate());
  }
  get videoQuality() {
    var e;
    return (e = this.requestedMaxQuality) !== null && e !== void 0 ? e : pe.HIGH;
  }
  /** @internal */
  setTrack(e) {
  ...
  get isAdaptiveStream() {
    return gi(this.track) && this.track.isAdaptiveStream;
  }
  /* @internal */
  emitTrackUpdate() {
    const e = new Co({
    ...
    if (this.kind === T.Kind.Video) {
      ...
      if (this.videoDimensionsAdaptiveStream !== void 0)
        if (t)
          cr(this.videoDimensionsAdaptiveStream, t) && (this.log.debug("using adaptive stream dimensions instead of requested", Object.assign(Object.assign({}, this.logContext), this.videoDimensionsAdaptiveStream)), t = this.videoDimensionsAdaptiveStream);
        else if (this.requestedMaxQuality !== void 0 && this.trackInfo) {
          const n = eu(this.trackInfo, this.requestedMaxQuality);
          n && cr(this.videoDimensionsAdaptiveStream, n) && (this.log.debug("using adaptive stream dimensions instead of max quality layer", Object.assign(Object.assign({}, this.logContext), this.videoDimensionsAdaptiveStream)), t = this.videoDimensionsAdaptiveStream);
        } else
...
class Zt extends Os {
  ...
  static fromParticipantInfo(e, t, n) {
    return new Zt(e, t.sid, t.identity, t.name, t.metadata, t.attributes, n, t.kind);
  }
  get logContext() {
  ...
  addSubscribedMediaTrack(e, t, n, s, r, a) {
    ...
    if (e.readyState === "ended") {
      this.log.error("unable to subscribe because MediaStreamTrack is ended. Do not call MediaStreamTrack.stop()", Object.assign(Object.assign({}, this.logContext), B(o))), this.emit(I.TrackSubscriptionFailed, t);
      return;
    }
    const c = e.kind === "video";
    let d;
    return c ? d = new Za(e, t, s, r) : d = new Xa(e, t, s, this.audioContext, this.audioOutput), d.source = o.source, d.isMuted = o.isMuted, d.setMediaStream(n), d.start(), o.setTrack(d), this.volumeMap.has(o.source) && Fn(d) && Ae(d) && d.setVolume(this.volumeMap.get(o.source)), o;
  }
  /** @internal */
  get hasMetadata() {
  ...
  updateInfo(e) {
    ...
    return e.tracks.forEach((s) => {
      ...
      if (o)
        ...
        if (!c)
          return;
        o = new ic(c, s, (r = this.signalClient.connectOptions) === null || r === void 0 ? void 0 : r.autoSubscribe, {
          loggerContextCb: () => this.logContext,
          loggerName: (a = this.loggerOptions) === null || a === void 0 ? void 0 : a.loggerName
        }), o.updateInfo(s), n.set(s.sid, o);
        const d = Array.from(this.trackPublications.values()).find((l) => l.source === (o == null ? void 0 : o.source));
      ...
      t.has(s.trackSid) || (this.log.trace("detected removed track on remote participant, unpublishing", Object.assign(Object.assign({}, this.logContext), B(s))), this.unpublishTrack(s.trackSid, !0));
    }), n.forEach((s) => {
  ...
  setAudioOutput(e) {
    return m(this, void 0, void 0, function* () {
      ...
      this.audioTrackPublications.forEach((n) => {
        var s;
        Ae(n.track) && Fn(n.track) && t.push(n.track.setSinkId((s = e.deviceId) !== null && s !== void 0 ? s : "default"));
      }), yield Promise.all(t);
    });
  }
  /** @internal */
...
const Bh = 4 * 1e3;
class Mt extends Fe.EventEmitter {
  get hasE2EESetup() {
  ...
  constructor(e) {
    ...
    if (super(), t = this, this.state = K.Disconnected, this.activeSpeakers = [], this.isE2EEEnabled = !1, this.audioEnabled = !0, this.isVideoPlaybackBlocked = !1, this.log = F, this.bufferedEvents = [], this.isResuming = !1, this.rpcHandlers = /* @__PURE__ */ new Map(), this.connect = (o, c, d) => m(this, void 0, void 0, function* () {
      ...
      this.setAndEmitConnectionState(K.Connecting), ((l = this.regionUrlProvider) === null || l === void 0 ? void 0 : l.getServerUrl().toString()) !== xa(o) && (this.regionUrl = void 0, this.regionUrlProvider = void 0), It(new URL(o)) && (this.regionUrlProvider === void 0 ? this.regionUrlProvider = new H(o, c) : this.regionUrlProvider.updateToken(c), this.regionUrlProvider.fetchRegionSettings().then((g) => {
        var v;
        (v = this.regionUrlProvider) === null || v === void 0 || v.setServerReportedRegions(g);
      }).catch((g) => {
        this.log.warn("could not fetch region settings", Object.assign(Object.assign({}, this.logContext), {
          error: g
        }));
      }));
      const h = (g, v, E) => m(this, void 0, void 0, function* () {
        var k, _;
        this.abortController && this.abortController.abort();
        const M = new AbortController();
        this.abortController = M, u == null || u();
        try {
          if (yield Ct.getInstance().getBackOffPromise(o), M.signal.aborted)
            throw U.cancelled("Connection attempt aborted");
          yield this.attemptConnection(E ?? o, c, d, M), this.abortController = void 0, g();
        } catch (y) {
          if (this.regionUrlProvider && y instanceof U && y.reason !== J.Cancelled && y.reason !== J.NotAllowed) {
            let b = null;
            try {
              ...
              if (S instanceof U && (S.status === 401 || S.reason === J.Cancelled)) {
                this.handleDisconnect(this.options.stopLocalTrackOnUnpublish), v(S);
                return;
              }
            }
            // making sure we only register failed attempts on things we actually care about
            [J.InternalError, J.ServerUnreachable, J.Timeout].includes(y.reason) && (this.log.debug("Adding failed connection attempt to back off"), Ct.getInstance().addFailedConnectionAttempt(o)), b && !(!((_ = this.abortController) === null || _ === void 0) && _.signal.aborted) ? (this.log.info("Initial connection failed with ConnectionError: ".concat(y.message, ". Retrying with another region: ").concat(b), this.logContext), this.recreateEngine(), yield h(g, v, b)) : (this.handleDisconnect(this.options.stopLocalTrackOnUnpublish, fr(y)), v(y));
          } else {
            let b = Ce.UNKNOWN_REASON;
            y instanceof U && (b = fr(y)), this.handleDisconnect(this.options.stopLocalTrackOnUnpublish, b), v(y);
          }
        }
      }), f = this.regionUrl;
      return this.regionUrl = void 0, this.connectFuture = new Ee((g, v) => {
        h(g, v, f);
      }, () => {
      ...
      if (k || (k = {
        ...
        roomSid: (g = E.room) === null || g === void 0 ? void 0 : g.sid,
        identity: (v = E.participant) === null || v === void 0 ? void 0 : v.identity
      }), !k.version)
        throw new ga("unknown server version");
      ...
      if (this.localParticipant.sid = c.sid, this.localParticipant.identity = c.identity, this.localParticipant.setEnabledPublishCodecs(o.enabledPublishCodecs), this.e2eeManager)
        try {
          this.e2eeManager.setSifTrailer(o.sifTrailer);
        } catch (d) {
      ...
      try {
        ...
        const g = l.signal.aborted ? U.cancelled("Signal connection aborted") : U.serverUnreachable("could not establish signal connection");
        throw f instanceof Error && (g.message = "".concat(g.message, ": ").concat(f.message)), f instanceof U && (g.reason = f.reason, g.status = f.status), this.log.debug("error trying to establish signal connection", Object.assign(Object.assign({}, this.logContext), {
          error: f
        })), g;
      }
      if (l.signal.aborted)
      ...
      ge() && this.options.disconnectOnPageLeave && (window.addEventListener("pagehide", this.onPageLeave), window.addEventListener("beforeunload", this.onPageLeave)), ge() && window.addEventListener("freeze", this.onPageLeave), this.setAndEmitConnectionState(K.Connected), this.emit(P.Connected), Ct.getInstance().resetFailedConnectionAttempts(o), this.registerConnectionReconcile(), this.regionUrlProvider && this.regionUrlProvider.notifyConnected();
        ...
        return function* () {
          ...
          try {
            ...
            if (l.log.info("disconnect from room", Object.assign({}, l.logContext)), l.state === K.Connecting || l.state === K.Reconnecting || l.isResuming) {
              ...
              l.log.warn(E, l.logContext), (h = l.abortController) === null || h === void 0 || h.abort(E), (g = (f = l.connectFuture) === null || f === void 0 ? void 0 : f.reject) === null || g === void 0 || g.call(f, U.cancelled("Client initiated disconnect")), l.connectFuture = void 0;
            }
            l.engine && (l.engine.client.isDisconnected || (yield l.engine.client.sendLeave()), yield l.engine.close()), l.handleDisconnect(u, Ce.CLIENT_INITIATED), l.engine = void 0;
          } finally {
            v();
          }
        }();
      ...
      if (c && c.os === "iOS") {
        ...
        if (!l) {
          l = document.createElement("audio"), l.id = d, l.autoplay = !0, l.hidden = !0;
          const u = kn();
          u.enabled = !0;
      ...
      try {
        yield Promise.all([this.acquireAudioContext(), ...o.map((d) => (d.muted = !1, d.play()))]), this.handleAudioPlaybackStarted();
      } catch (d) {
      ...
      yield Promise.all(o.map((c) => c.play())).then(() => {
      ...
      o.forEach((c) => {
        ...
        c.state === kt.DISCONNECTED ? this.handleParticipantDisconnected(c.identity, l) : l = this.getOrCreateParticipant(c.identity, c);
      });
    }, this.handleActiveSpeakersUpdate = (o) => {
      const c = [], d = {};
      o.forEach((l) => {
        if (d[l.sid] = !0, l.sid === this.localParticipant.sid)
          this.localParticipant.audioLevel = l.level, this.localParticipant.setIsSpeaking(!0), c.push(this.localParticipant);
        else {
          const u = this.getRemoteParticipantBySid(l.sid);
          u && (u.audioLevel = l.level, u.setIsSpeaking(!0), c.push(u));
        }
      ...
      d && d.setAllowed(o.allowed);
    }, this.handleSubscriptionError = (o) => {
      const c = Array.from(this.remoteParticipants.values()).find((l) => l.trackPublications.has(o.trackSid));
      if (!c)
        return;
      const d = c.getTrackPublicationBySid(o.trackSid);
      d && d.setSubscriptionError(o.err);
    }, this.handleDataPacket = (o, c) => {
      const d = this.remoteParticipants.get(o.participantIdentity);
      if (o.value.case === "user")
        this.handleUserPacket(d, o.value.value, o.kind, c);
      else if (o.value.case === "transcription")
        this.handleTranscription(d, o.value.value);
      else if (o.value.case === "sipDtmf")
        this.handleSipDtmf(d, o.value.value);
      else if (o.value.case === "chatMessage")
        this.handleChatMessage(d, o.value.value);
      else if (o.value.case === "metrics")
        this.handleMetrics(o.value.value, d);
      else if (o.value.case === "streamHeader" || o.value.case === "streamChunk" || o.value.case === "streamTrailer")
        this.handleDataStream(o, c);
      else if (o.value.case === "rpcRequest") {
        const l = o.value.value;
      ...
      const d = c.transcribedParticipantIdentity === this.localParticipant.identity ? this.localParticipant : this.getParticipantByIdentity(c.transcribedParticipantIdentity), l = d == null ? void 0 : d.trackPublications.get(c.trackId), u = gu(c, this.transcriptionReceivedTimes);
      l == null || l.emit(R.TranscriptionReceived, u), d == null || d.emit(I.TranscriptionReceived, u, l), this.emit(P.TranscriptionReceived, u, d, l);
      ...
      var o;
      ((o = ue()) === null || o === void 0 ? void 0 : o.os) !== "iOS" && (yield this.selectDefaultDevices()), this.emit(P.MediaDevicesChanged);
      ...
      var c, d, l, u, h, f;
      (c = o.track) === null || c === void 0 || c.on(R.TrackProcessorUpdate, this.onTrackProcessorUpdate), (d = o.track) === null || d === void 0 || d.on(R.Restarted, this.onLocalTrackRestarted), (h = (u = (l = o.track) === null || l === void 0 ? void 0 : l.getProcessor()) === null || u === void 0 ? void 0 : u.onPublish) === null || h === void 0 || h.call(u, this), this.emit(P.LocalTrackPublished, o, this.localParticipant), je(o.track) && (yield o.track.checkForSilence()) && this.emit(P.LocalAudioSilenceDetected, o);
      const g = yield (f = o.track) === null || f === void 0 ? void 0 : f.getDeviceId(!1), v = Hi(o.source);
      v && g && g !== this.localParticipant.activeDeviceMap.get(v) && (this.localParticipant.activeDeviceMap.set(v, g), this.emit(P.ActiveDeviceChanged, v, g));
    }), this.onLocalTrackUnpublished = (o) => {
      var c, d;
      (c = o.track) === null || c === void 0 || c.off(R.TrackProcessorUpdate, this.onTrackProcessorUpdate), (d = o.track) === null || d === void 0 || d.off(R.Restarted, this.onLocalTrackRestarted), this.emit(P.LocalTrackUnpublished, o, this.localParticipant);
    }, this.onLocalTrackRestarted = (o) => m(this, void 0, void 0, function* () {
      const c = yield o.getDeviceId(!1), d = Hi(o.source);
    ...
    }, this.setMaxListeners(100), this.remoteParticipants = /* @__PURE__ */ new Map(), this.sidToIdentity = /* @__PURE__ */ new Map(), this.options = Object.assign(Object.assign({}, sh), e), this.log = Le((n = this.options.loggerName) !== null && n !== void 0 ? n : _e.Room), this.transcriptionReceivedTimes = /* @__PURE__ */ new Map(), this.options.audioCaptureDefaults = Object.assign(Object.assign({}, Ha), e == null ? void 0 : e.audioCaptureDefaults), this.options.videoCaptureDefaults = Object.assign(Object.assign({}, Ka), e == null ? void 0 : e.videoCaptureDefaults), this.options.publishDefaults = Object.assign(Object.assign({}, ih), e == null ? void 0 : e.publishDefaults), this.maybeCreateEngine(), this.incomingDataStreamManager = new Dh(), this.outgoingDataStreamManager = new Lh(this.engine, this.log), this.disconnectLock = new ae(), this.localParticipant = new nc("", "", this.engine, this.options, this.rpcHandlers, this.outgoingDataStreamManager), (this.options.e2ee || this.options.encryption) && this.setupE2EE(), this.engine.e2eeManager = this.e2eeManager, this.options.videoCaptureDefaults.deviceId && this.localParticipant.activeDeviceMap.set("videoinput", nt(this.options.videoCaptureDefaults.deviceId)), this.options.audioCaptureDefaults.deviceId && this.localParticipant.activeDeviceMap.set("audioinput", nt(this.options.audioCaptureDefaults.deviceId)), !((s = this.options.audioOutput) === null || s === void 0) && s.deviceId && this.switchActiveDevice("audiooutput", nt(this.options.audioOutput.deviceId)).catch((o) => this.log.warn("Could not set audio output: ".concat(o.message), this.logContext)), ge()) {
  ...
  setupE2EE() {
    ...
    n && ("e2eeManager" in n ? (this.e2eeManager = n.e2eeManager, this.e2eeManager.isDataChannelEncryptionEnabled = t) : this.e2eeManager = new Vu(n, t), this.e2eeManager.on(Ke.ParticipantEncryptionStatusChanged, (s, r) => {
      Da(r) && (this.isE2EEEnabled = s), this.emit(P.ParticipantEncryptionStatusChanged, s, r);
    }), this.e2eeManager.on(Ke.EncryptionError, (s, r) => {
   ...
   */
  getSid() {
    return this.state === K.Disconnected ? fe.resolve("") : this.roomInfo && this.roomInfo.sid !== "" ? fe.resolve(this.roomInfo.sid) : new fe((e, t) => {
      const n = (s) => {
        s.sid !== "" && (this.engine.off(w.RoomUpdate, n), e(s.sid));
      };
      this.engine.on(w.RoomUpdate, n), this.once(P.Disconnected, () => {
        this.engine.off(w.RoomUpdate, n), t(new $("Room disconnected before room server id was available"));
      });
    });
  }
  ...
  maybeCreateEngine() {
    this.engine && !this.engine.isClosed || (this.engine = new Ih(this.options), this.engine.e2eeManager = this.e2eeManager, this.engine.on(w.ParticipantUpdate, this.handleParticipantUpdates).on(w.RoomUpdate, this.handleRoomUpdate).on(w.SpeakersChanged, this.handleSpeakersChanged).on(w.StreamStateChanged, this.handleStreamStateUpdate).on(w.ConnectionQualityUpdate, this.handleConnectionQualityUpdate).on(w.SubscriptionError, this.handleSubscriptionError).on(w.SubscriptionPermissionUpdate, this.handleSubscriptionPermissionUpdate).on(w.MediaTrackAdded, (e, t, n) => {
  ...
  prepareConnection(e, t) {
    return m(this, void 0, void 0, function* () {
      if (this.state === K.Disconnected) {
        ...
        try {
          if (It(new URL(e)) && t) {
            this.regionUrlProvider = new H(e, t);
            const n = yield this.regionUrlProvider.getNextBestRegionUrl();
            n && this.state === K.Disconnected && (this.regionUrl = n, yield fetch(Xt(n), {
              method: "HEAD"
    ...
    return m(this, arguments, void 0, function(n, s) {
      ...
      return function* () {
        ...
        if (n === "audioinput") {
          ...
          if (v = !0, !Un() && !r.options.webAudioMix || r.options.webAudioMix && r.audioContext && !("setSinkId" in r.audioContext))
        ...
        return v && (r.localParticipant.activeDeviceMap.set(n, s), r.emit(P.ActiveDeviceChanged, n, s)), g;
      }();
    });
  }
  ...
  onTrackAdded(e, t, n) {
    if (this.state === K.Connecting || this.state === K.Reconnecting) {
      const u = () => {
  ...
  handleDisconnect() {
    ...
    var n, s;
    if (this.clearConnectionReconcile(), this.isResuming = !1, this.bufferedEvents = [], this.transcriptionReceivedTimes.clear(), this.incomingDataStreamManager.clearControllers(), this.state !== K.Disconnected) {
  ...
  handleIncomingRpcRequest(e, t, n, s, r, a) {
    return m(this, void 0, void 0, function* () {
      ...
      try {
        ...
        _s(l) > Ja ? (c = Q.builtIn("RESPONSE_PAYLOAD_TOO_LARGE"), this.log.warn("RPC Response payload too large for ".concat(n))) : d = l;
      } catch (l) {
        l instanceof Q ? c = l : (this.log.warn("Uncaught error returned by RPC handler for ".concat(n, ". Returning APPLICATION_ERROR instead."), l), c = Q.builtIn("APPLICATION_ERROR"));
      }
  ...
  selectDefaultDevices() {
    return m(this, void 0, void 0, function* () {
      var e, t, n;
      const s = se.getInstance().previousDevices, r = yield se.getInstance().getDevices(void 0, !1), a = ue();
      ...
      for (let c of o) {
        const d = Yl(c), l = this.localParticipant.getTrackPublication(d);
        ...
        c === "audioinput" && !$t() || c === "videoinput" || u.length > 0 && !u.find((f) => f.deviceId === this.getActiveDevice(c)) && // avoid switching audio output on safari without explicit user action as it leads to slowed down audio playback
        (c !== "audiooutput" || !$t()) && (yield this.switchActiveDevice(c, u[0].deviceId));
      }
    });
  }
  acquireAudioContext() {
  ...
  createParticipant(e, t) {
    var n;
    let s;
    return t ? s = Zt.fromParticipantInfo(this.engine.client, t, {
      loggerContextCb: () => this.logContext,
      loggerName: this.options.loggerName
    }) : s = new Zt(this.engine.client, "", e, void 0, void 0, void 0, {
      loggerContextCb: () => this.logContext,
      loggerName: this.options.loggerName
  ...
  getOrCreateParticipant(e, t) {
    ...
    return this.remoteParticipants.set(e, n), this.sidToIdentity.set(t.sid, t.identity), this.emitWhenConnected(P.ParticipantConnected, n), n.on(I.TrackPublished, (s) => {
      ...
      s.kind === T.Kind.Audio ? (s.on(R.AudioPlaybackStarted, this.handleAudioPlaybackStarted), s.on(R.AudioPlaybackFailed, this.handleAudioPlaybackFailed)) : s.kind === T.Kind.Video && (s.on(R.VideoPlaybackFailed, this.handleVideoPlaybackFailed), s.on(R.VideoPlaybackStarted, this.handleVideoPlaybackStarted)), this.emit(P.TrackSubscribed, s, r, n);
      ...
      this.emitWhenConnected(P.ParticipantActive, n), n.kind === Pt.AGENT && this.localParticipant.setActiveAgent(n);
    }), t && n.updateInfo(t), n;
  }
  sendSyncState() {
  ...
  updateSubscriptions() {
    for (const e of this.remoteParticipants.values())
      for (const t of e.videoTrackPublications.values())
        t.isSubscribed && yu(t) && t.emitTrackUpdate();
  }
  ...
  registerConnectionReconcile() {
    ...
    this.connectionReconcileInterval = re.setInterval(() => {
      ...
      })), e >= 3 && (this.recreateEngine(), this.handleDisconnect(this.options.stopLocalTrackOnUnpublish, Ce.STATE_MISMATCH))) : e = 0;
    }, Bh);
  }
  ...
  emitWhenConnected(e) {
    for (var t = arguments.length, n = new Array(t > 1 ? t - 1 : 0), s = 1; s < t; s++)
      n[s - 1] = arguments[s];
    if (this.state === K.Reconnecting || this.isResuming || !this.engine || this.engine.pendingReconnect)
      this.bufferedEvents.push([e, n]);
    else if (this.state === K.Connected)
  ...
  simulateParticipants(e) {
    return m(this, void 0, void 0, function* () {
      ...
      if (this.handleDisconnect(), this.roomInfo = new Hn({
        sid: "RM_SIMULATED",
        name: "simulated-room",
        emptyTimeout: 0,
        maxParticipants: 0,
        creationTime: z.parse((/* @__PURE__ */ new Date()).getTime()),
        metadata: "",
        numParticipants: 1,
        numPublishers: 1,
        turnPassword: "",
        ...
        const c = new Bn(T.Kind.Video, new yt({
          source: te.CAMERA,
          sid: Math.floor(Math.random() * 1e4).toString(),
          type: Se.AUDIO,
          name: "video-dummy"
        }), new At(a.useRealTracks && (!((t = window.navigator.mediaDevices) === null || t === void 0) && t.getUserMedia) ? (yield window.navigator.mediaDevices.getUserMedia({
          video: !0
        })).getVideoTracks()[0] : Wi(160 * ((n = o.aspectRatios[0]) !== null && n !== void 0 ? n : 1), 160, !0, !0), void 0, !1, {
          loggerName: this.options.loggerName,
          loggerContextCb: () => this.logContext
        }), {
          loggerName: this.options.loggerName,
      ...
      if (a.audio) {
        const c = new Bn(T.Kind.Audio, new yt({
          source: te.MICROPHONE,
          sid: Math.floor(Math.random() * 1e4).toString(),
          type: Se.AUDIO
        }), new Ot(a.useRealTracks && (!((s = navigator.mediaDevices) === null || s === void 0) && s.getUserMedia) ? (yield navigator.mediaDevices.getUserMedia({
          audio: !0
        })).getAudioTracks()[0] : kn(), void 0, !1, this.audioContext, {
          loggerName: this.options.loggerName,
          loggerContextCb: () => this.logContext
        }), {
          loggerName: this.options.loggerName,
      ...
      for (let c = 0; c < o.count - 1; c += 1) {
        ...
        if (o.video) {
          const u = Wi(160 * ((r = o.aspectRatios[c % o.aspectRatios.length]) !== null && r !== void 0 ? r : 1), 160, !1, !0), h = new yt({
            source: te.CAMERA,
            sid: Math.floor(Math.random() * 1e4).toString(),
            type: Se.AUDIO
          });
  ...
  emit(e) {
    ...
    if (e !== P.ActiveSpeakersChanged && e !== P.TranscriptionReceived) {
      const r = sc(n).filter((a) => a !== void 0);
...
Mt.cleanupRegistry = typeof FinalizationRegistry < "u" && new FinalizationRegistry((i) => {
  i();
});
function sc(i) {
  return i.map((e) => {
    if (e)
      return Array.isArray(e) ? sc(e) : typeof e == "object" ? "logContext" in e ? e.logContext : void 0 : e;
  });
}
class qh {
...
var Hh = /* @__PURE__ */ Object.freeze({ __proto__: null, Convert: qh }), Te;
(function(i) {
  i[i.IDLE = 0] = "IDLE", i[i.RUNNING = 1] = "RUNNING", i[i.SKIPPED = 2] = "SKIPPED", i[i.SUCCESS = 3] = "SUCCESS", i[i.FAILED = 4] = "FAILED";
})(Te || (Te = {}));
class ze extends Fe.EventEmitter {
  constructor(e, t) {
    let n = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
    super(), this.status = Te.IDLE, this.logs = [], this.options = {}, this.url = e, this.token = t, this.name = this.constructor.name, this.room = new Mt(n.roomOptions), this.connectOptions = n.connectOptions, this.options = n;
  }
  run(e) {
    return m(this, void 0, void 0, function* () {
      if (this.status !== Te.IDLE)
...
class Wh extends ze {
  ...
  perform() {
    return m(this, void 0, void 0, function* () {
      ...
      this.appendMessage("upstream bitrate: ".concat((n.bitrateTotal / n.count / 1e3 / 1e3).toFixed(2), " mbps")), this.appendMessage("RTT: ".concat((n.rttTotal / n.count * 1e3).toFixed(2), " ms")), this.appendMessage("jitter: ".concat((n.jitterTotal / n.count * 1e3).toFixed(2), " ms")), n.packetsLost > 0 && this.appendWarning("packets lost: ".concat((n.packetsLost / n.packetsSent * 100).toFixed(2), "%")), n.qualityLimitationDurations.bandwidth > 1 && this.appendWarning("bandwidth limited ".concat((n.qualityLimitationDurations.bandwidth / (Ei / 1e3) * 100).toFixed(2), "%")), n.qualityLimitationDurations.cpu > 0 && this.appendWarning("cpu limited ".concat((n.qualityLimitationDurations.cpu / (Ei / 1e3) * 100).toFixed(2), "%"));
    });
  }
  getInfo() {
    const e = super.getInfo();
  ...
  checkConnectionProtocol(e) {
    return m(this, void 0, void 0, function* () {
      ...
      return yield new Promise((h) => setTimeout(h, Ei)), clearInterval(u), o.stop(), t.remove(), yield this.disconnect(), l;
    });
  }
}
class Jh extends ze {
...
class Gh extends ze {
  ...
  perform() {
    return m(this, void 0, void 0, function* () {
      var e;
      const t = yield this.connect(), n = yield ec();
  ...
  checkForVideo(e) {
    return m(this, void 0, void 0, function* () {
      ...
      n.srcObject = t, n.muted = !0, n.autoplay = !0, n.playsInline = !0, n.setAttribute("playsinline", "true"), document.body.appendChild(n), yield new Promise((s) => {
        n.onplay = () => {
          setTimeout(() => {
            var r, a, o, c;
            ...
            f.drawImage(n, 0, 0);
            const v = f.getImageData(0, 0, d.width, d.height).data;
            let E = !0;
            for (let k = 0; k < v.length; k += 4)
              if (v[k] !== 0 || v[k + 1] !== 0 || v[k + 2] !== 0) {
                E = !1;
                break;
              }
            E ? this.appendError("camera appears to be producing only black frames") : this.appendMessage("received video frames"), s();
          }, 1e3);
        }, n.play();
...
class Yh extends ze {
  ...
  perform() {
    return m(this, void 0, void 0, function* () {
      var e, t, n;
      It(new URL(this.url)) && (this.appendMessage("Using region specific url"), this.url = (e = yield new H(this.url, this.token).getNextBestRegionUrl()) !== null && e !== void 0 ? e : this.url);
      const s = new ks(), r = yield s.join(this.url, this.token, {
        autoSubscribe: !0,
        maxRetries: 0,
        e2eeEnabled: !1,
        websocketTimeout: 15e3
      ...
      let a = !1, o = !1, c = !1;
      for (let d of r.iceServers)
        for (let l of d.urls)
          l.startsWith("turn:") ? (o = !0, c = !0) : l.startsWith("turns:") && (o = !0, c = !0, a = !0), l.startsWith("stun:") && (c = !0);
      c ? o && !a && this.appendWarning("TURN is configured server side, but TURN/TLS is unavailable.") : this.appendWarning("No STUN servers configured on server side."), yield s.close(), !((n = (t = this.connectOptions) === null || t === void 0 ? void 0 : t.rtcConfig) === null || n === void 0) && n.iceServers || o ? yield this.room.connect(this.url, this.token, {
        rtcConfig: {
...
class Qh extends ze {
  ...
  perform() {
    return m(this, void 0, void 0, function* () {
      ...
      this.room.on(P.SignalConnected, () => {
        ...
        this.room.engine.client.onTrickle = (r, a) => {
          if (r.candidate) {
            ...
            o.address && ($h(o.address) ? c += " (private)" : o.protocol === "tcp" && o.tcpType === "passive" ? (e = !0, c += " (passive)") : o.protocol === "udp" && (t = !0)), this.appendMessage(c);
          }
          s && s(r, a);
      ...
      try {
        yield this.connect(), F.info("now the room is connected");
      } catch (n) {
    ...
    return m(this, void 0, void 0, function* () {
      ...
      try {
        ...
        if (It(new URL(this.url))) {
          ...
          const c = yield new H(this.url, this.token).getNextBestRegionUrl();
...
class Zh extends Fe.EventEmitter {
  ...
  checkPublishAudio() {
    return m(this, void 0, void 0, function* () {
      return this.createAndRunCheck(Jh);
    });
  }
  checkPublishVideo() {
    return m(this, void 0, void 0, function* () {
  ...
  checkConnectionProtocol() {
    return m(this, void 0, void 0, function* () {
      const e = yield this.createAndRunCheck(Wh);
...
function q(i, e, t) {
  return (e = tm(e)) in i ? Object.defineProperty(i, e, {
    value: t,
    enumerable: !0,
    configurable: !0,
    writable: !0
...
function im(i) {
  if (Uint8Array.fromBase64)
    return Uint8Array.fromBase64(typeof i == "string" ? i : Xi.decode(i), {
      alphabet: "base64url"
    });
  let e = i;
  e instanceof Uint8Array && (e = Xi.decode(e)), e = e.replace(/-/g, "+").replace(/_/g, "/");
  try {
    return nm(e);
  } catch {
...
q(sm, "code", "ERR_JWT_CLAIM_VALIDATION_FAILED");
class rm extends he {
  constructor(e, t) {
    let n = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : "unspecified", s = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : "unspecified";
    super(e, {
...
q(mm, "code", "ERR_JWKS_NO_MATCHING_KEY");
class fm extends he {
  constructor() {
...
q(fm, "code", "ERR_JWKS_MULTIPLE_MATCHING_KEYS");
class pm extends he {
  constructor() {
...
q(gm, "code", "ERR_JWS_SIGNATURE_VERIFICATION_FAILED");
const vm = (i) => typeof i == "object" && i !== null;
function ym(i) {
  if (!vm(i) || Object.prototype.toString.call(i) !== "[object Object]")
    return !1;
...
function bm(i) {
  if (typeof i != "string") throw new Ze("JWTs must use Compact JWS serialization, JWT must be a string");
  const {
    1: e,
    length: t
  } = i.split(".");
  if (t === 5) throw new Ze("Only JWTs using Compact JWS serialization can be decoded");
  if (t !== 3) throw new Ze("Invalid JWT");
  if (!e) throw new Ze("JWTs must contain a payload");
  let n;
  try {
    n = im(e);
  } catch {
  ...
  let s;
  try {
    s = JSON.parse(Xi.decode(n));
  } catch {
    throw new Ze("Failed to parse the decoded payload as JSON");
  }
  if (!ym(s)) throw new Ze("Invalid JWT Claims Set");
  return s;
}
const Zi = 1e3, km = 60 * Zi;
function Tm(i) {
  const e = As(i.participantToken);
  if (!(e != null && e.nbf) || !(e != null && e.exp))
    return !0;
  const t = /* @__PURE__ */ new Date(), n = e.nbf * Zi, s = new Date(n), r = e.exp * Zi, a = new Date(r - km);
  return s <= t && a > t;
}
function As(i) {
  const e = bm(i), {
  ...
  return Object.assign(Object.assign({}, n), {
    roomConfig: e.roomConfig ? Dn.fromJson(e.roomConfig) : void 0
  });
}
function Sm(i, e) {
...
class ac extends oc {
  constructor() {
    super(...arguments), this.cachedFetchOptions = null, this.cachedResponse = null, this.fetchMutex = new ae();
  }
  ...
  shouldReturnCachedValueFromFetch(e) {
    return !(!this.cachedResponse || !Tm(this.cachedResponse) || this.isSameAsCachedFetchOptions(e));
  }
  getCachedResponseJwtPayload() {
    return this.cachedResponse ? As(this.cachedResponse.participantToken) : null;
  }
  fetch(e) {
    return m(this, void 0, void 0, function* () {
      ...
      try {
        ...
        const n = yield this.update(e);
        return this.cachedResponse = n, n.toJson();
      } finally {
        t();
      }
    });
...
class Em extends ac {
  ...
  update(e) {
    return m(this, void 0, void 0, function* () {
      ...
      let n;
      return t instanceof Promise ? n = yield t : n = t, Do.fromJson(n, {
    ...
    for (const a of Object.keys(e))
      switch (a) {
        ...
        case "participantAttributes":
          ...
          break;
        case "agentName":
          r.roomConfig = (n = r.roomConfig) !== null && n !== void 0 ? n : new Dn(), r.roomConfig.agents.length === 0 && r.roomConfig.agents.push(new wi()), r.roomConfig.agents[0].agentName = e.agentName;
          break;
        case "agentMetadata":
          r.roomConfig = (s = r.roomConfig) !== null && s !== void 0 ? s : new Dn(), r.roomConfig.agents.length === 0 && r.roomConfig.agents.push(new wi()), r.roomConfig.agents[0].metadata = e.agentMetadata;
          break;
        default:
          const o = a;
...
function Rm(i) {
  ...
  if ("facingMode" in s) {
    ...
    F.trace("rawFacingMode", {
      rawFacingMode: a
    }), a && typeof a == "string" && Im(a) && (r = {
      facingMode: a,
      confidence: "high"
    });
  }
  if (["low", "medium"].includes(r.confidence)) {
    F.trace("Try to get facing mode from device label: (".concat(n.label, ")"));
    const a = dc(n.label);
    a !== void 0 && (r = a);
  }
  return r;
}
const Nr = /* @__PURE__ */ new Map([["obs virtual camera", {
  facingMode: "environment",
  confidence: "medium"
}]]), wm = /* @__PURE__ */ new Map([["iphone", {
  facingMode: "environment",
  confidence: "medium"
}], ["ipad", {
  facingMode: "environment",
...
function dc(i) {
  ...
  if (t !== "")
    return Nr.has(t) ? Nr.get(t) : (e = Array.from(wm.entries()).find((n) => {
      let [s] = n;
      return t.includes(s);
...
const Om = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  DefaultReconnectPolicy: No,
  DeviceUnsupportedError: tn,
  DisconnectReason: Ce,
  get EncryptionEvent() {
    return Ke;
  },
  ...
  LocalParticipant: nc,
  LocalTrack: ws,
  LocalTrackPublication: Bn,
  LocalTrackRecorder: Ga,
  LocalVideoTrack: At,
  get LogLevel() {
    return Rt;
  },
  ...
  ParticipantKind: Pt,
  PublishDataError: Jl,
  PublishTrackError: ji,
  RemoteAudioTrack: Xa,
  RemoteParticipant: Zt,
  RemoteTrack: Is,
  RemoteTrackPublication: ic,
  RemoteVideoTrack: Za,
  Room: Mt,
  get RoomEvent() {
    return P;
  ...
  RpcError: Q,
  ScreenSharePresets: nn,
  SignalReconnectError: ct,
  SignalRequestError: Vi,
  SimulatedError: Wl,
  SubscriptionError: io,
  TokenSource: Pm,
  TokenSourceConfigurable: oc,
  TokenSourceFixed: rc,
  get Track() {
    return T;
  },
  ...
  TrackInvalidError: we,
  get TrackPublication() {
    return Ne;
  },
  TrackType: Se,
  UnexpectedConnectionState: $,
  UnsupportedServer: ga,
  VideoPreset: W,
  VideoPresets: wt,
  VideoPresets43: Nn,
  get VideoQuality() {
  ...
  areTokenSourceFetchOptionsEqual: Sm,
  asEncryptablePacket: Ba,
  attachToElement: lt,
  attributes: Hh,
  audioCodecs: va,
  compareVersions: Me,
  createAudioAnalyser: mu,
  createE2EEKey: Mu,
  createKeyMaterialFromBuffer: ja,
  createKeyMaterialFromString: Fa,
  createLocalAudioTrack: tc,
  createLocalScreenTracks: Fh,
  createLocalTracks: sn,
  createLocalVideoTrack: ec,
  decodeTokenPayload: As,
  deriveKeys: Au,
  detachTrack: ht,
  facingModeFromDeviceLabel: dc,
  facingModeFromLocalTrack: Rm,
  getBrowser: ue,
  getEmptyAudioStreamTrack: kn,
  getEmptyVideoStreamTrack: hu,
  getLogger: Le,
  importKey: Ou,
  isAudioCodec: fu,
  isAudioTrack: Ae,
  ...
  isE2EESupported: La,
  isInsertableStreamSupported: Ua,
  isLocalParticipant: Da,
  isLocalTrack: it,
  isRemoteParticipant: bu,
  isRemoteTrack: Fn,
  isScriptTransformSupported: jn,
  isVideoCodec: Ma,
  isVideoFrame: Iu,
  isVideoTrack: Ge,
  needsRbspUnescaping: xu,
  parseRbsp: Nu,
  protocolVersion: pa,
  ratchet: Du,
  setLogExtension: Ol,
  setLogLevel: Il,
  supportsAV1: _a,
  supportsAdaptiveStream: su,
  supportsAudioOutputSelection: ou,
  supportsDynacast: ru,
  supportsVP9: Pa,
  version: fa,
  videoCodecs: ya,
...
export {
  Sn as AuthenticationError,
  Bt as NetworkError,
  Cn as RateLimitError,
  Ds as SpotInfoAnalytics,
  bc as SpotInfoChat,
  Mm as SpotInfoClient,
  Je as SpotInfoError,
  kc as SpotInfoVoice,
  Oe as ValidationError,
  qt as VoiceConnectionError
};
